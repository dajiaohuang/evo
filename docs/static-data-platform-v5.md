...
`data/` is the version-controlled scientific layer. The 189 canonical entity concepts live in the navigation ontology, canonical range evidence has its own ledger, and shared references, claims, events, stories and the normalized PBDB snapshots remain single sources. Registry and package identity/taxonomy/range/review/locale/query-ledger files are generated projections; Perissodactyla narratives and topology use separate `.source.json` inputs. `npm run data:registry:check` regenerates projections in a temporary directory and requires byte-for-byte equality. The inventory is an explicit educational subset rather than a whole-life coverage claim. `atlas-core` is Core, Perissodactyla is a curator draft, and the other 22 scientific packages are generated scaffolds. None is currently represented as expert-reviewed or published-featured.

Package schema v5 separates `entityKind`, `contentLevel` and `externalResolutionStatus`, and keeps platform maturity separate from scientific maturity. Automated schema validation is not scientific review. The scientific ladder is `generated-scaffold → curator-draft → source-complete → expert-reviewed → published-featured`. A package without identified human scientific review and an explicit acceptance decision cannot be `expert-reviewed`; the featured gate additionally requires fit primary/review sources, locators, claim linkage and reviewer identity/scope/conflict metadata.
