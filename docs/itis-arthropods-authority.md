...
| Myriapoda | `crustaceans-insects`, `L2G4H` | TSN `563885` | 14,210 | 3,040 | 0 | 2 | 11,168 | 3,445 | 3 |
| **Total** | — | — | **1,135,834** | **280,789** | **3,148** | **873** | **851,024** | **42,507** | **161** |
...
The packages are mixed navigation packages, so their non-applicable remainders are not additive. Insecta, Crustacea and Myriapoda remain separate exact scopes in `crustaceans-insects`; the Myriapoda sidecar excludes the one `Euthycarcinoidea` COL record and retains only 14,210 living COL rows. COL26.8 has no accepted Chilopoda species below that root, so Chilopoda is represented only where it occurs in the separate ITIS-only partition. Chelicerata is the living branch of `trilobites-chelicerates`; all 4,615 Trilobita records remain explicitly non-applicable because ITIS has no current Trilobita authority branch.
...
- Android and iOS build `25` use `native-full`: they copy every one of the 161 non-empty JSONL gzip files byte-for-byte from the immutable inventory. This is 1,135,834 COL outcomes plus 42,507 null-COL ITIS-only current species, or 1,178,341 native authority records.
