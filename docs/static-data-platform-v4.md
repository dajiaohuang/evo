...
`data/` is the version-controlled scientific layer. The 189 canonical entity concepts live in the navigation ontology, canonical range evidence has its own ledger, and shared references, claims, events, stories and the normalized PBDB snapshots remain single sources. Registry and package identity/taxonomy/range/review/locale files are generated projections; Perissodactyla narratives and topology now use separate `.source.json` inputs. `npm run data:registry:check` regenerates projections in a temporary directory and requires byte-for-byte equality. The inventory is an explicit educational subset rather than a whole-life coverage claim. `atlas-core` is Core, Perissodactyla is a curated draft, and the other 22 scientific packages are generated scaffolds. None is currently represented as expert-reviewed or Gold.
