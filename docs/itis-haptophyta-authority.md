...
infer a Haptophyta subset from individual-name overlap or from a different
...
COL release supplies an exact, auditable Haptophyta root, the generator fails
...
`scripts/build-itis-haptophyta-sidecar.mjs` verifies the pinned ITIS SQLite
...
Haptophyta node, generator checksum and output checksums. The gzip member is
