...
原生“数据”页直接显示安装包内置的文件数与体积，不再提供把同一数据重复写入 WebView Cache Storage 的按钮。当前 rc55 交互集继续包含 rc54 的完整 PBDB、CAO2024 与 COL26.8 字节，并加入 23 份来源限定研究预设；七个命名包清单、14 个物种分片和共享来源账本都由同一发布清单自动纳入。精确文件数与体积由构建时发布清单计算并显示，不写死到客户端代码。浏览器版对富内容包保留单包与全包缓存，对命名包提供逐包离线保存和 ZIP 下载，并继续提供“保存完整图谱”。
