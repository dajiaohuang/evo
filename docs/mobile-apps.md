...
原生“数据”页直接显示安装包内置的文件数与体积，不再提供把同一数据重复写入 WebView Cache Storage 的按钮。当前 rc54 交互集为 3,810 个文件、537.47 MiB；七个命名包清单、14 个物种分片、共享来源账本和 rc54 更新后的 PBDB 查询数据都由同一发布清单自动纳入。实际文件数与体积仍由构建时发布清单计算，不写死到客户端代码。浏览器版对富内容包保留单包与全包缓存，对命名包提供逐包离线保存和 ZIP 下载，并继续提供“保存完整图谱”。
