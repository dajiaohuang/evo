...
当前核心内容边界：`data/pages-preview.json` 是 App 与 GitHub Pages 静态预览共用的唯一精选清单，包含 Atlas 核心、灵长目重点包、奇蹄目、鲸偶蹄目和恐龙。灵长目排在核心优先故事和内容包前列。当前范围为 5 个内容包、102 个类群/导航节点、8 个故事和 44 个事件；七份灵长目策展档案与精选灵长类故事属于核心内容。59 个分片中的 6,957 条全物种来源档案、完整物种注册表和权威来源行继续留在 canonical/Web 侧，不进入 Android/iOS 安装包或 Pages 静态预览。安装包检查会拒绝完整 COL hierarchy/search、命名来源记录和 ZIP 下载文件，并以 384 MiB 作为硬上限。
...
同批新增 96 条 COL source-1033 Ichthyosporea 的 Trichomycetes 原始来源投影，保留名称、作者、历史分类与命名文献；66 个原始空标题不补写。完整数据配置包含全部 96 条，Pages 仅保留摘要。这不是新增物种、独立科学佐证或完整生物档案；新前端的展示入口由独立前端项目接续实现。见 [来源与重建方法](trichomycetes-archive.md)。

rc113 / app `0.20.64` / build `67` 补上两栖类、弹尾类／原尾类、28 个其他动物范围与 8 个具有 COL 记录的原生生物范围的默认收起 ITIS 详情。卵菌类严格限于既有四目投影的 1,494 条，不扩为整个门。

在“数据”页面展开“浏览 ITIS 来源记录”，依次选择资源包、ITIS 集合、COL／来源独有分区及文件，即可在 Android/iOS 内置数据中分页浏览；查找功能只搜索当前所选文件。真菌与细菌的独立 ITIS 集合、没有 COL 对应的来源独有类群也可从这里访问。网页版保留摘要，不读取明细分片；来源独有记录不自动归入 COL，也不计为全球去重新增物种。此版本不增加科学行、不替代 Index Fungorum 或 LPSN、不改变评审状态。历史迭代见 [数据更新记录](../data/CHANGELOG.md)。
...
rc108 / app `0.20.59` / build `62` 修复基础资源包及独立来源重建时的内容保留问题。两端继续包含全部 62 个资源包权威扩展及其数据分片，Pages 保持轻量；本次不增加物种或改变科学结论。重建规则及历史来源输入的保留方式见[资源包重建说明](resource-pack-rebuilds.md)。

rc107 / app `0.20.58` / build `61` 增加“其他动物”包中的独立 WoRMS 环节动物档案对应层。该范围内全部 18,982 条 COL 接受种记录都有明确结果，来源独有的接受种另行保留且不分配 COL ID；既有 ITIS 对应层不被替换。两端包含全部新增记录分片，详情按单个 COL-ID 区间读取，来源独有记录单独展开。Pages 仅保留摘要和完整文件清单；不同来源的行数不能相加声称为去重后的全球物种总数。见[范围及来源说明](worms-annelida-archive.md)。
...
rc103 将两端同步到 build `57` / app `0.20.54`，完整交付六个资源包新增的 18 个双语研究场景，共 312 个场景、513 条场景—主张链接、258 个年龄入口。地图路由提供类群/地质时期背景，关联档案承载论文标本、实验和模型证据，不冒充已定位的论文采样点。原生全量与 Pages 轻量数据的分流保持不变。

rc102 将 Android 与 iOS 同步到 build `56` / app `0.20.53`。图谱核心与软体动物/腕足动物各新增 3 个来源限定场景；24 个富内容包至此全部至少有 12 个场景，共 294 个研究场景、480 条场景—主张链接和 240 个年龄驱动入口。局部群落、足迹解释、竞争性系统发育模型、发育比较与现生观察均保留来源边界，不代表全球首末现、直接祖先或精确化石—地图共定位。完整原生数据与 Pages 轻量数据的分流保持不变。

rc101 将 Android 与 iOS 同步到 build `55` / app `0.20.52`。两端完整获得恐龙、甲壳类与昆虫、三叶虫与螯肢动物新增的 9 张来源限定地图卡，三个包均达到 12 个场景，原生包合计 288 个研究场景和 470 条场景—主张链接。全部 235 个年龄驱动场景在路由年龄命中共享实体和主张的 `available` 范围；具名标本、地层包络、分类矩阵、功能解释和生态重建继续与直接观察、全球首末现、完整分布、直接祖先及精确化石—地图共定位分开。`native-full` 继续逐字节保留 COL26.8 全部 2,183,133 个接受种、全部权威分片和 109 帧 0.1° PaleoDEM，Pages 保持 `web-light`。

rc100 将 Android 与 iOS 同步到 build `54` / app `0.20.51`。两端完整获得四足登陆、食肉目和棘皮动物新增的 9 张来源限定地图卡，三个包分别达到 12、13、13 个场景，原生包合计 279 个研究场景和 449 条场景—主张链接。全部 226 个年龄驱动场景在路由年龄命中共享实体和主张的 `available` 范围；具名标本、地层包络、竞争性亲缘、功能解释和时钟模型继续与直接观察、全球首末现、完整分布、直接祖先及精确化石—地图共定位分开。`native-full` 继续逐字节保留 COL26.8 全部 2,183,133 个接受种、全部权威分片和 109 帧 0.1° PaleoDEM，Pages 保持 `web-light`。

rc99 将 Android 与 iOS 同步到 build `53` / app `0.20.50`。两端完整获得其他哺乳类、灵长类和裸子植物新增的 10 张来源限定地图卡，三个包分别达到 12、13、12 个场景，原生包合计 270 个研究场景和 436 条场景—主张链接。全部 217 个年龄驱动场景在路由年龄命中共享实体和主张的 `available` 范围；分子与生物地理模型、总证据矩阵、采样解剖、区域记录和取整年龄继续与直接观察、全球首末现、完整分布、直接祖先及精确化石—地图共定位分开。`native-full` 继续逐字节保留 COL26.8 全部 2,183,133 个接受种、全部权威分片和 109 帧 0.1° PaleoDEM，Pages 保持 `web-light`。

rc98 将 Android 与 iOS 同步到 build `52` / app `0.20.49`。两端完整获得软骨鱼类、被子植物和早期陆生植物新增的 11 张来源限定地图卡，三个包分别达到 12、13、13 个场景，原生包合计 260 个研究场景和 411 条场景—主张链接。全部 207 个年龄驱动场景在路由年龄命中共享实体和主张的 `available` 范围；系统发育与时钟模型、代用指标组合、采样解剖、区域记录和取整年龄继续与直接观察、全球首末现、完整分布、直接祖先及精确化石—地图共定位分开。`native-full` 继续逐字节保留 COL26.8 全部 2,183,133 个接受种、全部权威分片和 109 帧 0.1° PaleoDEM，Pages 保持 `web-light`。

rc97 将 Android 与 iOS 同步到 build `51` / app `0.20.48`。两端完整获得奇蹄目、鲸偶蹄目、龟鳖与有鳞类新增的 11 张来源限定地图卡，三个包分别达到 12、12、13 个场景，原生包合计 249 个研究场景和 382 条场景—主张链接。全部 196 个年龄驱动场景在路由年龄命中共享实体和主张的 `available` 范围；竞争性拓扑、tip-dating 后验、比较样本、区域记录与取整年龄继续与直接观察、全球首末现、完整分布、直接祖先及精确化石—地图共定位分开。`native-full` 继续逐字节保留 COL26.8 全部 2,183,133 个接受种、全部权威分片和 109 帧 0.1° PaleoDEM，Pages 保持 `web-light`。

rc96 将 Android 与 iOS 同步到 build `50` / app `0.20.47`。两端完整获得鳄形类与鸟类、哺乳动物起源、海绵与刺胞动物新增的 12 张来源限定地图卡，三个包均达到 12 个场景，原生包合计 238 个研究场景和 358 条场景—主张链接。全部 185 个年龄驱动场景在路由年龄命中共享实体和主张的 `available` 范围；模型、比较窗口、孤立材料、地点、地层与取整年龄继续与直接观察、全球首末现、完整分布、直接祖先及精确化石—地图共定位分开。`native-full` 继续逐字节保留 COL26.8 全部 2,183,133 个接受种、全部权威分片和 109 帧 0.1° PaleoDEM，Pages 保持 `web-light`。

rc95 将 Android 与 iOS 同步到 build `49` / app `0.20.46`。两端完整获得早期鱼类、辐鳍鱼类与两栖类新增的 12 张来源限定地图卡，三个包均达到 12 个场景，原生包合计 226 个研究场景和 329 条场景—主张链接。全部 173 个年龄驱动场景在路由年龄命中共享实体和主张的 `available` 范围；标本、地点、层位、取整年龄、采样和模型边界继续与全球首末现、完整分布、观察行为、直接祖先及精确化石—地图共定位分开。`native-full` 继续逐字节保留 COL26.8 全部 2,183,133 个接受种、全部权威分片和 109 帧 0.1° PaleoDEM，Pages 保持 `web-light`。

rc94 将 Android 与 iOS 同步到 build `48` / app `0.20.45`。两端完整获得恐龙、软体动物与腕足动物、甲壳类与昆虫新增的 11 张来源限定地图卡，三个包均达到 9 个场景，原生包合计 214 个研究场景和 302 条场景—主张链接。全部 161 个年龄驱动场景都在路由年龄命中一个共享实体和主张的 `available` 范围；具名标本、地点与地层证据不会把食性、同源、功能、生态或系统发育解释冒充为直接观察，也不会外推全球首末现、完整分布或直接祖先。`native-full` 继续逐字节保留 COL26.8 全部 2,183,133 个接受种、全部权威分片和 109 帧 0.1° PaleoDEM，Pages 保持 `web-light`。

rc93 将 Android 与 iOS 同步到 build `47` / app `0.20.44`。两端完整获得全生命图谱核心、四足动物过渡、三叶虫与螯肢类新增的 18 张来源限定地图卡，三个包均由 3 个场景扩展到 9 个，原生包合计 203 个研究场景和 288 条场景—主张链接。全部 150 个年龄驱动场景都在路由年龄命中一个共享实体和主张的 `available` 范围；足迹、功能与同源解释、系统发育位置、活动模型和取整年龄不被冒充为观察行为、全球首末现或直接祖先。`native-full` 继续逐字节保留 COL26.8 全部 2,183,133 个接受种、全部权威分片和 109 帧 0.1° PaleoDEM，Pages 保持 `web-light`。

rc92 将 Android 与 iOS 同步到 build `46` / app `0.20.43`。两端完整获得早期陆生植物、裸子植物与被子植物新增的 18 张来源限定地图卡，三个包均由 3 个场景扩展到 9 个，原生包合计 185 个研究场景和 260 条场景—主张链接。全部 132 个年龄驱动场景都在路由年龄命中一个共享实体和主张的 `available` 范围；非花粉校准与其他模型窗口不会冒充化石记录，具名标本、地点、层位、功能解释与取整年龄均保持边界。`native-full` 继续逐字节保留 COL26.8 全部 2,183,133 个接受种、全部权威分片和 109 帧 0.1° PaleoDEM，Pages 保持 `web-light`。
