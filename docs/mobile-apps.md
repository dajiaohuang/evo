...
客户端先从应用资源读取 `current.json`，再读取 `releases/<datasetVersion>/` 下的不可变数据。清单和分片仍经过现有 SHA-256 与版本一致性检查。Android、iOS 与 Web 不存在内容白名单或精简版数据注册表：三端都能访问 Core、全部资源包、全局化石、全部 1,889 个 CAO2024 几何帧、44,175 条 CAO2024 观测/约束点和完整 COL 名录。

原生“数据”页直接显示安装包内置的文件数与体积，不再提供把同一数据重复写入 WebView Cache Storage 的按钮。当前 rc52 交互集为 3,788 个文件、约 528.95 MiB，包含全部 20 个 CAO2024 观测分片；实际值始终从发布清单计算，不写死到代码。浏览器版仍保留单包、全部资源包和“保存完整图谱”两级显式缓存能力。
