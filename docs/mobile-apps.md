...
rc112 / app `0.20.63` / build `66` 将其余 15 个 typed ITIS 包范围接入物种页，与既有三个范围共用默认收起的来源详情。展开后只读取命中的 COL-ID 分片，Web 仍仅显示摘要；所有范围保留真实谱系及资源归属限制。软体动物包读取计数同步到既有 159,801 条结果与 7,219 个接受名对应，含七个 Rhabdopleura 物种的名称对应，不主张其属于软体动物或腕足动物。本版不增加科学数据行、不改变物种总数或审查状态。
