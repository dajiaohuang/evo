...
claim that the five groups or FishBase contain all known fish diversity.
...
| Sarcopterygii | `8VSMX` | `tetrapod-transition` | 8 |

The five roots are disjoint in the pinned hierarchy, for a total of 37,436
...
It returned a source identifier for all 37,436 COL species. The crosswalk keeps
...
service count and must not be substituted for the pinned 37,436 COL boundary.

## Reproducible artifacts

- `data/sources/fishbase-authority-crosswalk-col26.8.json.gz` is the canonical
  deterministic snapshot. It contains 37,436 direct records and no fabricated
  redirects, ambiguous matches, unmatched rows or withheld rows. Its SHA-256
  is `e41f560b231959b50d2026d105b7509dbc15cd86a5b82374124f14f76279e52c`.
...
- The standalone supplement is registered under `fishBaseIdentifiers` in
  ...
  It is intentionally not counted as an eighth base resource pack: its 37,436
...
- `scripts/fetch-fishbase-authority-crosswalk.mjs` walks the five roots and
...
The generated shard total is 37,436 records / 2,970,063 compressed bytes /
24,020,982 uncompressed JSONL bytes. Web-light integration may retain the
...
The focused suite verifies source pinning, five-root counts, one-to-one COL ↔
