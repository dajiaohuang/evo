...
RC158 / COL26.8（2026-08-20）。分类、来源原文、来源字段标签、入门概述、完整档案与专家评审分别计量，不能互相替代。
...
10. 对 SANBI 短页码候选做可复现的元数据筛查；4,960 种 / 6,149 个来源 ID 对仅是 triage，不视为已验证证据。人工核对 *Cullumia cirsioides* `32DNH`、*C. decurrens* `32DNJ`、*C. floccosa* `32DNK`、*C. reticulata* `32DNV` 与 *C. rigida* `32DNW` 的 WFO 精确映射和 Strelitzia 29（2012）第 367 页各自账户；每种仅形态、花期、栖地和区域分布部分支持，其余主题未评估。排除引用章节范围过广且页中实际描述另一物种的候选。方法与限制见 [SANBI 来源审计](sanbi-descriptions.md)。
...
当前 55 条概述属于有来源的入门介绍，尚不是完整生活史、系统发育、化石、性状、分布、保育与媒体档案。逐主题档案目前有红狐、`Ctenium concinnum` 和四种 Cullumia，共 7 种；每种 Cullumia 仅有四个部分支持主题，其他三项未评估。完整档案与外部领域专家评审仍为 0。
