...
10. 审核 SANBI 两条形态/生境记录的同来源、同引文、短页码候选队列；4,973 种 / 6,168 个来源 ID 对仅作 triage，不视为已验证证据。对 `Cullumia cirsioides` `32DNH` 核实 WFO 精确映射及 Strelitzia 29（2012）第 367 页，新增形态、花期、栖地、区域分布四个部分支持主题；其余三主题未评估。方法与限制见 [SANBI 来源审计](sanbi-descriptions.md)。
...
当前 55 条概述属于有来源的入门介绍，尚不是完整生活史、系统发育、化石、性状、分布、保育与媒体档案。逐主题档案目前有红狐、`Ctenium concinnum` 与 `Cullumia cirsioides` 共 3 种；Cullumia 有四个部分支持主题，其他三项未评估。完整档案与外部领域专家评审仍为 0。
