...
RC157 / COL26.8（2026-08-20）。分类、来源原文、来源字段标签、入门概述、完整档案与专家评审分别计量，不能互相替代。
...
10. 审核 SANBI 两条形态/生境记录的同来源、同引文、短页码候选队列；4,973 种 / 6,168 个来源 ID 对仅作 triage，不视为已验证证据。对 `Cullumia cirsioides` `32DNH` 与 `Cullumia decurrens` `32DNJ` 核实 WFO 精确映射及 Strelitzia 29（2012）第 367 页，各新增形态、花期、栖地、区域分布四个部分支持主题；其余主题未评估。排除引用章节范围过广且页中实际描述另一物种的候选。方法与限制见 [SANBI 来源审计](sanbi-descriptions.md)。
...
当前 55 条概述属于有来源的入门介绍，尚不是完整生活史、系统发育、化石、性状、分布、保育与媒体档案。逐主题档案目前有红狐、`Ctenium concinnum`、`Cullumia cirsioides` 与 `Cullumia decurrens` 共 4 种；两种 Cullumia 各有四个部分支持主题，其他三项未评估。完整档案与外部领域专家评审仍为 0。
