...
The map contains 12 build-time, six-layer snapshots from the CAO2024 global plate model (Cao et al. 2024, v2.4, DOI `10.5281/zenodo.13628813`, CC BY 4.0), reconstructed at the midpoint of each ICS 2026/06 geological period. Each snapshot combines reconstructed coastlines, resolved topological plate polygons, typed plate-boundary lines, modelled continental-crust polygons, continent–ocean boundary (COB) outlines and rigid static reconstruction partitions. Boundary features retain the service's classification and, where supplied, its `Left` or `Right` subduction-polarity attribute. The undocumented `length` response attribute is deliberately omitted because the service documentation does not establish a unit. The three detailed technical layers default off and load independently when selected.

The import records model, anchor plate, retrieval URL/date, processing-script commit, raw-response checksum and normalized-geometry checksum for every layer. It also distinguishes the mutable GWS model descriptor and mirror archive from the fixed Zenodo v2.4 record and records their hashes instead of treating the alias as immutable. New polygon and line families are clipped at the antimeridian before publication, with split-feature counts retained per layer and period so a two-dimensional map never draws a false world-spanning segment. Continental polygons represent modelled continental-crust extent, not exposed land, coastline, elevation or water depth. COBs represent interpreted continental–ocean crust transitions, not coastlines or active plate boundaries. Static polygons are rigid reconstruction partitions used for plate-ID assignment, not dynamic topological plate coverage. These are representative-age geometries, not direct observations, a continuous movie or an ancient elevation surface. Reconstruction uncertainty and model dependence increase into deep time.
