...
The descriptor and import ledger copy the pinned metadata's exact title, version, version DOI and citation HTML. They also record bytes and SHA-256 for every member of the ZIP archive, including members not used to build the compact crosswalk, so the source snapshot can be audited without inventing metadata.
