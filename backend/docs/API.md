...
`GET /v1/sync/files?profile=full` returns a sorted, stable file page. Each descriptor includes `path`, `profile`, `bytes`, `sha256`, `mediaType`, `encoding`, `releaseVersion`, `deltaFrom`, and is addressable below `/v1/resources/`. `limit` defaults to 500 and is capped at 5,000. `nextCursor` resumes the exact file list. `since=<current datasetVersion>` returns `upToDate: true` with no files; other versions receive the current full inventory and each descriptor's `deltaFrom` records the requested version.
