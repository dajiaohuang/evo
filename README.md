...
`rc58` 首次发布 8 幅证据锚定的 AI 辅助二维解释性复原：Asteroxylon、Eocyathispongia、Kimberella、Waptia、Shenacanthus、Tiktaalik、Ambulocetus 与 Archaeopteryx。每幅 WebP 都永久显示中英解释性标签与不确定性，不充当标本照片、比例图或直接证据；prompt、工作流、模型与许可哈希、固定 seed、拒收候选、输出 SHA-256 和人工原图复核均保留。相同字节经 manifest、包 ZIP、浏览器离线存储和 release inventory 进入 Web、Android 与 iOS，同时完整保留 rc57 的 790 条 LPSN 标识侧车。Android `versionCode` 与 iOS build number 同步为 `12`。
