...
当前开发数据快照为 `2026.08-static-v5-rc62`，Web/Android/iOS 客户端版本为 `0.20.13`。精确记录数、校验和限制以 [`data/manifest.json`](data/manifest.json) 为准；科学内容变更见 [`data/CHANGELOG.md`](data/CHANGELOG.md)。

部署最多尝试保留两份历史数据版，但实际数量受 release retention byte budget 约束；只有部署产物 `data/releases.json` 当前列出的版本保证可达。rc58 因当前大体积发布超出历史抓取预算而未被 rc60 重建或伪造。
