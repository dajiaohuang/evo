...
const descriptorPath = join(nomenclatureRoot, 'itis-tunicata-cephalochordata-sidecar.json')
const ledgerPath = join(root, 'data/sources/itis-tunicata-cephalochordata-sidecar-import-ledger.json')
const ITIS_ROOTS = [
  { tsn: 158853, scientificName: 'Urochordata', alias: 'Tunicata', aliasTsn: 203347 },
  { tsn: 914170, scientificName: 'Cephalochordata', alias: null },
]
const COL_ROOTS = [
  { id: '7NF2Z', scientificName: 'Tunicata' },
  { id: '7NF2Q', scientificName: 'Cephalochordata' },
]
...
const synonymQuery = `WITH RECURSIVE roots(tsn) AS (VALUES (?1), (?2)), descendants(tsn) AS (
  ...
  SELECT u.tsn FROM taxonomic_units u JOIN descendants d ON u.parent_tsn = d.tsn
  WHERE u.name_usage = 'valid'
), accepted_target_species(tsn) AS (
...
JOIN accepted_target_species a ON a.tsn = s.tsn_accepted
...
async function loadColSpecies(manifest) {
  ...
  for (const path of files) await eachGzipJsonLine(path, (record) => {
    ...
    while (ancestor) {
      const root = COL_ROOTS.find((candidate) => candidate.id === ancestor)
      if (root) { matchedRoot = root; break }
      const parent = parents.get(ancestor)
      if (parent === undefined) throw new Error(`COL hierarchy is broken for ${record.id} at ${ancestor}`)
      ancestor = parent
    }
    if (matchedRoot) result.push({ ...record, scopeRootUsageId: matchedRoot.id, scopeRootScientificName: matchedRoot.scientificName })
  })
...
function loadItis(sqlitePath) {
  ...
  try {
    const rootRecords = ITIS_ROOTS.map((expected) => database.prepare(`SELECT u.tsn, l.completename, r.rank_name, u.name_usage
      FROM taxonomic_units u JOIN longnames l ON l.tsn = u.tsn
      JOIN taxon_unit_types r ON r.kingdom_id = u.kingdom_id AND r.rank_id = u.rank_id
      WHERE u.tsn = ?1`).get(expected.tsn))
    const aliasRecords = ITIS_ROOTS.map((expected) => expected.aliasTsn ? database.prepare(`SELECT u.tsn, l.completename, r.rank_name, u.name_usage
      FROM taxonomic_units u JOIN longnames l ON l.tsn = u.tsn
      JOIN taxon_unit_types r ON r.kingdom_id = u.kingdom_id AND r.rank_id = u.rank_id
      WHERE u.tsn = ?1`).get(expected.aliasTsn) : null)
    for (const [index, rootRecord] of rootRecords.entries()) {
      const expected = ITIS_ROOTS[index]
      if (!rootRecord || rootRecord.completename !== expected.scientificName || rootRecord.rank_name !== 'Subphylum' || rootRecord.name_usage !== 'valid') throw new Error(`Pinned ITIS ${expected.scientificName} root no longer has the expected valid subphylum identity`)
      const aliasRecord = aliasRecords[index]
      if (expected.aliasTsn && (!aliasRecord || aliasRecord.completename !== expected.alias || aliasRecord.rank_name !== 'Subphylum' || aliasRecord.name_usage !== 'invalid')) throw new Error(`Pinned ITIS ${expected.alias} alias no longer has the expected invalid subphylum identity`)
      if (expected.aliasTsn && !database.prepare('SELECT 1 FROM synonym_links WHERE tsn = ?1 AND tsn_accepted = ?2').get(expected.aliasTsn, expected.tsn)) throw new Error(`Pinned ITIS ${expected.alias} alias no longer resolves to ${expected.scientificName}`)
    }
    ...
    return { rootRecords, aliasRecords, maxima, currentRows: database.prepare(currentSpeciesQuery).all(...ITIS_ROOTS.map((root) => root.tsn)), synonymRows: database.prepare(synonymQuery).all(...ITIS_ROOTS.map((root) => root.tsn)) }
  } finally { database.close() }
}

async function main() {
  const argument = process.argv.indexOf('--itis-sqlite')
  if (argument < 0 || !process.argv[argument + 1]) throw new Error('Usage: node scripts/build-itis-tunicata-cephalochordata-sidecar.mjs --itis-sqlite <verified ITIS.sqlite>')
  ...
  if (colSpecies.some((record) => !packIds.has(record.id))) throw new Error('Tunicata/Cephalochordata species are not all present in the declared Other Animals resource pack')
  if (new Set(colSpecies.map((record) => record.scopeRootUsageId)).size !== COL_ROOTS.length) throw new Error('Both declared COL roots must contain at least one strict accepted species')
  const { rootRecords, aliasRecords, maxima, currentRows, synonymRows } = loadItis(sqlitePath)
  ...
  for (const colRecord of colSpecies) {
    const result = matchColSpecies(colRecord, index); const record = { status: result.status, ...result.record, scopeRootUsageId: colRecord.scopeRootUsageId, scopeRootScientificName: colRecord.scopeRootScientificName }; crosswalk.push(record)
  ...
  if (new Set(crosswalk.map((record) => record.colUsageId)).size !== colSpecies.length) throw new Error('COL Tunicata/Cephalochordata records are not uniquely addressable')
  const upstreamOnly = currentRows.filter((row) => !evidencedTsns.has(String(row.tsn))).map((row) => ({ colUsageId: null, currentName: currentName(row), basis: 'No strict COL26.8 Tunicata or Cephalochordata accepted-species name or official ITIS species-synonym evidence resolves to this current ITIS TSN.' })).sort((a, b) => Number(a.currentName.tsn) - Number(b.currentName.tsn))
  mkdirSync(nomenclatureRoot, { recursive: true })
  for (const name of readdirSync(nomenclatureRoot)) if (/^itis-tunicata-cephalochordata-(?:sidecar|upstream-only)-\d{4}\.jsonl\.gz$/u.test(name)) rmSync(join(nomenclatureRoot, name))
  const shards = chunkBySourceBytes(crosswalk).map((records, indexNumber) => {
    const filename = `itis-tunicata-cephalochordata-sidecar-${String(indexNumber).padStart(4, '0')}.jsonl.gz`; const sourcePayload = jsonlBytes(records); const compressed = Buffer.from(deterministicGzip(sourcePayload, { level: 9 })); const path = join(nomenclatureRoot, filename); writeFileSync(path, compressed); return outputDescriptor(path, records, compressed, sourcePayload)
  })
  const upstreamSource = jsonlBytes(upstreamOnly); const upstreamCompressed = Buffer.from(deterministicGzip(upstreamSource, { level: 9 })); const upstreamPath = join(nomenclatureRoot, 'itis-tunicata-cephalochordata-upstream-only-0000.jsonl.gz'); writeFileSync(upstreamPath, upstreamCompressed)
  ...
  const scopeDatasetCounts = Object.fromEntries(Object.entries(colSpecies.reduce((map, row) => { const key = row.sourceDatasetId ?? 'null'; map[key] = (map[key] ?? 0) + 1; return map }, {})).sort(([a], [b]) => Number(a) - Number(b)))
  const scopeCounts = Object.fromEntries(COL_ROOTS.map((root) => [root.id, colSpecies.filter((record) => record.scopeRootUsageId === root.id).length]))
  const descriptor = {
    schemaVersion: 1, sidecarType: 'release-pinned-exact-nomenclatural-crosswalk', packageId: PACKAGE_ID,
    scope: { colRoots: COL_ROOTS, colStrictAcceptedSpecies: colSpecies.length, speciesByColRoot: scopeCounts, packageStrictAcceptedSpecies: packageCount, packageOutOfScopeStrictAcceptedSpecies: packageCount - colSpecies.length, boundary: 'Other Animals is the deterministic Animalia remainder after more-specific static-package routes. This sidecar covers only strict accepted COL26.8 species descending from the exact Tunicata and Cephalochordata roots; every other Other Animals-owned species is explicitly nonapplicable.' },
    mixedResourcePack: { packageId: PACKAGE_ID, manifestPath: repoPath(resourcePackManifestPath), manifestSha256: sha256(resourcePackBytes), acceptedSpeciesCount: packSpecies.length, inScopeSpecies: colSpecies.length, outOfScopeSpecies: packSpecies.length - colSpecies.length, nonApplicableSpecies: packSpecies.length - colSpecies.length, packageSourceDatasetCounts: packDatasetCounts, scopeSourceDatasetCounts: scopeDatasetCounts, outOfScopeBoundary: `The remaining ${packSpecies.length - colSpecies.length} accepted species in Other Animals are outside Tunicata/Cephalochordata and receive no match in this sidecar.` },
    sources: { col: { releaseAlias: 'COL26.8', releaseDate: '2026-08-20', registryManifestPath: repoPath(registryManifestPath), registryManifestSha256: sha256(registryBytes), ownershipPath: repoPath(ownershipPath), ownershipSha256: sha256(ownershipBytes) }, itis: { datasetId: source.datasetId, exportDate: source.release.exportDate, roots: ITIS_ROOTS.map((item) => ({ tsn: String(item.tsn), scientificName: item.scientificName })), aliases: aliasRecords.filter(Boolean).map((item) => ({ tsn: String(item.tsn), scientificName: item.completename, usage: item.name_usage, resolvesToTsn: '158853' })), sourceLedgerPath: repoPath(sourcePath), sourceLedgerSha256: sha256(sourceBytes), license: source.license.spdx, citationDoi: source.citation.doi } },
    exactMatching: { normalization: source.importLedger.normalization, statuses: { accepted: 'The normalized COL name resolves to exactly one valid ITIS Tunicata or Cephalochordata species and directly equals that current ITIS name.', 'synonym-current-name-redirect': 'The normalized COL name equals official ITIS invalid species-name evidence that resolves to exactly one valid ITIS Tunicata or Cephalochordata species.', ambiguous: 'The normalized exact evidence resolves to more than one valid ITIS Tunicata or Cephalochordata species TSN.', unmatched: 'No normalized exact valid-name or official ITIS species-synonym evidence resolves to a valid ITIS Tunicata or Cephalochordata species.' }, prohibited: 'No fuzzy, edit-distance, phonetic, case-folded, diacritic-stripped, token-reordered or taxon-substituted matching is used.' },
  ...
  descriptor.evidenceBoundary = {
    en: 'This CC0 ITIS sidecar is a frozen exact nomenclatural crosswalk for Tunicata and Cephalochordata inside the mixed Other Animals resource pack. ITIS uses valid Urochordata as the Tunicata root in this export; invalid ITIS Tunicata is retained as an explicit alias. This is not a final classification authority, phylogeny, species-concept equivalence assertion, biological dossier or scientific-review record.',
    zh: '此 CC0 ITIS 侧车是混合“其他动物”资源包中尾索动物与头索动物分区的冻结严格命名交叉映射。该 ITIS 导出以有效名 Urochordata 作为 Tunicata 的根，并将无效名 Tunicata 明确保留为别名。它不是最终分类权威、系统发育树、物种概念等同性声明、生物档案或科学审查记录。',
  }
  const descriptorBytes = jsonBytes(descriptor); writeFileSync(descriptorPath, descriptorBytes)
  const ledger = { schemaVersion: 1, importType: 'COL26.8-to-ITIS-exact-tunicata-cephalochordata-nomenclatural-sidecar', generatedFrom: { sourcePath: repoPath(sourcePath), sourceSha256: sha256(sourceBytes), itisDatabaseMember: source.archive.databaseMember, itisDatabaseSha256: sqliteSha256, colRegistryManifestPath: repoPath(registryManifestPath), colRegistryManifestSha256: sha256(registryBytes), colOwnershipPath: repoPath(ownershipPath), colOwnershipSha256: sha256(ownershipBytes), resourcePackManifestPath: repoPath(resourcePackManifestPath), resourcePackManifestSha256: sha256(resourcePackBytes) }, scopeAudit: { ...descriptor.scope, mixedResourcePack: descriptor.mixedResourcePack, itisRoots: rootRecords.map((record) => ({ tsn: String(record.tsn), scientificName: record.completename, rank: record.rank_name, usage: record.name_usage })), itisCurrentSpecies: currentRows.length, itisSpeciesSynonymLinks: synonymRows.length, maximumUpdateDates: maxima }, matchingContract: descriptor.exactMatching, totals: descriptor.counts, output: { descriptor: { path: repoPath(descriptorPath), bytes: descriptorBytes.length, sha256: sha256(descriptorBytes) }, colUsageIdShards: shards, upstreamOnly: upstreamDescriptor }, deliveryContract: { pagesLight: 'Pages needs only this small descriptor and may omit all row-level JSONL gzip shards.', androidIosFull: 'Android and iOS complete-data inventories must include the descriptor and every listed row-level shard as the same checksum-addressed bytes.', runtimeChange: 'This import deliberately changes no formal runtime or published release manifest.' }, generatedBy: { scriptPath: 'scripts/build-itis-tunicata-cephalochordata-sidecar.mjs', scriptSha256: await sha256File(fileURLToPath(import.meta.url)), deterministic: 'Pinned input checksums, fixed roots, exact SQL, exact representation-only normalization, code-unit ID ordering and deterministic gzip; no wall-clock fields or fuzzy matching.' } }
