...
describe('ITIS Tunicata and Cephalochordata exact sidecar shards', () => {
  ...
  it('covers the exact COL26.8 Tunicata/Cephalochordata partition once in deterministic, non-overlapping ranges', () => {
    expect(descriptor.packageId).toBe('other-animals')
    expect(descriptor.scope.colRoots).toEqual([{ id: '7NF2Z', scientificName: 'Tunicata' }, { id: '7NF2Q', scientificName: 'Cephalochordata' }])
    expect(descriptor.scope.speciesByColRoot).toEqual({ '7NF2Z': 3146, '7NF2Q': 30 })
    expect(descriptor.scope.colStrictAcceptedSpecies).toBe(3176)
  ...
  it('retains only exact ITIS evidence and addresses each COL species through one shard', () => {
    expect(descriptor.sources.itis.roots).toEqual([
      { tsn: '158853', scientificName: 'Urochordata' },
      { tsn: '914170', scientificName: 'Cephalochordata' },
    ])
    expect(descriptor.sources.itis.aliases).toEqual([{ tsn: '203347', scientificName: 'Tunicata', usage: 'invalid', resolvesToTsn: '158853' }])
  ...
  it('separates ITIS-only current species and preserves byte-exact delivery evidence', () => {
    ...
    expect(upstream).toHaveLength(66)
    ...
    expect(descriptor.mixedResourcePack.nonApplicableSpecies).toBe(95985)
