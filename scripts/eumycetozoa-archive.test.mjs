...
test('source1053 retains 1330 exact matches and links seven differing source records by official ID', () => {
  ...
  assert.deepEqual(descriptor.counts, { total: 1337, accepted: 1337, redirect: 0, ambiguous: 0, unmatched: 0, withheld: 0, upstreamOnly: 0, records: 1337 })
  assert.equal(rows.length, 1337)
  assert.equal(new Set(rows.map((row) => row.colId)).size, 1337)
  assert.equal(matched.length, 1337)
  assert.equal(matched.filter((row) => row.mappingBasis.startsWith('Exact source')).length, 1330)
  assert.equal(matched.filter((row) => row.mappingBasis.includes('source-record relation')).length, 7)
  ...
  assert.equal(ledger.scopeAudit.matchedUniqueSourceAcceptedTaxonIds, 1337)
  assert.equal(ledger.scopeAudit.sourceReferenceLinks, 3926)
  assert.equal(ledger.scopeAudit.sourceReferences, 2932)
  assert.equal(ledger.scopeAudit.excludedUnlinkedAcceptedSourceIds.length, 8)
