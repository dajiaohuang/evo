...
export function integrateWormsNematoda({ rootDir = defaultRoot } = {}) {
  const releaseRoot = join(rootDir, 'data/catalogue-of-life/releases/2026-08-20')
  const packRoot = join(releaseRoot, 'resource-packs/other-animals')
  const descriptorPath = join(packRoot, 'worms-nematoda-sidecar.json')
  ...
  const extension = {
    ...
    source: {
      ...descriptor.source,
      canonicalDescriptorPath: 'data/catalogue-of-life/releases/2026-08-20/resource-packs/other-animals/worms-nematoda-sidecar.json',
  ...
  const pack = collection.packs.find((candidate) => candidate.packageId === 'other-animals')
...
if (process.argv[1] && resolve(process.argv[1]) === resolve(fileURLToPath(import.meta.url))) {
  const result = integrateWormsNematoda()
  console.log(`Integrated WoRMS Nematoda: ${result.records} records, ${result.fileCount} files; preserved ${result.extensionCount - 1} other authorities.`)
}
