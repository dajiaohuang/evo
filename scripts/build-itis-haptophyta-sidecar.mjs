...
const descriptorPath = join(outputRoot, 'itis-haptophyta-sidecar.json')
const outputLedgerPath = join(repositoryRoot, 'data', 'sources', 'itis-haptophyta-sidecar-import-ledger.json')
const rootTsn = 2134
...
function parseArgs(argv) {
  ...
  if (!options.help && !options.itisSqlitePath) throw new Error('Usage: node scripts/build-itis-haptophyta-sidecar.mjs --itis-sqlite <verified ITIS.sqlite> [--registry-root <path>] [--ownership <path>] [--itis-source-ledger <path>]')
  return options
}
...
async function auditColScope(registryRoot) {
  ...
  const exactHaptophytaNodes = []
  for (const file of manifest.hierarchy.nodes.files) {
    await eachGzipJsonLine(join(registryRoot, ...file.path.split('/')), (record) => {
      if (record.scientificName === 'Haptophyta') exactHaptophytaNodes.push(record)
    })
  }
  if (exactHaptophytaNodes.length !== 0) throw new Error(`The explicit zero-COL-root boundary must be revisited: found ${exactHaptophytaNodes.length} Haptophyta node(s) in COL26.8`)
  return { manifestPath, manifestBytes, exactHaptophytaNodes }
}
...
function loadItis(sqlitePath) {
  ...
  try {
    ...
    if (!root || root.scientific_name !== 'Haptophyta' || root.rank_name !== 'Division' || root.name_usage !== 'accepted') {
      throw new Error(`Pinned ITIS root is not the expected accepted Haptophyta division: ${JSON.stringify(root)}`)
    }
...
async function main() {
  ...
  if (options.help) { console.log('Usage: node scripts/build-itis-haptophyta-sidecar.mjs --itis-sqlite <verified ITIS.sqlite>'); return }
  ...
  const { manifestPath, manifestBytes, exactHaptophytaNodes } = await auditColScope(options.registryRoot)
  ...
  const upstreamOnly = currentRows.map((row) => ({
    ...
    basis: 'COL26.8 has no exact Haptophyta usage node, so no strict COL Haptophyta partition or exact crosswalk is claimed for this release.',
  }))
  ...
  for (const name of readdirSync(outputRoot)) if (/^itis-haptophyta-(?:sidecar|upstream-only)-\d{4}\.jsonl\.gz$/u.test(name)) rmSync(join(outputRoot, name))
  const upstreamPath = join(outputRoot, 'itis-haptophyta-upstream-only-0000.jsonl.gz')
  ...
  const scope = {
    ...
    colRootUsageId: null, colRootScientificName: 'Haptophyta', colStrictAcceptedSpecies: 0,
    ...
    colRootAudit: 'No exact COL26.8 usage node named Haptophyta exists in the complete pinned hierarchy. Therefore this release cannot derive a strict COL Haptophyta partition or infer one from name overlap.',
    boundary: 'This ITIS-only sidecar retains every current species below the accepted ITIS Haptophyta division. It deliberately contains no COL match rows until a pinned COL release materializes an exact auditable Haptophyta root.',
  }
  const descriptor = {
    ...
    exactMatching: {
      ...
      statuses: { accepted: 'Unavailable: COL26.8 has no exact Haptophyta root from which to derive a strict accepted-species partition.', 'synonym-current-name-redirect': 'Unavailable: no COL Haptophyta partition exists in this release.', ambiguous: 'Unavailable: no COL Haptophyta partition exists in this release.', unmatched: 'Unavailable: no COL Haptophyta partition exists in this release.' },
    ...
    evidenceBoundary: {
      en: 'This CC0 ITIS inventory is a frozen, ITIS-only nomenclatural partition. It is not a global haptophyte checklist, a final classification authority, a phylogeny, a species-concept equivalence assertion, a biological dossier or a scientific-review record.',
      zh: '此 CC0 ITIS 清单是冻结的、仅限 ITIS 的命名分区；它不是全球定鞭藻名录、最终分类权威、系统发育、物种概念等同性声明、生物档案或科学审查记录。',
    },
    counts,
    colUsageIdLocator: { key: 'colUsageId', ordering: 'Unicode code-unit ascending', stableAddressing: 'No COL row-level files exist because COL26.8 does not materialize an exact Haptophyta root.', files: [] },
    upstreamOnly: { colOwnership: null, stableAddressing: 'No COL usage ID is assigned. The complete current ITIS Haptophyta species partition is one immutable JSONL gzip shard.', files: [upstreamDescriptor] },
  }
  ...
  const ledger = {
    schemaVersion: 1, importType: 'ITIS-2026-08-26-Haptophyta-itis-only-inventory',
    ...
    scopeAudit: { ...scope, exactHaptophytaNodes, itisRoot: { tsn: String(root.tsn), scientificName: root.scientific_name, rank: root.rank_name, usage: root.name_usage }, itisCurrentSpecies: currentRows.length, itisSpeciesSynonymLinks: synonymRows.length, maximumUpdateDates: maxima },
    ...
    generatedBy: { scriptPath: 'scripts/build-itis-haptophyta-sidecar.mjs', scriptSha256: await sha256File(scriptPath), deterministic: 'Pinned input checksums, exact accepted-status SQL, explicit zero-COL-root audit, ascending TSN order and deterministic gzip; no wall-clock fields or fuzzy matching.' },
  }
