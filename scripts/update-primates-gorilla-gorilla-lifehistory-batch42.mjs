...
const SOURCE_PATH = join(ROOT, 'data', 'sources', 'primates-gorilla-gorilla-lifehistory-b42-2026-09-25.json')
...
const MANIFEST_PATH = join(ROOT, 'data', 'knowledge', 'primates-gorilla-gorilla-lifehistory-b42-2026-09-25.update-manifest.json')
const REGISTRY_ROOT = join(ROOT, 'data', 'catalogue-of-life', 'releases', '2026-08-20', 'registry')
const EXPECTED_SOURCE_SHA256 = '31b8cd6b86a6ab1d37f42344d6445e7701d66cb35813c7ca89809b8576a1e6da'
...
assert.equal(sha256(sourceBytes), EXPECTED_SOURCE_SHA256, 'Frozen B42 evidence source changed')
const source = JSON.parse(sourceBytes.toString('utf8'))
assert.equal(source.batchId, 'primates-gorilla-gorilla-lifehistory-batch42-2026-09-25')
...
function validateSourceClaim(dossier) {
  ...
  const claim = source.claims.lifeHistory
  ...
  for (const item of source.sources) {
    ...
    for (const key of ['stableId', 'rightsHolder', 'licenseVersion', 'licenseAppliesTo', 'attribution', 'scope']) {
      assert.ok(item[key], 'B42 source missing ' + key)
    }
...
assert.equal(indexed.count, source.audit.indexedRecordCount, 'Indexed record count changed since B42 identity audit')
const targetMatches = [...indexed.recordsById.values()].filter(row => row.colId === source.target.colId)
assert.equal(targetMatches.length, source.audit.targetRecordCount, 'B42 target must occur exactly once')
...
if (alreadyApplied) {
  ...
  assert.equal(priorManifest.input?.sha256, EXPECTED_SOURCE_SHA256, 'B42 was applied from a different input')
  assert.ok(baseline.facets.lifeHistory.claims.some(claim => JSON.stringify(claim) === JSON.stringify(source.claims.lifeHistory)))
  ...
  assert.ok(!source.sources.some(item => baseline.sources.some(existing => existing.id === item.id)), 'B42 source id already occurs on target')
  const allOccurrences = [...indexed.recordsById.values()].flatMap(record => (record.sources ?? []).map(item => ({ colId: record.colId, source: item })))
  for (const item of source.sources) {
    assert.equal(allOccurrences.filter(entry => entry.source.stableId === item.stableId).length, 0, 'B42 DOI is already indexed: ' + item.stableId)
  }
  ...
  dossier.facets.lifeHistory = {
    status: 'partially-supported',
    claims: [...(baseline.facets.lifeHistory.claims ?? []), structuredClone(source.claims.lifeHistory)],
    gaps: structuredClone(source.gaps.lifeHistory),
  }
  dossier.identity.scope = 'COL26.8 usage 3H3C9 only. Existing ecology evidence is from one western lowland group at Loango and two tool-use observations at Mbeli Bai; the evolutionary evidence is one model-based demographic history from 14 western lowland gorilla genomes. Female life-history estimates added here concern one free-ranging western lowland population at Mbeli Bai. These bounded samples do not constitute a species-wide or range-wide synthesis.'
  dossier.lifeStatusScope.wild = 'Ecology evidence is from free-ranging gorillas at Loango and Mbeli Bai; the life-history evidence is from female western lowland gorillas at one monitored Mbeli Bai population during 1995–2020. The genomic study used samples mostly from wild-caught zoo-held specimens and is not contemporary field sampling. No claim is extrapolated to every Gorilla gorilla population.'
  dossier.completeness = {
    status: 'incomplete',
    reasons: [
      'The record contains site-specific ecology, one model-based evolutionary estimate for sampled western lowland genomes, and female life-history estimates from one Mbeli Bai population; morphology, species-wide distribution, fossil evidence, and conservation remain unassessed.',
      'Systematic literature coverage across the accepted species concept and life-history variation among other populations and subspecies remain incomplete.',
  ...
  assert.ok(!rawBytes.includes(0x0d), 'B42 raw JSONL must use LF line endings')
  ...
  assert.deepEqual(roundTrip, rawBytes, 'B42 Brotli round trip must preserve exact raw bytes')
  ...
  const updateAudit = {
    ...
    openPullRequests: [],
    openPullRequestSearch: { query: '3H3C9', checkedAt: source.checkedAt, returned: 0 },
    mode: 'in-place-add-one-site-bounded-life-history-source-to-existing-record',
  ...
  const supplementalUpdate = {
    ...
    generator: relative(join(ROOT, 'scripts', 'update-primates-gorilla-gorilla-lifehistory-batch42.mjs')),
    baseHead: source.audit.baseHead,
    indexedRecordCountAtAudit: indexed.count,
  ...
  assert.ok(!metadata.supplementalUpdates.some(item => item.batchId === source.batchId), 'B42 metadata audit already exists')
