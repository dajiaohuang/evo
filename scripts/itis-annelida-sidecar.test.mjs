...
describe('ITIS Annelida exact sidecar shards', () => {
  ...
  it('covers the exact COL26.8 Annelida partition once in deterministic, non-overlapping ranges', () => {
    expect(descriptor.packageId).toBe('other-animals')
    expect(descriptor.scope.colRootUsageId).toBe('NN')
    expect(descriptor.scope.colStrictAcceptedSpecies).toBe(18982)
  ...
  it('retains only exact ITIS evidence and addresses each COL species through one shard', () => {
    expect(descriptor.sources.itis.rootTsn).toBe('64357')
  ...
  it('separates ITIS-only current species and preserves byte-exact delivery evidence', () => {
    ...
    expect(upstream).toHaveLength(5092)
