...
const nomenclatureRoot = join(root, 'data/packages/invertebrata/echinoderms/nomenclature')
const descriptorPath = join(nomenclatureRoot, 'itis-echinodermata-sidecar.json')
const ledgerPath = join(root, 'data/sources/itis-echinodermata-sidecar-import-ledger.json')
...
describe('ITIS Echinodermata exact sidecar shards', () => {
  ...
  it('preserves the exact COL root without overlap and every accepted species once', () => {
    expect(descriptor.packageId).toBe('echinoderms')
    expect(descriptor.scope.colRootUsageIds).toEqual(['CHN'])
    expect(descriptor.scope.colRootScientificNames).toEqual(['Echinodermata'])
    expect(descriptor.scope.colStrictAcceptedSpecies).toBe(11891)
    expect(descriptor.scope.packageStrictAcceptedSpecies).toBe(11891)
    expect(descriptor.scope.packageOutOfScopeStrictAcceptedSpecies).toBe(0)
    expect(descriptor.scope.colStrictAcceptedSpeciesByRoot.Echinodermata).toBe(11891)
    ...
    expect(rows.filter((row) => row.colRootUsageId === 'CHN')).toHaveLength(descriptor.scope.colStrictAcceptedSpeciesByRoot.Echinodermata)
  })

  it('uses only exact official ITIS evidence and resolves each detail through one shard', () => {
    expect(descriptor.sources.itis.rootTsns).toEqual({ Echinodermata: '156857' })
