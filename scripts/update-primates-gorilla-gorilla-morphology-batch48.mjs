...
const SOURCE_PATH = join(ROOT, 'data', 'sources', 'primates-gorilla-gorilla-morphology-b48-2026-09-26.json')
...
const MANIFEST_PATH = join(ROOT, 'data', 'knowledge', 'primates-gorilla-gorilla-morphology-b48-2026-09-26.update-manifest.json')
const REGISTRY_ROOT = join(ROOT, 'data', 'catalogue-of-life', 'releases', '2026-08-20', 'registry')
const EXPECTED_SOURCE_SHA256 = 'ed52fdc9728f9b7147296f5ddf221a918069d09a0cf596713c00acee4396d3a5'
...
assert.equal(sha256(sourceBytes), EXPECTED_SOURCE_SHA256, 'Frozen B48 evidence source changed')
const source = JSON.parse(sourceBytes.toString('utf8'))
assert.equal(source.batchId, 'primates-gorilla-gorilla-morphology-batch48-2026-09-26')
...
function validateSourceClaim(dossier) {
  ...
  const claim = source.claims.morphology
  ...
  for (const item of source.sources) {
    ...
    for (const key of ['stableId', 'rightsHolder', 'licenseVersion', 'licenseAppliesTo', 'attribution', 'scope']) {
      assert.ok(item[key], 'B48 source missing ' + key)
    }
...
assert.equal(indexed.count, source.audit.indexedRecordCount, 'Indexed record count changed since B48 identity audit')
const targetMatches = [...indexed.recordsById.values()].filter(row => row.colId === source.target.colId)
assert.equal(targetMatches.length, source.audit.targetRecordCount, 'B48 target must occur exactly once')
...
if (alreadyApplied) {
  ...
  assert.equal(priorManifest.input?.sha256, EXPECTED_SOURCE_SHA256, 'B48 was applied from a different input')
  assert.ok(baseline.facets.morphology.claims.some(claim => JSON.stringify(claim) === JSON.stringify(source.claims.morphology)))
  ...
  assert.ok(!source.sources.some(item => baseline.sources.some(existing => existing.id === item.id)), 'B48 source id already occurs on target')
  const allOccurrences = [...indexed.recordsById.values()].flatMap(record => (record.sources ?? []).map(item => ({ colId: record.colId, source: item })))
  for (const item of source.sources) {
    assert.equal(allOccurrences.filter(entry => entry.source.stableId === item.stableId).length, 0, 'B48 DOI is already indexed: ' + item.stableId)
  }

  const dossier = structuredClone(baseline)
  assert.equal(baseline.facets.morphology.status, 'not-assessed', 'B48 expects the existing morphology gap')
  dossier.checkedAt = source.checkedAt
  dossier.sources.push(...structuredClone(source.sources))
  dossier.facets.morphology = {
    status: 'partially-supported',
    claims: [...(baseline.facets.morphology.claims ?? []), structuredClone(source.claims.morphology)],
    gaps: structuredClone(source.gaps.morphology),
  }
  ...
  assert.ok(!rawBytes.includes(0x0d), 'B48 raw JSONL must use LF line endings')
  ...
  assert.deepEqual(roundTrip, rawBytes, 'B48 Brotli round trip must preserve exact raw bytes')
  ...
  const updateAudit = {
    ...
    mode: 'in-place-add-one-sample-bounded-morphology-source-to-existing-record',
  ...
  const supplementalUpdate = {
    ...
    generator: relative(join(ROOT, 'scripts', 'update-primates-gorilla-gorilla-morphology-batch48.mjs')),
    baseHead: source.audit.baseHead,
    indexedRecordCountAtAudit: indexed.count,
  ...
  assert.ok(!metadata.supplementalUpdates.some(item => item.batchId === source.batchId), 'B48 metadata audit already exists')
