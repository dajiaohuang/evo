...
describe('ITIS Nematoda exact sidecar shards', () => {
  ...
  it('limits the remainder package to the exact Nematoda root and preserves every in-scope COL species once', () => {
    expect(descriptor.packageId).toBe('other-animals')
    expect(descriptor.scope.colRootUsageId).toBe('NM')
  ...
  it('uses exact official ITIS evidence only and resolves each detail through one shard', () => {
    expect(descriptor.sources.itis.rootTsn).toBe('59490')
