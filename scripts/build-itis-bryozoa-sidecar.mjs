...
const ITIS_ROOT_TSN = 155469
const COL_ROOT_USAGE_ID = '622CG'
const PACKAGE_ID = 'other-animals'
const SHARD_SOURCE_LIMIT_BYTES = 2 * 1024 * 1024

const currentSpeciesQuery = `WITH RECURSIVE bryozoa_descendants(tsn) AS (
  SELECT ?1
  UNION ALL
  SELECT u.tsn FROM taxonomic_units u JOIN bryozoa_descendants d ON u.parent_tsn = d.tsn
...
const synonymQuery = `WITH RECURSIVE bryozoa_descendants(tsn) AS (
  SELECT ?1
  UNION ALL
  SELECT u.tsn FROM taxonomic_units u JOIN bryozoa_descendants d ON u.parent_tsn = d.tsn
  WHERE u.name_usage = 'valid'
), accepted_bryozoa_species(tsn) AS (
  SELECT u.tsn FROM bryozoa_descendants d
  JOIN taxonomic_units u ON u.tsn = d.tsn
  JOIN taxon_unit_types r ON r.kingdom_id = u.kingdom_id AND r.rank_id = u.rank_id
  WHERE lower(trim(r.rank_name)) = 'species' AND u.name_usage = 'valid'
)
...
JOIN accepted_bryozoa_species a ON a.tsn = s.tsn_accepted
...
async function loadColBryozoaSpecies(manifest) {
...
function loadItis(sqlitePath) {
  ...
  try {
    ...
    if (!rootRecord || rootRecord.completename !== 'Bryozoa' || rootRecord.rank_name !== 'Phylum' || rootRecord.name_usage !== 'valid') throw new Error('Pinned ITIS Bryozoa root no longer has the expected valid phylum identity')
...
async function main() {
  ...
  if (argument < 0 || !process.argv[argument + 1]) throw new Error('Usage: node scripts/build-itis-bryozoa-sidecar.mjs --itis-sqlite <verified ITIS.sqlite>')
  ...
  const colSpecies = await loadColBryozoaSpecies(JSON.parse(registryBytes)); const packSpecies = await loadResourcePackSpecies(resourcePack)
  const packIds = new Set(packSpecies.map((record) => record.id))
  if (colSpecies.some((record) => !packIds.has(record.id))) throw new Error('Bryozoa species are not all present in the declared Other Animals resource pack')
  ...
  if (new Set(crosswalk.map((record) => record.colUsageId)).size !== colSpecies.length) throw new Error('COL Bryozoa records are not uniquely addressable')
  const upstreamOnly = currentRows.filter((row) => !evidencedTsns.has(String(row.tsn))).map((row) => ({ colUsageId: null, currentName: currentName(row), basis: 'No strict COL26.8 Bryozoa accepted-species name or official ITIS species-synonym evidence resolves to this current ITIS TSN.' })).sort((a, b) => Number(a.currentName.tsn) - Number(b.currentName.tsn))
  mkdirSync(nomenclatureRoot, { recursive: true })
  for (const name of readdirSync(nomenclatureRoot)) if (/^itis-bryozoa-(?:sidecar|upstream-only)-\d{4}\.jsonl\.gz$/u.test(name)) rmSync(join(nomenclatureRoot, name))
  const shards = chunkBySourceBytes(crosswalk).map((records, indexNumber) => {
    const filename = `itis-bryozoa-sidecar-${String(indexNumber).padStart(4, '0')}.jsonl.gz`; const sourcePayload = jsonlBytes(records); const compressed = Buffer.from(deterministicGzip(sourcePayload, { level: 9 })); const path = join(nomenclatureRoot, filename); writeFileSync(path, compressed); return outputDescriptor(path, records, compressed, sourcePayload)
  })
  const upstreamSource = jsonlBytes(upstreamOnly); const upstreamCompressed = Buffer.from(deterministicGzip(upstreamSource, { level: 9 })); const upstreamPath = join(nomenclatureRoot, 'itis-bryozoa-upstream-only-0000.jsonl.gz'); writeFileSync(upstreamPath, upstreamCompressed)
  ...
  const bryozoaDatasetCounts = Object.fromEntries(Object.entries(colSpecies.reduce((map, row) => { const key = row.sourceDatasetId ?? 'null'; map[key] = (map[key] ?? 0) + 1; return map }, {})).sort(([a], [b]) => Number(a) - Number(b)))
  const descriptor = {
    schemaVersion: 1, sidecarType: 'release-pinned-exact-nomenclatural-crosswalk', packageId: PACKAGE_ID,
    scope: { colRootUsageId: COL_ROOT_USAGE_ID, colRootScientificName: 'Bryozoa', colStrictAcceptedSpecies: colSpecies.length, packageStrictAcceptedSpecies: packageCount, packageOutOfScopeStrictAcceptedSpecies: packageCount - colSpecies.length, boundary: 'other-animals is the deterministic Animalia remainder after more-specific static-package routes. This sidecar covers only strict accepted COL26.8 species descending from exact Bryozoa root 622CG; every other other-animals-owned species is explicitly outside this sidecar.' },
    mixedResourcePack: { packageId: PACKAGE_ID, manifestPath: repoPath(resourcePackManifestPath), manifestSha256: sha256(resourcePackBytes), acceptedSpeciesCount: packSpecies.length, inScopeSpecies: colSpecies.length, outOfScopeSpecies: packSpecies.length - colSpecies.length, packageSourceDatasetCounts: packDatasetCounts, bryozoaSourceDatasetCounts: bryozoaDatasetCounts, outOfScopeBoundary: 'The remaining 78,794 accepted species in Other Animals are not Bryozoa and receive no ITIS Bryozoa match in this sidecar.' },
    ...
    exactMatching: { normalization: source.importLedger.normalization, statuses: { accepted: 'The normalized COL name resolves to exactly one valid ITIS Bryozoa species and directly equals that current ITIS name.', 'synonym-current-name-redirect': 'The normalized COL name equals official ITIS invalid species-name evidence that resolves to exactly one valid ITIS Bryozoa species.', ambiguous: 'The normalized exact evidence resolves to more than one valid ITIS Bryozoa species TSN.', unmatched: 'No normalized exact valid-name or official ITIS species-synonym evidence resolves to a valid ITIS Bryozoa species.' }, prohibited: 'No fuzzy, edit-distance, phonetic, case-folded, diacritic-stripped, token-reordered or taxon-substituted matching is used.' },
    evidenceBoundary: { en: 'This CC0 ITIS sidecar is a frozen exact nomenclatural crosswalk for the declared Bryozoa partition inside the mixed Other Animals resource pack. It is not a global bryozoan checklist, final classification authority, phylogeny, species-concept equivalence assertion, biological dossier or scientific-review record.', zh: '此 CC0 ITIS 侧车是混合“其他动物”资源包中所声明苔藓动物分区的冻结严格命名交叉映射；它不是全球苔藓动物名录、最终分类权威、系统发育树、物种概念等同性声明、生物档案或科学审查记录。' },
