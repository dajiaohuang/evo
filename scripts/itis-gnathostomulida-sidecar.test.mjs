...
describe('ITIS Gnathostomulida exact sidecar shards', () => {
  ...
  it('covers the exact COL26.8 Gnathostomulida partition once in deterministic, non-overlapping ranges', () => {
    expect(descriptor.packageId).toBe('other-animals')
    expect(descriptor.scope.colRootUsageId).toBe('B8VF3')
    expect(descriptor.scope.colStrictAcceptedSpecies).toBe(100)
  ...
  it('retains only exact ITIS evidence and addresses each COL species through one shard', () => {
    expect(descriptor.sources.itis.rootTsn).toBe('57405')
