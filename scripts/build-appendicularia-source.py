...
ARCHIVE_SHA = '5a4a49450d581faa30d0fa3d6beb54b4b561f920f075174124efbfd8bdfa8c1f'
ARCHIVE_BYTES = 32041
ROOT_ID = '622C5'
...
def archive_member_digests(path):
    with zipfile.ZipFile(path) as z:
        return [{'name': info.filename, 'bytes': len(z.read(info.filename)),
                 'sha256': sha(z.read(info.filename))} for info in z.infolist()]
...
def read_col(registry=REGISTRY):
    ...
    for r in candidates:
        ...
        if x == ROOT_ID and str(r.get('sourceDatasetId')) == '1178': rows[r['id']]=r
    for r in candidates:
    ...
    shards=[]
    for p in paths:
        b=p.read_bytes(); shards.append({'path':'data/catalogue-of-life/releases/2026-08-20/registry/'+p.relative_to(registry).as_posix(),'bytes':len(b),'sha256':sha(b)})
...
def record(col, candidates, syns, strict, names, refs, nr):
    ...
    return {'colId':col['id'],'colScientificName':col.get('scientificName'),'colAuthorship':col.get('authorship'), 'status':status,'matchedName':matched,'acceptedName':accepted,'candidates':[obj(*v) for v in targets.values()] if status=='ambiguous' else [],'mappingBasis':'Exact scientific name+authorship; explicit Synonym target retained as redirect.','sourceRows':[]}
def chunks(rows):
    out=[]; cur=[]; used=3
    for r in rows:
        n=len(enc(r))
    ...
    return out
def main():
    ap=argparse.ArgumentParser(); ap.add_argument('--archive',type=Path,default=ROOT/'data/sources/archives/checklistbank-1178-appendicularia-2026-09-01.zip'); ap.add_argument('--metadata',type=Path,default=ROOT/'data/sources/archives/checklistbank-1178-appendicularia-2026-09-01.metadata.json'); ap.add_argument('--output-root',type=Path,default=ROOT); a=ap.parse_args()
    output_root=a.output_root.resolve(); registry=REGISTRY
    raw=a.archive.read_bytes(); assert len(raw)==ARCHIVE_BYTES and sha(raw)==ARCHIVE_SHA
    metadata=json.loads(a.metadata.read_bytes()); assert metadata['versionDoi']=='10.48580/d3fn.v89'
    archive_members=archive_member_digests(a.archive)
    ...
    out=output_root/'data/catalogue-of-life/releases/2026-08-20/resource-packs/other-animals'; out.mkdir(parents=True,exist_ok=True); prefix='worms-appendicularia'; files=[]; upfiles=[]
    keep=set()
    for up,rows,arr in ((False,records,files),(True,upstream,upfiles)):
        for i,part in enumerate(chunks(rows)):
            ...
            q.update(role='upstream-only' if up else 'col-partition', encoding='gzip', mediaType='application/json')
    ...
    ledger={'schemaVersion':1,'importType':'COL26.8-to-Appendicularia-1178-authority-crosswalk','sourceArchive':{'path':'data/sources/archives/checklistbank-1178-appendicularia-2026-09-01.zip','bytes':ARCHIVE_BYTES,'sha256':ARCHIVE_SHA,'members':archive_members},'sourceMetadata':{'path':'data/sources/archives/checklistbank-1178-appendicularia-2026-09-01.metadata.json','sha256':sha(a.metadata.read_bytes()),'title':metadata['title'],'version':metadata['version'],'versionDoi':metadata['versionDoi'],'citation':metadata['citation'],'editor':metadata['editor'],'contributor':metadata['contributor']},'colInput':{'registryManifestSha256':msh,'nodeShards':node},'scope':{'colRootUsageId':ROOT_ID,'colParentClosureRootUsageId':PARENT_ROOT_ID,'colSpecies':len(col),'sourceSpeciesRankTaxa':species_total,'provisionalExcluded':provisional,'excludedParentClosureSpecies':excluded,'counts':descriptor['counts']},'outputs':{'descriptorSha256':sha((out/f'{prefix}-sidecar.json').read_bytes()),'files':files,'upstreamOnlyFiles':upfiles}}
