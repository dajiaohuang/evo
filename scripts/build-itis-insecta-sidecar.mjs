...
const descriptorPath = join(nomenclatureRoot, 'itis-insecta-sidecar.json')
const ledgerPath = join(root, 'data/sources/itis-insecta-sidecar-import-ledger.json')
const ITIS_ROOT_TSN = 99208
const COL_ROOT_USAGE_ID = 'H6'
const PACKAGE_ID = 'crustaceans-insects'
const SHARD_SOURCE_LIMIT_BYTES = 2 * 1024 * 1024

const currentSpeciesQuery = `WITH RECURSIVE insecta_descendants(tsn) AS (
  SELECT ?1
  UNION ALL
  SELECT u.tsn FROM taxonomic_units u JOIN insecta_descendants d ON u.parent_tsn = d.tsn
...
const synonymQuery = `WITH RECURSIVE insecta_descendants(tsn) AS (
  SELECT ?1
  UNION ALL
  SELECT u.tsn FROM taxonomic_units u JOIN insecta_descendants d ON u.parent_tsn = d.tsn
  WHERE u.name_usage = 'valid'
), accepted_insecta_species(tsn) AS (
  SELECT u.tsn FROM insecta_descendants d
  JOIN taxonomic_units u ON u.tsn = d.tsn
  JOIN taxon_unit_types r ON r.kingdom_id = u.kingdom_id AND r.rank_id = u.rank_id
  WHERE lower(trim(r.rank_name)) = 'species' AND u.name_usage = 'valid'
)
...
JOIN accepted_insecta_species a ON a.tsn = s.tsn_accepted
...
async function loadColInsectaSpecies(manifest) {
...
function loadItis(sqlitePath) {
  ...
  try {
    ...
    if (!rootRecord || rootRecord.completename !== 'Insecta' || rootRecord.rank_name !== 'Class' || rootRecord.name_usage !== 'valid') {
      throw new Error('Pinned ITIS Insecta root no longer has the expected valid class identity')
    }
...
async function main() {
  ...
  if (argument < 0 || !process.argv[argument + 1]) throw new Error('Usage: node scripts/build-itis-insecta-sidecar.mjs --itis-sqlite <verified ITIS.sqlite>')
  ...
  const colSpecies = await loadColInsectaSpecies(JSON.parse(registryBytes))
  ...
  if (new Set(crosswalk.map((record) => record.colUsageId)).size !== colSpecies.length) throw new Error('COL Insecta records are not uniquely addressable')
  const upstreamOnly = currentRows.filter((row) => !evidencedTsns.has(String(row.tsn))).map((row) => ({
    ...
    basis: 'No strict COL26.8 Insecta accepted-species name or official ITIS species-synonym evidence resolves to this current ITIS TSN.',
  })).sort((left, right) => Number(left.currentName.tsn) - Number(right.currentName.tsn))
  mkdirSync(nomenclatureRoot, { recursive: true })
  for (const name of readdirSync(nomenclatureRoot)) if (/^itis-insecta-(?:sidecar|upstream-only)-\d{3,4}\.jsonl\.gz$/u.test(name)) rmSync(join(nomenclatureRoot, name))
  const shards = chunkBySourceBytes(crosswalk).map((records, indexNumber) => {
    const filename = `itis-insecta-sidecar-${String(indexNumber).padStart(4, '0')}.jsonl.gz`
  ...
  const upstreamPath = join(nomenclatureRoot, 'itis-insecta-upstream-only-0000.jsonl.gz')
  ...
  const descriptor = {
    ...
    scope: {
      colRootUsageId: COL_ROOT_USAGE_ID, colRootScientificName: 'Insecta', colStrictAcceptedSpecies: colSpecies.length,
      packageStrictAcceptedSpecies: packageCount, packageOutOfScopeStrictAcceptedSpecies: packageCount - colSpecies.length,
      boundary: 'This mixed package contains Crustacea, Myriapoda and broader Arthropoda navigation content. This sidecar covers only the strict accepted COL26.8 species descending from the exact Insecta root H6; all remaining package-owned species are explicitly out of scope.',
    },
    sources: {
    ...
    exactMatching: {
      ...
      statuses: {
        accepted: 'The normalized COL name resolves to exactly one valid ITIS Insecta species and directly equals that current ITIS name.',
        'synonym-current-name-redirect': 'The normalized COL name equals official ITIS invalid species-name evidence that resolves to exactly one valid ITIS Insecta species.',
        ambiguous: 'The normalized exact evidence resolves to more than one valid ITIS Insecta species TSN.',
        unmatched: 'No normalized exact valid-name or official ITIS species-synonym evidence resolves to a valid ITIS Insecta species.',
      },
    ...
    evidenceBoundary: {
      en: 'This CC0 ITIS sidecar is a frozen exact nomenclatural crosswalk, not a global insect checklist, final classification authority, phylogeny, species-concept equivalence assertion, biological dossier or scientific-review record.',
  ...
  const ledger = {
    schemaVersion: 1, importType: 'COL26.8-to-ITIS-exact-insecta-nomenclatural-sidecar',
    ...
    generatedBy: { scriptPath: 'scripts/build-itis-insecta-sidecar.mjs', scriptSha256: await sha256File(fileURLToPath(import.meta.url)), deterministic: 'Pinned input checksums, fixed roots, exact SQL, exact representation-only normalization, code-unit ID ordering and deterministic gzip; no wall-clock fields or fuzzy matching.' },
  }
