...
const descriptorPath = join(nomenclatureRoot, 'itis-nemertea-sidecar.json')
const ledgerPath = join(root, 'data/sources/itis-nemertea-sidecar-import-ledger.json')
const ITIS_ROOT_TSN = 57411
const COL_ROOT_USAGE_ID = '5C'
...
function loadItis(sqlitePath) {
  ...
  try {
    ...
    if (!rootRecord || rootRecord.completename !== 'Nemertea' || rootRecord.rank_name !== 'Phylum' || rootRecord.name_usage !== 'valid') throw new Error('Pinned ITIS root is not the expected valid Nemertea phylum')
...
async function main() {
  ...
  if (argument < 0 || !process.argv[argument + 1]) throw new Error('Usage: node scripts/build-itis-nemertea-sidecar.mjs --itis-sqlite <verified ITIS.sqlite>')
  ...
  if (resourcePack.packageId !== PACKAGE_ID || resourcePack.acceptedSpeciesCount !== ownership.packageCounts[PACKAGE_ID] || colSpecies.some((record) => !packIds.has(record.id))) throw new Error('Nemertea and Other Animals ownership boundaries changed')
  ...
  crosswalk.sort((a, b) => compareCodeUnits(a.colUsageId, b.colUsageId)); if (new Set(crosswalk.map((record) => record.colUsageId)).size !== colSpecies.length) throw new Error('COL Nemertea records are not uniquely addressable')
  const upstreamOnly = currentRows.filter((row) => !evidencedTsns.has(String(row.tsn))).map((row) => ({ colUsageId: null, currentName: currentName(row), basis: 'No strict COL26.8 Nemertea accepted-species name or official ITIS species-synonym evidence resolves to this current ITIS TSN.' })).sort((a, b) => Number(a.currentName.tsn) - Number(b.currentName.tsn))
  mkdirSync(nomenclatureRoot, { recursive: true }); for (const name of readdirSync(nomenclatureRoot)) if (/^itis-nemertea-(?:sidecar|upstream-only)-\d{4}\.jsonl\.gz$/u.test(name)) rmSync(join(nomenclatureRoot, name))
  const shards = chunkBySourceBytes(crosswalk).map((records, number) => { const path = join(nomenclatureRoot, `itis-nemertea-sidecar-${String(number).padStart(4, '0')}.jsonl.gz`); const sourcePayload = jsonlBytes(records); const compressed = Buffer.from(deterministicGzip(sourcePayload, { level: 9 })); writeFileSync(path, compressed); return outputDescriptor(path, records, compressed, sourcePayload) })
  const upstreamPath = join(nomenclatureRoot, 'itis-nemertea-upstream-only-0000.jsonl.gz'); const upstreamSource = jsonlBytes(upstreamOnly); const upstreamCompressed = Buffer.from(deterministicGzip(upstreamSource, { level: 9 })); writeFileSync(upstreamPath, upstreamCompressed)
  ...
  const descriptor = { schemaVersion: 1, sidecarType: 'release-pinned-exact-nomenclatural-crosswalk', packageId: PACKAGE_ID,
    scope: { colRootUsageId: COL_ROOT_USAGE_ID, colRootScientificName: 'Nemertea', colStrictAcceptedSpecies: colSpecies.length, packageStrictAcceptedSpecies: resourcePack.acceptedSpeciesCount, packageOutOfScopeStrictAcceptedSpecies: resourcePack.acceptedSpeciesCount - colSpecies.length, boundary: 'other-animals is the deterministic Animalia remainder after more-specific static-package routes. This sidecar covers only strict accepted COL26.8 species descending from exact Nemertea root 5C; every other other-animals-owned species is explicitly outside this sidecar.' },
    mixedResourcePack: { packageId: PACKAGE_ID, manifestPath: repoPath(resourcePackManifestPath), manifestSha256: sha256(resourcePackBytes), acceptedSpeciesCount: packSpecies.length, inScopeSpecies: colSpecies.length, outOfScopeSpecies: packSpecies.length - colSpecies.length, packageSourceDatasetCounts: sourceDatasetCounts(packSpecies), nemerteaSourceDatasetCounts: sourceDatasetCounts(colSpecies), outOfScopeBoundary: 'All remaining accepted species in Other Animals are not Nemertea and receive no ITIS Nemertea match in this sidecar.' },
    ...
    exactMatching: { normalization: source.importLedger.normalization, statuses: { accepted: 'The normalized COL name resolves to exactly one valid ITIS Nemertea species and directly equals that current ITIS name.', 'synonym-current-name-redirect': 'The normalized COL name equals official ITIS invalid species-name evidence that resolves to exactly one valid ITIS Nemertea species.', ambiguous: 'The normalized exact evidence resolves to more than one valid ITIS Nemertea species TSN.', unmatched: 'No normalized exact valid-name or official ITIS species-synonym evidence resolves to a valid ITIS Nemertea species.' }, prohibited: 'No fuzzy, edit-distance, phonetic, case-folded, diacritic-stripped, token-reordered or taxon-substituted matching is used.' },
    evidenceBoundary: { en: 'This CC0 ITIS sidecar is a frozen exact nomenclatural crosswalk for the declared Nemertea partition inside the mixed Other Animals resource pack. It is not a global nemertean checklist, final classification authority, phylogeny, species-concept equivalence assertion, biological dossier or scientific-review record.', zh: '此 CC0 ITIS 侧车是混合“其他动物”资源包中已声明 Nemertea 分区的冻结严格命名交叉映射；它不是全球纽形动物名录、最终分类权威、系统发育、物种概念等同性声明、生物档案或科学审查记录。' },
