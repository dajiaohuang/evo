...
assert.equal(source.updateAudit.priorGeneratedRawSha256, 'c0d05e75a3b79bc7d847f935da42109b7a082bd21005a469679e7efc01e58782')
assert.equal(source.updateAudit.priorGeneratedShardSha256, 'b9f51c5101c0d727fd0b4b6281e5ba87907af878a4f109ac5ff9af1064527747')
...
assert.ok(dossier.facets.ecology.claims[1].text.includes('scan-based fruit-feeding observations'))
...
assert.ok(currentRawSha256 === source.updateAudit.previousRawSha256 || currentRawSha256 === source.updateAudit.priorGeneratedRawSha256 || currentRawBytes.equals(rawBytes), 'Existing raw record differs from the audited baseline, prior generated version, and deterministic update')
assert.ok(currentShardSha256 === source.updateAudit.previousShardSha256 || currentShardSha256 === source.updateAudit.priorGeneratedShardSha256 || currentCompressedBytes.equals(compressed), 'Existing shard differs from the audited baseline, prior generated version, and deterministic update')
