...
function loadItis(sqlitePath) {
  ...
  try {
    ...
    if (!rootRecord || rootRecord.completename !== 'Ciliophora' || rootRecord.rank_name !== 'Phylum' || rootRecord.name_usage !== 'valid') throw new Error('Pinned ITIS root is not the expected valid Ciliophora phylum')
...
async function main() {
  ...
  if (argument < 0 || !process.argv[argument + 1]) throw new Error('Usage: node scripts/build-itis-ciliophora-sidecar.mjs --itis-sqlite <verified ITIS.sqlite>')
  ...
  crosswalk.sort((a, b) => compareCodeUnits(a.colUsageId, b.colUsageId)); if (new Set(crosswalk.map((record) => record.colUsageId)).size !== colSpecies.length) throw new Error('COL Ciliophora records are not uniquely addressable')
  const upstreamOnly = currentRows.filter((row) => !evidencedTsns.has(String(row.tsn))).map((row) => ({ colUsageId: null, currentName: currentName(row), basis: 'No strict COL26.8 Ciliophora accepted-species name or official ITIS species-synonym evidence resolves to this current ITIS TSN.' })).sort((a, b) => Number(a.currentName.tsn) - Number(b.currentName.tsn))
  mkdirSync(packRoot, { recursive: true }); for (const name of readdirSync(packRoot)) if (/^itis-ciliophora-(?:sidecar|upstream-only)-\d{4}\.jsonl\.gz$/u.test(name)) rmSync(join(packRoot, name))
  const shards = chunks(crosswalk).map((records, indexNumber) => { const path = join(packRoot, `itis-ciliophora-sidecar-${String(indexNumber).padStart(4, '0')}.jsonl.gz`); const sourcePayload = jsonlBytes(records); const bytes = Buffer.from(deterministicGzip(sourcePayload, { level: 9 })); writeFileSync(path, bytes); return shardDescriptor(path, records, bytes, sourcePayload) })
  const upstreamSource = jsonlBytes(upstreamOnly); const upstreamBytes = Buffer.from(deterministicGzip(upstreamSource, { level: 9 })); const upstreamPath = join(packRoot, 'itis-ciliophora-upstream-only-0000.jsonl.gz'); writeFileSync(upstreamPath, upstreamBytes)
