...
const SOURCE_PATH = join(ROOT, 'data', 'sources', 'primates-papio-anubis-ontogeny-b44-2026-09-26.json')
...
const MANIFEST_PATH = join(ROOT, 'data', 'knowledge', 'primates-papio-anubis-ontogeny-b44-2026-09-26.update-manifest.json')
const REGISTRY_ROOT = join(ROOT, 'data', 'catalogue-of-life', 'releases', '2026-08-20', 'registry')
const EXPECTED_SOURCE_SHA256 = 'b79c02335b6b6e6a1157b56920f9775ed5d0ed19d5d287f06bd373c31b7b1cf3'
const FACETS = ['morphology', 'lifeHistory', 'ecology', 'evolution', 'distribution', 'fossil', 'conservation']
const TARGET_FACETS = ['morphology', 'lifeHistory']
...
assert.equal(sha256(sourceBytes), EXPECTED_SOURCE_SHA256, 'Frozen B44 evidence source changed')
const source = JSON.parse(sourceBytes.toString('utf8'))
assert.equal(source.batchId, 'primates-papio-anubis-ontogeny-batch44-2026-09-26')
...
function validateEvidence(dossier) {
  ...
  for (const item of source.sources) {
    ...
    for (const key of ['stableId', 'rightsHolder', 'licenseVersion', 'licenseAppliesTo', 'attribution', 'scope']) {
      assert.ok(item[key], 'B44 source missing ' + key)
    }
...
assert.equal(indexed.count, source.audit.indexedRecordCount, 'B44 dossier index count changed since identity audit')
const targetMatches = [...indexed.recordsById.values()].filter(row => row.colId === source.target.colId)
assert.equal(targetMatches.length, source.audit.targetRecordCount, 'B44 target must occur exactly once')
...
if (alreadyApplied) {
  assert.equal(alreadyApplied.sourceSha256, EXPECTED_SOURCE_SHA256, 'B44 source digest differs from prior application')
  assert.equal(sha256(rawBytesBefore), metadata.rawSha256, 'B44 raw JSONL digest differs after prior application')
  ...
  assert.deepEqual(originalRows.map(row => row.colId).filter(id => id !== source.target.colId).sort(), [...source.audit.siblingColIds].sort(), 'B44 sibling list differs from the audited shard')
  const baseline = originalRows.find(row => row.colId === source.target.colId)
  assert.ok(baseline, 'Missing B44 target in audited raw shard')
  assert.equal(sha256(Buffer.from(JSON.stringify(baseline), 'utf8')), source.audit.previousTargetRecordSha256, 'B44 target record differs from audited baseline')
  ...
  assert.ok(!baseline.sources.some(item => item.id === source.sources[0].id), 'B44 source id already occurs on target')
  assert.ok(!baseline.sources.some(item => item.stableId === source.sources[0].stableId), 'B44 stable source ID already occurs on target')

  const allOccurrences = [...indexed.recordsById.values()].flatMap(record => (record.sources ?? []).map(item => ({ colId: record.colId, source: item })))
  assert.equal(allOccurrences.filter(entry => entry.source.stableId === source.sources[0].stableId).length, source.audit.sourceOccurrenceCountBeforeUpdate, 'B44 DOI occurrence count changed')
  assert.equal(allOccurrences.filter(entry => entry.source.id === source.sources[0].id).length, 0, 'B44 source id is already indexed')
  ...
  dossier.identity.scope = 'COL26.8 usage 6TM9B only. The ecological claim concerns monitoring of one wild Lake Manyara National Park population; the morphology and growth claims concern one captive cohort at the CNRS Primatology Station, Rousset-sur-Arc, France. These distinct samples do not support a species-wide generalization.'
  dossier.lifeStatusScope.wild = 'Ecology evidence concerns wild olive baboons at Lake Manyara National Park, Tanzania, monitored 2011–2019. Morphology and growth evidence concerns 14 female and 16 male captive olive baboons at the CNRS Primatology Station, Rousset-sur-Arc, France, followed from infancy to adulthood over seven years. The wild and captive scopes remain separate; no claim is extrapolated to all populations.'
  dossier.completeness = {
    status: 'incomplete',
    reasons: [
      'The dossier now includes site-specific ecology and captive-cohort morphology and growth evidence; evolution, species-wide distribution, fossil evidence, and conservation remain unassessed.',
      'Systematic literature coverage across the accepted species concept, including wild growth and life-history variation, remains incomplete.',
  ...
  const rawLines = originalLines.map(line => {
    ...
    assert.deepEqual(indexed.recordsById.get(row.colId), row, 'B44 sibling changed: ' + row.colId)
    return line
  })
  const rawBytes = Buffer.from(rawLines.join('\n') + '\n', 'utf8')
  assert.ok(!rawBytes.includes(0x0d), 'B44 raw JSONL must use LF line endings')
  ...
  assert.deepEqual(roundTrip, rawBytes, 'B44 Brotli round trip must preserve exact raw bytes')
  ...
  const updateAudit = {
    ...
    sourceOccurrenceCountBeforeUpdate: 0,
    mode: 'in-place-add-captive-ontogeny-morphology-and-growth-evidence-to-existing-record',
  ...
  const supplementalUpdate = {
    ...
    generator: relative(join(ROOT, 'scripts', 'update-primates-papio-anubis-ontogeny-b44.mjs')),
    baseHead: source.audit.baseHead,
    indexedRecordCountAtAudit: indexed.count,
  ...
  assert.ok(!metadata.supplementalUpdates.some(item => item.batchId === source.batchId), 'B44 metadata audit already exists')
