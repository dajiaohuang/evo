...
def project(archive,metadata,output_root=None):
 ...
 descriptor['source'].update(title=meta['title'],version=meta['version'],versionDoi=meta['versionDoi'],license='CC-BY-4.0',metadataBytes=len(metadata_bytes),metadataSha256=digest(metadata_bytes))
 descriptor['counts'].pop('bytes',None); descriptor['totalCompressedBytes']=totalbytes; descriptor['totalSourceBytes']=sum(x['sourceBytes'] for x in cf+uf); descriptor['evidenceBoundary']['zh']='冻结的严格命名交叉映射；不是物种概念等同性、生物档案、化石证据或专家审查。'
