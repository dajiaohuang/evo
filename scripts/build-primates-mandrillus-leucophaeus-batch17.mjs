...
const SOURCE = 'data/sources/primates-mandrillus-leucophaeus-batch17-2026-09-24.json'
const RAW = 'data/knowledge/raw-dossiers/primates-mandrillus-leucophaeus-batch17-2026-09-24.jsonl'
const SHARD = 'data/knowledge/catalogue-dossiers-primates-mandrillus-leucophaeus-batch17-2026-09-24.jsonl.br'
const MANIFEST = 'data/knowledge/catalogue-dossiers-primates-mandrillus-leucophaeus-batch17-2026-09-24.batch-manifest.json'
...
const EXPECTED_SOURCE_SHA256 = 'b22d92a207b3446aea0d23860440970d178da46168e7ffcbcc9e479c78d41feb'
...
function resolveAcceptedUsage(registry, record) {
  const route = normalize(record.scientificName).slice(0, 2)
  const matches = (registry.search.routes[route] ?? []).flatMap(readRegistryRows).filter(row => row.id === record.colId)
  assert.equal(matches.length, 1, `Expected one pinned COL26.8 search usage for ${record.colId}`)
  const usage = matches[0]
  for (const [key, value] of Object.entries({ scientificName: record.scientificName, authorship: record.authorship, rank: 'species', status: 'accepted', sourceDatasetId: record.sourceDatasetId })) {
    assert.equal(String(usage[key]), String(value), `Accepted usage ${key} mismatch for ${record.colId}`)
  }
  ...
  assert.equal(chain.at(-1).id, record.colId)
  return chain.map(({ id, scientificName, authorship, rank, status, sourceDatasetId }) => ({ id, scientificName, authorship, rank, status, sourceDatasetId }))
}
...
function validateRecord(record) {
  ...
  assert.equal(record.sources.find(source => source.id === 'col')?.licenseAssessment, 'identity-only')
...
assert.equal(source.duplicateAudit.baseHead, '36b6961f8fb380c23a0c884529ce2e85bf32eddb')
...
const records = source.records.map(record => ({ ...record, classificationPath: resolveAcceptedUsage(registry, record) }))
for (const record of records) {
  assert.ok(!indexedIds.has(record.colId), `COL id already indexed: ${record.colId}`)
  assert.ok(!indexedNames.has(normalize(record.scientificName)), `Scientific name already indexed: ${record.scientificName}`)
...
const manifest = {
  ...
  generator: 'scripts/build-primates-mandrillus-leucophaeus-batch17.mjs',
}
writeFileSync(join(ROOT, MANIFEST), `${JSON.stringify(manifest, null, 2)}\n`, 'utf8')
console.log(JSON.stringify({ recordCount: records.length, colIds: records.map(record => record.colId), scientificNames: records.map(record => record.scientificName), indexedRecordCount: indexedCount, registryManifestSha256: sha256(registryBytes), rawSha256: sha256(rawBytes), compressedSha256: sha256(compressedBytes), roundTrip: 'exact-byte-match' }, null, 2))
