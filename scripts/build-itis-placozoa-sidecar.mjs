...
const descriptorPath = join(packRoot, 'itis-placozoa-sidecar.json')
const ledgerPath = join(root, 'data/sources/itis-placozoa-sidecar-import-ledger.json')
const ITIS_ROOT_TSN = 563955
const COL_ROOT_USAGE_ID = 'B8V3N'
...
function loadItis(sqlitePath) {
  ...
  try {
    ...
    if (!rootRecord || rootRecord.completename !== 'Placozoa' || rootRecord.rank_name !== 'Phylum' || rootRecord.name_usage !== 'valid') throw new Error('Pinned ITIS root is not the expected valid Placozoa phylum')
...
async function main() {
  ...
  if (argument < 0 || !process.argv[argument + 1]) throw new Error('Usage: node scripts/build-itis-placozoa-sidecar.mjs --itis-sqlite <verified ITIS.sqlite>')
  ...
  if (colSpecies.length > packageCount) throw new Error('Placozoa COL scope exceeds Other Animals ownership')
  ...
  crosswalk.sort((a, b) => compareCodeUnits(a.colUsageId, b.colUsageId)); if (new Set(crosswalk.map((record) => record.colUsageId)).size !== colSpecies.length) throw new Error('COL Placozoa records are not uniquely addressable')
  const upstreamOnly = currentRows.filter((row) => !evidencedTsns.has(String(row.tsn))).map((row) => ({ colUsageId: null, currentName: currentName(row), basis: 'No strict COL26.8 Placozoa accepted-species name or official ITIS species-synonym evidence resolves to this current ITIS TSN.' })).sort((a, b) => Number(a.currentName.tsn) - Number(b.currentName.tsn))
  mkdirSync(packRoot, { recursive: true }); for (const name of readdirSync(packRoot)) if (/^itis-placozoa-(?:sidecar|upstream-only)-\d{4}\.jsonl\.gz$/u.test(name)) rmSync(join(packRoot, name))
  const shards = chunks(crosswalk).map((records, indexNumber) => { const path = join(packRoot, `itis-placozoa-sidecar-${String(indexNumber).padStart(4, '0')}.jsonl.gz`); const sourcePayload = jsonlBytes(records); const bytes = Buffer.from(deterministicGzip(sourcePayload, { level: 9 })); writeFileSync(path, bytes); return shardDescriptor(path, records, bytes, sourcePayload) })
  const upstreamSource = jsonlBytes(upstreamOnly); const upstreamBytes = Buffer.from(deterministicGzip(upstreamSource, { level: 9 })); const upstreamPath = join(packRoot, 'itis-placozoa-upstream-only-0000.jsonl.gz'); writeFileSync(upstreamPath, upstreamBytes)
  ...
  const descriptor = { schemaVersion: 1, sidecarType: 'release-pinned-exact-nomenclatural-crosswalk', packageId: PACKAGE_ID,
    scope: { packageRootUsageId: 'N', packageRootScientificName: 'Animalia', colRootUsageId: COL_ROOT_USAGE_ID, colRootScientificName: 'Placozoa', colStrictAcceptedSpecies: colSpecies.length, packageStrictAcceptedSpecies: packageCount, packageOutOfScopeStrictAcceptedSpecies: packageCount - colSpecies.length, nonApplicableRemainder: 'All other strict accepted COL26.8 species assigned to Other Animals remain outside this Placozoa sidecar.', boundary: 'This sidecar covers only strict accepted COL26.8 species descending from exact Placozoa root B8V3N; all remaining mixed-pack species are explicitly out of scope.' },
    ...
    exactMatching: { normalization: source.importLedger.normalization, statuses: { accepted: 'The normalized COL name resolves to exactly one valid ITIS Placozoa species and directly equals that current ITIS name.', 'synonym-current-name-redirect': 'The normalized COL name equals official ITIS invalid species-name evidence that resolves to exactly one valid ITIS Placozoa species.', ambiguous: 'The normalized exact evidence resolves to more than one valid ITIS Placozoa species TSN.', unmatched: 'No normalized exact valid-name or official ITIS species-synonym evidence resolves to a valid ITIS Placozoa species.' }, prohibited: 'No fuzzy, edit-distance, phonetic, case-folded, diacritic-stripped, token-reordered or taxon-substituted matching is used.' },
    evidenceBoundary: { en: 'This CC0 ITIS sidecar is a frozen exact nomenclatural crosswalk for the declared Placozoa partition. It is not a global placozoan checklist, final classification authority, phylogeny, species-concept equivalence assertion, biological dossier or scientific-review record.', zh: '此 CC0 ITIS 侧车是已声明 Placozoa 分区的冻结严格命名交叉映射；它不是全球扁盘动物名录、最终分类权威、系统发育、物种概念等同性声明、生物档案或科学审查记录。' },
