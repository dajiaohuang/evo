...
describe('ITIS Rotifera exact sidecar shards', () => {
  ...
  it('limits the mixed package to the exact Rotifera root and preserves every in-scope COL species once', () => {
    expect(descriptor.packageId).toBe('other-animals')
    expect(descriptor.scope.colRootUsageId).toBe('5Y')
  ...
  it('uses exact official ITIS evidence only and resolves each detail through one shard', () => {
    expect(descriptor.sources.itis.rootTsn).toBe('58239')
