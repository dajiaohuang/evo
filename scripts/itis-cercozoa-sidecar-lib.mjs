...
export function normalizeScientificName(value) {
  const normalized = String(value ?? '').normalize('NFC').replaceAll('_', ' ').replace(/\s+/gu, ' ').trim()
...
export function colExactMatchName(record) {
  ...
  const canonical = suffix && scientificName.endsWith(suffix) ? scientificName.slice(0, -suffix.length) : scientificName
...
function synonymRecord(row) {
  return { tsn: String(row.synonym_tsn), scientificName: String(row.synonym_name), usage: String(row.synonym_usage), unacceptabilityReason: clean(row.unaccept_reason), updateDate: clean(row.synonym_update_date) }
}
...
export function createItisNameIndex(currentRows, synonymRows) {
  ...
  function resolution(name, targetTsn) {
    ...
    if (!targets.has(targetTsn)) {
      ...
      if (!current) throw new Error(`ITIS synonym target is absent from the current Cercozoa species query: ${targetTsn}`)
  ...
  for (const row of currentRows) {
    ...
    if (current.usage !== 'accepted') throw new Error(`Expected accepted ITIS Cercozoa species: ${current.tsn}/${current.usage}`)
  ...
  for (const row of synonymRows) {
    ...
    resolution(synonymRecord(row).scientificName, targetTsn).synonyms.push(synonymRecord(row))
  }
  for (const targets of names.values()) for (const candidate of targets.values()) { candidate.direct.sort(compareTsn); candidate.synonyms.sort(compareTsn) }
  return names
}

function candidateRecord(candidate) {
  return { currentName: candidate.current, evidence: [...candidate.direct.map((name) => ({ type: 'accepted-name', name })), ...candidate.synonyms.map((name) => ({ type: 'synonym', name }))] }
}
...
export function matchColSpecies(record, itisNameIndex) {
  ...
  const candidates = [...(itisNameIndex.get(exactMatchName)?.values() ?? [])].sort((left, right) => compareTsn(left.current, right.current))
  ...
  return { status: 'synonym-current-name-redirect', record: { ...base, matchedSynonyms: candidate.synonyms, currentName: candidate.current } }
}
