...
def load_col() -> list[dict[str, str]]:
...
def load_source(path: pathlib.Path) -> tuple[dict, dict[str, list[dict]], dict[str, dict]]:
    raw = path.read_bytes()
    ...
    with zipfile.ZipFile(path) as archive:
        ...
        metadata = archive.read("metadata.yml").decode("utf-8-sig")
...
def main() -> None:
    ...
    source, accepted, refs_for_name = load_source(args.archive)
    col = load_col()
    ...
    for row in col:
        ...
        if len(hits) == 1:
            item = hits[0]
            name, taxon = item["name"], item["taxon"]
            source_record = source_name(name, taxon)
            ...
            record = {"colId": row["id"], "colScientificName": row["scientificName"], "colAuthorship": row.get("authorship", ""), "status": "ambiguous", "matchedName": None, "acceptedName": None, "candidates": [source_name(x["name"], x["taxon"]) for x in hits], "mappingBasis": "Exact key has multiple source accepted rows; no winner selected.", "sourceRows": []}
            counts["ambiguous"] += 1
        else:
    ...
    for is_upstream, values in ((False, records), (True, upstream)):
        for index, part in enumerate(chunks(values)):
            ...
            entry = {"path": f"other-animals/{path.name}", "records": len(part), "bytes": len(compressed), "sha256": digest(compressed), "sourceBytes": len(raw), "sourceSha256": digest(raw)}
            if not is_upstream:
    ...
    ledger = {"schemaVersion": 1, "importType": "COL26.8-to-WoRMS-Monogenea-archive-crosswalk", "source": source, "scopeAudit": {"colStrictAcceptedSpecies": len(col), "sourceStrictAcceptedSpecies": len(accepted), "matched": counts["accepted"], "unmatched": counts["unmatched"], "ambiguous": counts["ambiguous"], "sourceOnly": len(upstream)}, "output": {"descriptorPath": str(descriptor_path.relative_to(args.output_root)).replace('\\\\', '/'), "descriptorBytes": len(descriptor_bytes), "descriptorSha256": digest(descriptor_bytes), "files": files, "upstreamOnlyFiles": upstream_files}}
