"""Project the pinned WoRMS Oligochaeta ColDP archive into a COL-scoped sidecar."""
...
ARCHIVE = ROOT / 'data/sources/archives/checklistbank-1099-oligochaeta-2026-09-01.zip'
METADATA = ROOT / 'data/sources/archives/checklistbank-1099-oligochaeta-2026-09-01.metadata.json'
ARCHIVE_SHA = '41a1388a97e661d8c749cfc5c04ced11eacf415aca56fa07f0279a7f5bcf7cdc'
ARCHIVE_BYTES = 1592402
COL_SOURCE = '1099'
COL_ROOTS = ('B8W74',)
...
def project(archive, output_root=None):
    ...
    def write_shards(prefix, rows, role):
        ...
        for index, part in enumerate(parts):
            ...
            items.append(item)
        return items
    col_files = write_shards('worms-oligochaeta', records, 'col-partition')
    source_files = write_shards('worms-oligochaeta-source-only', upstream, 'upstream-only')
    descriptor = {'schemaVersion': 1, 'recordType': 'release-pinned-authority-archive-crosswalk',
                  'id': 'worms-oligochaeta-archive-crosswalk', 'packageId': 'other-animals',
                  'provider': 'World Register of Marine Species via ChecklistBank', 'rowEncoding': 'json',
                  'colIdField': 'colId', 'totalCountField': 'total',
                  'source': {'datasetId': '1099', 'title': metadata['title'], 'version': metadata['version'],
                             'versionDoi': metadata['versionDoi'], 'doi': metadata['doi'], 'issued': metadata['issued'],
                             'citation': metadata['citation'], 'editor': metadata['editor'], 'contributor': metadata['contributor'],
                             'metadataBytes': len(metadata_bytes),
                             'metadataSha256': digest(metadata_bytes), 'license': 'CC-BY-4.0',
                             'licenseUrl': 'https://creativecommons.org/licenses/by/4.0/',
                             'archiveUrl': 'https://api.checklistbank.org/dataset/1099/archive',
                             'archivePath': 'data/sources/archives/checklistbank-1099-oligochaeta-2026-09-01.zip',
                             'metadataPath': 'data/sources/archives/checklistbank-1099-oligochaeta-2026-09-01.metadata.json',
                             'archiveBytes': len(raw), 'archiveSha256': digest(raw), 'members': members},
                  'scope': {'colRootUsageIds': list(COL_ROOTS), 'scientificName': 'Oligochaeta',
                  ...
                  'evidenceBoundary': {'en': 'Frozen WoRMS nomenclatural/source projection for the exact COL26.8 source-1099 Oligochaeta root; not species-concept equivalence, a biological dossier, fossil evidence or expert review.',
                                       'zh': '精确 COL26.8 source-1099 Oligochaeta 根节点范围的 WoRMS 冻结命名/来源投影；不是物种概念等同性、生物档案、化石证据或专家审查。'},
                  'limitations': ['Source-only rows are relative only to COL source dataset 1099.', 'Name.status is nomenclatural metadata and is not used as taxonomic acceptance.', 'Synonym.taxonID targets do not remove accepted Taxon rows.'],
                  ...
                  'deliveryProfiles': {'web-light': {'mode': 'summary-only', 'records': 0, 'files': [],
                                       ...
                                       'native-full': {'mode': 'complete', 'records': len(all_rows),
                                                       ...
                                                       'totalCompressedBytes': sum(x['bytes'] for x in col_files + source_files),
                                                       'totalSourceBytes': sum(x['sourceBytes'] for x in col_files + source_files)}}}
    descriptor_path = destination / 'worms-oligochaeta-sidecar.json'
    descriptor_bytes = dump(descriptor, True); descriptor_path.write_bytes(descriptor_bytes)
    ledger = {'schemaVersion': 1, 'importType': 'COL26.8-to-WoRMS-1099-archive-projection',
              'source': descriptor['source'], 'registryManifestSha256': col_sha, 'registryInputs': col_inputs,
              'generatedBy': {'script': 'scripts/build-worms-oligochaeta-source.py', 'scriptSha256': script_digest(Path(__file__)), 'hashNormalization': 'LF'},
              'outputs': {'descriptor': {'path': 'data/catalogue-of-life/releases/2026-08-20/resource-packs/other-animals/worms-oligochaeta-sidecar.json', 'bytes': len(descriptor_bytes), 'sha256': digest(descriptor_bytes)},
    ...
    ledger_path = output_base / 'data/sources/worms-oligochaeta-archive-1099-import-ledger.json'
