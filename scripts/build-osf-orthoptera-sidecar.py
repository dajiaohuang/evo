...
import csv, gzip, hashlib, json, os, zipfile
...
CACHE = Path('D:/repo/repostew/.repostew/source-cache/osf-1021-2026-09-04')
ARCHIVE = CACHE / 'dataset-1021.zip'
ACQ = CACHE / 'acquisition.json'
...
def col_species():
    files = [REG / Path(item['path']) for item in json.loads((REG / 'manifest.json').read_text())['hierarchy']['nodes']['files']]
    parents, species = {}, []
    for path in files:
        with gzip.open(path, 'rt', encoding='utf-8') as stream:
            for line in stream:
                row = json.loads(line); parents[row['id']] = row.get('parentId')
                if row.get('rank') == 'species' and row.get('status') == 'accepted' and row.get('sourceDatasetId') == '1021': species.append(row)
    def in_root(row):
        ...
        return False
    return sorted((x for x in species if in_root(x)), key=lambda x: x['id'])

def rows_for(*rows): return [{'member': x['__member'], 'row': x['__row']} for x in rows if x]
def source_name(name, taxon):
    return {'id': taxon['ID'], 'nameId': name['ID'], 'scientificName': name['scientificName'], 'authorship': name['authorship'] or None, 'status': 'accepted', 'url': taxon.get('link') or None}
def strip_col(row):
...
def main():
    acquisition_bytes = ACQ.read_bytes(); acquisition = json.loads(acquisition_bytes)
    ...
    for taxon in taxa:
        ...
        if not name or name.get('rank') != 'species': continue
        parent = taxon['ID']
        while parent:
    ...
    for row in col:
        ...
        rec = {'colId': row['id'], 'colScientificName': row['scientificName'], 'colAuthorship': row.get('authorship') or None, 'matchedName': None, 'acceptedName': None, 'candidates': [], 'mappingBasis': None, 'sourceRows': []}
        if len(hits) == 1:
            ...
            relations = syns.get(key, []); qualifying = [(s, n, t) for s, n, t in relations if t and by_name.get(t.get('nameID'), {}).get('rank') == 'species' and any(t is ot for ot, on in osf_taxa)]
            targets = {t['ID']: (s, n, t) for s, n, t in qualifying}
            if len(targets) == 1:
                s, n, t = next(iter(targets.values())); value = source_name(by_name[t['nameID']], t); rec.update(status='redirect', matchedName=value, acceptedName=value, mappingBasis='Explicit OSF Synonym.tsv relation to an accepted species target.', sourceRows=sum((rows_for(s, n, t) for s, n, t in targets.values()), [])); evidenced.add(t['ID']); counts['redirect'] += 1
            elif len(targets) > 1:
    ...
    package_count = json.loads((ROOT / 'data/registry/package-species-coverage.json').read_text(encoding='utf-8'))['packageCounts'][PACKAGE_ID]; manifest_path = REG / 'manifest.json'; manifest_bytes = manifest_path.read_bytes()
