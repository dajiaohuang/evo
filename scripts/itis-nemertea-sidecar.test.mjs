...
const descriptorPath = join(root, 'data/catalogue-of-life/releases/2026-08-20/resource-packs/other-animals/itis-nemertea-sidecar.json')
...
describe('ITIS Nemertea exact sidecar shards', () => {
  ...
  it('covers the complete strict COL26.8 Nemertea partition once in deterministic non-overlapping ranges', () => {
    expect(descriptor.packageId).toBe('other-animals'); expect(descriptor.scope.colRootUsageId).toBe('5C'); expect(descriptor.scope.colStrictAcceptedSpecies).toBe(1364); expect(descriptor.scope.packageOutOfScopeStrictAcceptedSpecies).toBe(97797); expect(descriptorBytes.length).toBeLessThan(64 * 1024)
    expect(rows).toHaveLength(1364); expect(new Set(rows.map((row) => row.colUsageId)).size).toBe(rows.length); expect(files.every((file) => file.sourceBytes <= descriptor.colUsageIdLocator.sourceShardLimitBytes)).toBe(true)
  ...
  it('retains only exact ITIS evidence and makes each COL record addressable through one shard', () => {
    expect(descriptor.sources.itis.rootTsn).toBe('57411'); expect(descriptor.exactMatching.prohibited).toContain('No fuzzy')
  ...
  it('separates ITIS-only current species and retains byte-exact mobile delivery evidence', () => {
    expect(upstream).toHaveLength(52); expect(upstream.every((row) => row.colUsageId === null && row.currentName.usage === 'valid')).toBe(true)
