...
spec = importlib.util.spec_from_file_location('ctenophora', Path(__file__).with_name('build-worms-ctenophora-source.py'))
mod = importlib.util.module_from_spec(spec); spec.loader.exec_module(mod)


class CtenophoraProjectionTests(unittest.TestCase):
    def test_real_offline_rebuild_is_deterministic_and_preserves_scope_counts(self):
        canonical_ledger = ROOT / 'data/sources/worms-ctenophora-archive-1180-import-ledger.json'
        ...
        with tempfile.TemporaryDirectory() as one, tempfile.TemporaryDirectory() as two:
            ...
            names = sorted(p.name for p in roots[0].glob('worms-ctenophora*.json.gz')) + ['worms-ctenophora-sidecar.json']
            for name in names:
            ...
            descriptor = json.loads((roots[0] / 'worms-ctenophora-sidecar.json').read_text(encoding='utf8'))
            ledgers = [Path(one) / 'data/sources/worms-ctenophora-archive-1180-import-ledger.json', Path(two) / 'data/sources/worms-ctenophora-archive-1180-import-ledger.json']
