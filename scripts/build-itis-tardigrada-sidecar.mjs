...
const descriptorPath = join(nomenclatureRoot, 'itis-tardigrada-sidecar.json')
const ledgerPath = join(root, 'data/sources/itis-tardigrada-sidecar-import-ledger.json')
const ITIS_ROOT_TSN = 155166
const COL_ROOT_USAGE_ID = 'L2QNW'
const PACKAGE_ID = 'other-animals'
const SHARD_SOURCE_LIMIT_BYTES = 512 * 1024

const currentSpeciesQuery = `WITH RECURSIVE tardigrada_descendants(tsn) AS (
  SELECT ?1
  UNION ALL
SELECT u.tsn FROM taxonomic_units u JOIN tardigrada_descendants d ON u.parent_tsn = d.tsn
...
const synonymQuery = `WITH RECURSIVE tardigrada_descendants(tsn) AS (
  SELECT ?1
  UNION ALL
SELECT u.tsn FROM taxonomic_units u JOIN tardigrada_descendants d ON u.parent_tsn = d.tsn
  WHERE u.name_usage = 'valid'
), accepted_tardigrada_species(tsn) AS (
SELECT u.tsn FROM tardigrada_descendants d
  JOIN taxonomic_units u ON u.tsn = d.tsn
  JOIN taxon_unit_types r ON r.kingdom_id = u.kingdom_id AND r.rank_id = u.rank_id
  WHERE lower(trim(r.rank_name)) = 'species' AND u.name_usage = 'valid'
)
...
JOIN accepted_tardigrada_species a ON a.tsn = s.tsn_accepted
...
async function loadColTardigradaSpecies(manifest) {
...
if (!rootRecord || rootRecord.completename !== 'Tardigrada' || rootRecord.rank_name !== 'Phylum' || rootRecord.name_usage !== 'valid') {
      throw new Error('Pinned ITIS Tardigrada root no longer has the expected valid phylum identity')
    }
...
async function main() {
  ...
  if (argument < 0 || !process.argv[argument + 1]) throw new Error('Usage: node scripts/build-itis-tardigrada-sidecar.mjs --itis-sqlite <verified ITIS.sqlite>')
...
const colSpecies = await loadColTardigradaSpecies(JSON.parse(registryBytes))
...
if (new Set(crosswalk.map((record) => record.colUsageId)).size !== colSpecies.length) throw new Error('COL Tardigrada records are not uniquely addressable')
  const upstreamOnly = currentRows.filter((row) => !evidencedTsns.has(String(row.tsn))).map((row) => ({
    ...
    basis: 'No strict COL26.8 Tardigrada accepted-species name or official ITIS species-synonym evidence resolves to this current ITIS TSN.',
  })).sort((left, right) => Number(left.currentName.tsn) - Number(right.currentName.tsn))
  mkdirSync(nomenclatureRoot, { recursive: true })
for (const name of readdirSync(nomenclatureRoot)) if (/^itis-tardigrada-(?:sidecar|upstream-only)-\d{3,4}\.jsonl\.gz$/u.test(name)) rmSync(join(nomenclatureRoot, name))
  const shards = chunkBySourceBytes(crosswalk).map((records, indexNumber) => {
const filename = `itis-tardigrada-sidecar-${String(indexNumber).padStart(4, '0')}.jsonl.gz`
...
const upstreamPath = join(nomenclatureRoot, 'itis-tardigrada-upstream-only-0000.jsonl.gz')
  ...
  const descriptor = {
    ...
    scope: {
      packageRootUsageId: 'N', packageRootScientificName: 'Animalia',
      colRootUsageId: COL_ROOT_USAGE_ID, colRootScientificName: 'Tardigrada', colStrictAcceptedSpecies: colSpecies.length,
      packageStrictAcceptedSpecies: packageCount, packageOutOfScopeStrictAcceptedSpecies: packageCount - colSpecies.length,
      nonApplicableRemainder: 'Every strict accepted COL26.8 species owned by the other-animals remainder route below N but not descending from L2QNW; this complete remainder is explicitly excluded from the Tardigrada sidecar.',
      boundary: 'This mixed package contains many animal phyla and broader Animalia navigation content. This sidecar covers only the strict accepted COL26.8 species descending from the exact Tardigrada root L2QNW; all remaining package-owned species are explicitly out of scope.',
    },
    sources: {
    ...
    exactMatching: {
      ...
      statuses: {
accepted: 'The normalized COL name resolves to exactly one valid ITIS Tardigrada species and directly equals that current ITIS name.',
'synonym-current-name-redirect': 'The normalized COL name equals official ITIS invalid species-name evidence that resolves to exactly one valid ITIS Tardigrada species.',
ambiguous: 'The normalized exact evidence resolves to more than one valid ITIS Tardigrada species TSN.',
unmatched: 'No normalized exact valid-name or official ITIS species-synonym evidence resolves to a valid ITIS Tardigrada species.',
      },
    ...
    evidenceBoundary: {
en: 'This CC0 ITIS sidecar is a frozen exact nomenclatural crosswalk, not a global tardigrade checklist, final classification authority, phylogeny, species-concept equivalence assertion, biological dossier or scientific-review record.',
      zh: 'æ­¤ CC0 ITIS ä¾§è½¦æ˜¯å†»ç»“çš„ä¸¥æ ¼å‘½åäº¤å‰æ˜ å°„ï¼›å®ƒä¸æ˜¯å…¨çƒæ˜†è™«åå½•ã€æœ€ç»ˆåˆ†ç±»æƒå¨ã€ç³»ç»Ÿå‘è‚²æ ‘ã€ç‰©ç§æ¦‚å¿µç­‰åŒæ€§å£°æ˜Žã€ç”Ÿç‰©æ¡£æ¡ˆæˆ–ç§‘å­¦å®¡æŸ¥è®°å½•ã€‚',
    },
  ...
  const ledger = {
schemaVersion: 1, importType: 'COL26.8-to-ITIS-exact-tardigrada-nomenclatural-sidecar',
...
generatedBy: { scriptPath: 'scripts/build-itis-tardigrada-sidecar.mjs', scriptSha256: await sha256File(fileURLToPath(import.meta.url)), deterministic: 'Pinned input checksums, fixed roots, exact SQL, exact representation-only normalization, code-unit ID ordering and deterministic gzip; no wall-clock fields or fuzzy matching.' },
  }
