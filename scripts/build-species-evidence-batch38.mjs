...
const INPUT = 'data/sources/species-evidence-batch38-2026-09-25.json'
const OUTPUT = 'data/knowledge/species-evidence-batch38-2026-09-25.batch-manifest.json'
const NEW_RAW = 'data/knowledge/raw-dossiers/species-evidence-batch38-2026-09-25.jsonl'
const NEW_SHARD = 'data/knowledge/catalogue-dossiers-species-evidence-batch38-2026-09-25.jsonl.br'
...
const EXPECTED_INPUT_SHA256 = '14197972f6d1f53df2fad15d435d62e3c84f135d0041f2fa386b7b7debf9836c'
...
assert.equal(input.batchId, 'species-evidence-batch38-2026-09-25')
assert.equal(input.releaseAlias, 'COL26.8')
assert.equal(input.baseAudit.indexedRecordCount, 6963)
assert.equal(input.baseAudit.indexedShardCount, 61)
...
const expectedShardCount = input.baseAudit.indexedShardCount + Number(index.shards.some(shard => shard.path === NEW_SHARD))
assert.equal(index.shards.length, expectedShardCount, 'Unexpected dossier shard base count')
  ...
  if (!alreadyApplied) {
    ...
    for (const update of input.updates.filter(item => item.targetShardPath === targetShard)) {
      ...
      const supplement = {
        ...
        generator: 'scripts/build-species-evidence-batch38.mjs',
...
console.log(JSON.stringify({ updates: appliedUpdates, newRecords: newBuilt.map(({ colId, scientificName }) => ({ colId, scientificName })), indexedRecordsAfter: index.recordCount, newRawSha256: sha256(newRawBytes), newCompressedSha256: sha256(newCompressedBytes), rebuild: newAlreadyExists && appliedUpdates.every(item => item.alreadyApplied) ? 'verified-idempotent-rebuild' : 'applied-species-evidence-batch' }, null, 2))
