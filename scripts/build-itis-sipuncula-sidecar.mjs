...
const DESCRIPTOR_PATH = join(OUTPUT_ROOT, 'itis-sipuncula-sidecar.json')
const LEDGER_PATH = join(REPOSITORY_ROOT, 'data', 'sources', 'itis-sipuncula-sidecar-import-ledger.json')
const COL_ROOT_USAGE_ID = '63'
const COL_ROOT_NAME = 'Sipuncula'
const ITIS_ROOT_TSN = 154520
const PACKAGE_ID = 'other-animals'
const SHARD_SOURCE_LIMIT_BYTES = 512 * 1024

const currentSpeciesQuery = `WITH RECURSIVE sipuncula_descendants(tsn) AS (
  SELECT ?1
  UNION ALL
  SELECT u.tsn FROM taxonomic_units u
  JOIN sipuncula_descendants d ON u.parent_tsn = d.tsn
...
const synonymQuery = `WITH RECURSIVE sipuncula_descendants(tsn) AS (
  SELECT ?1
  UNION ALL
  SELECT u.tsn FROM taxonomic_units u
  JOIN sipuncula_descendants d ON u.parent_tsn = d.tsn
  WHERE u.name_usage = 'valid'
), accepted_sipuncula_species(tsn) AS (
  SELECT u.tsn FROM sipuncula_descendants d
  JOIN taxonomic_units u ON u.tsn = d.tsn
  JOIN taxon_unit_types r ON r.kingdom_id = u.kingdom_id AND r.rank_id = u.rank_id
  WHERE lower(trim(r.rank_name)) = 'species' AND u.name_usage = 'valid'
)
...
JOIN accepted_sipuncula_species a ON a.tsn = s.tsn_accepted
...
async function loadColSipunculaSpecies(manifest) {
  const nodes = new Map()
  const species = []
...
function loadItis(sqlitePath) {
  ...
  try {
    ...
    if (!root || root.completename !== 'Sipuncula' || root.rank_name !== 'Phylum' || root.name_usage !== 'valid') {
      throw new Error('Pinned ITIS Sipuncula root no longer has the expected valid phylum identity')
    }
...
async function main() {
  ...
  if (argument < 0 || !process.argv[argument + 1]) throw new Error('Usage: node scripts/build-itis-sipuncula-sidecar.mjs --itis-sqlite <verified ITIS.sqlite>')
  ...
  const colSpecies = await loadColSipunculaSpecies(JSON.parse(registryBytes))
  const packageIds = await loadPackageSpecies(resourcePack)
  if (!colSpecies.every((record) => packageIds.has(record.id))) throw new Error('COL Sipuncula species escaped the other-animals ownership boundary')
  const nonApplicable = resourcePack.acceptedSpeciesCount - colSpecies.length
  if (nonApplicable !== 99015) throw new Error(`Unexpected other-animals non-applicable remainder: ${nonApplicable}`)
  ...
  if (itis.currentRows.length !== 205 || itis.synonymRows.length !== 575) throw new Error(`Unexpected ITIS Sipuncula scope: ${itis.currentRows.length}/${itis.synonymRows.length}`)
  ...
  if (new Set(crosswalk.map((record) => record.colUsageId)).size !== colSpecies.length) throw new Error('COL Sipuncula records are not uniquely addressable')
  const upstreamOnly = itis.currentRows.filter((row) => !evidencedTsns.has(String(row.tsn))).map((row) => ({
    ...
    basis: 'No strict COL26.8 Sipuncula accepted-species name or official ITIS species-synonym evidence resolves to this current ITIS Sipuncula species.',
  })).sort((left, right) => Number(left.currentName.tsn) - Number(right.currentName.tsn))
  for (const name of readdirSync(OUTPUT_ROOT)) if (/^itis-sipuncula-(?:sidecar|upstream-only)-\d{3}\.jsonl\.gz$/u.test(name)) rmSync(join(OUTPUT_ROOT, name))
  const shards = chunkBySourceBytes(crosswalk).map((records, indexNumber) => {
    const filename = `itis-sipuncula-sidecar-${String(indexNumber).padStart(3, '0')}.jsonl.gz`
  ...
  const upstreamPath = join(OUTPUT_ROOT, 'itis-sipuncula-upstream-only-000.jsonl.gz')
  ...
  const descriptor = {
    ...
    scope: {
      ...
      packageOwnership: 'other-animals is the deterministic COL26.8 residual route below Animalia (N); Sipuncula 63 has no more-specific static-package route.',
      nonApplicableBoundary: 'All other-animals strict accepted species not descending from 63 are non-applicable to this Sipuncula sidecar. They remain in the mixed resource pack and are not silently mapped to ITIS Sipuncula.',
      sourceComposition,
    },
    sources: {
      col: {
    ...
    exactMatching: {
      ...
      statuses: {
        accepted: 'The normalized COL name resolves to exactly one valid ITIS Sipuncula species and directly equals that current ITIS name.',
        'synonym-current-name-redirect': 'The normalized COL name equals official ITIS invalid species-name evidence that resolves to exactly one valid ITIS Sipuncula species.',
        ambiguous: 'The normalized exact evidence resolves to more than one valid ITIS Sipuncula species TSN.',
        unmatched: 'No normalized exact valid-name or official ITIS species-synonym evidence resolves to a valid ITIS Sipuncula species.',
      },
  ...
  const ledger = {
    schemaVersion: 1,
    importType: 'COL26.8-to-ITIS-exact-sipuncula-nomenclatural-sidecar',
