...
const PACKAGE_ROOT = join(ROOT, 'data/packages/invertebrata/sponges-cnidarians')
const NOMENCLATURE_ROOT = join(PACKAGE_ROOT, 'nomenclature')
const DESCRIPTOR_PATH = join(NOMENCLATURE_ROOT, 'itis-porifera-cnidaria-sidecar.json')
const LEDGER_PATH = join(ROOT, 'data/sources/itis-porifera-cnidaria-sidecar-import-ledger.json')
const PACKAGE_ID = 'sponges-cnidarians'
const SHARD_SOURCE_LIMIT_BYTES = 2 * 1024 * 1024
const COL_ROOTS = [
  { usageId: 'B8TXQ', scientificName: 'Porifera', itisTsn: 46861 },
  { usageId: 'CN2', scientificName: 'Cnidaria', itisTsn: 48738 },
]
...
async function main() {
  ...
  if (argument < 0 || !process.argv[argument + 1]) throw new Error('Usage: node scripts/build-itis-porifera-cnidaria-sidecars.mjs --itis-sqlite <verified ITIS.sqlite>')
  ...
  if (ownership.packageCounts[PACKAGE_ID] !== 30521) throw new Error(`Unexpected COL package boundary: ${ownership.packageCounts[PACKAGE_ID]}`)
  ...
  const upstreamOnly = itis.currentRows.filter((row) => !evidencedTsns.has(String(row.tsn))).map((row) => ({
    ...
    basis: 'No strict COL26.8 accepted species or official ITIS species-synonym evidence resolves to this current ITIS Porifera/Cnidaria species; it remains ITIS-only upstream data.',
  }))
  mkdirSync(NOMENCLATURE_ROOT, { recursive: true })
  for (const name of readdirSync(NOMENCLATURE_ROOT)) if (/^itis-(?:porifera-cnidaria-sidecar|porifera-cnidaria-upstream-only)-\d{4}\.jsonl\.gz$/u.test(name)) rmSync(join(NOMENCLATURE_ROOT, name))
  const shards = chunkBySourceBytes(matches).map((chunk, index) => {
    ...
    const path = join(NOMENCLATURE_ROOT, `itis-porifera-cnidaria-sidecar-${String(index).padStart(4, '0')}.jsonl.gz`)
  ...
  const upstreamPath = join(NOMENCLATURE_ROOT, 'itis-porifera-cnidaria-upstream-only-0000.jsonl.gz')
  ...
  const descriptor = {
    ...
    scope: {
      ...
      nonApplicableBoundary: 'This mixed-package sidecar applies only to the exact COL26.8 Porifera B8TXQ and Cnidaria CN2 usage roots. The package has no additional strict accepted-species roots; all COL26.8 species outside these two roots, including other Animalia and non-species records, are non-applicable and are not emitted.',
    },
    sources: {
    ...
    exactMatching: {
      ...
      statuses: {
        accepted: 'The normalized COL name resolves to exactly one valid ITIS Porifera/Cnidaria species and directly equals that current ITIS name.',
        'synonym-current-name-redirect': 'The normalized COL name equals official ITIS invalid species-name evidence that resolves to exactly one valid ITIS Porifera/Cnidaria species.',
        ambiguous: 'The normalized exact evidence resolves to more than one valid ITIS Porifera/Cnidaria species TSN.',
        unmatched: 'No normalized exact valid-name or official ITIS species-synonym evidence resolves to a valid ITIS Porifera/Cnidaria species.',
      },
    ...
    evidenceBoundary: {
      en: 'This CC0 ITIS sidecar is a frozen exact nomenclatural crosswalk for the declared Porifera and Cnidaria partitions only. It is not a final classification authority, phylogeny, species-concept equivalence assertion, biological dossier, fossil record or expert-review record.',
      zh: '此 CC0 ITIS 侧车仅为明确声明的海绵动物与刺胞动物分区提供冻结的严格命名交叉映射。它不是最终分类权威、系统发育、物种概念等同性声明、生物档案、化石记录或专家审阅记录。',
    },
    counts,
  ...
  const ledger = {
    schemaVersion: 1,
    importType: 'COL26.8-to-ITIS-exact-porifera-cnidaria-nomenclatural-sidecar',
