...
describe('Nematoda resource-pack integration', () => {
  it('preserves unrelated extensions and collection summary across repeated integration', async () => {
    ...
    const unrelated = { id: 'unrelated-authority', files: [{ path: 'other.json.gz', bytes: 11, sourceBytes: 13 }], upstreamOnlyFiles: [{ path: 'other-upstream.json.gz', bytes: 17, sourceBytes: 19 }] }
    ...
    const firstManifest = JSON.parse(await readFile(join(pack, 'manifest.json'), 'utf8'))
    const firstCollection = JSON.parse(await readFile(collectionPath, 'utf8'))
    ...
    expect(firstCollection.packs[0]).toMatchObject({ extensionCount: 2, extensionFileCount: 4 })
    const second = integrateWormsNematoda({ rootDir: root })
    expect(second).toEqual(first)
    expect(JSON.parse(await readFile(join(pack, 'manifest.json'), 'utf8')).extensions.map(({ id }) => id)).toEqual(['unrelated-authority', 'worms-nematoda-archive-crosswalk'])
    expect(JSON.parse(await readFile(collectionPath, 'utf8')).packs[0].manifestSha256).toBe(firstCollection.packs[0].manifestSha256)
  })
})
