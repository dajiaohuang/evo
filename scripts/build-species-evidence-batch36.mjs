...
const INPUT = 'data/sources/species-evidence-batch36-2026-09-25.json'
const OUTPUT = 'data/knowledge/species-evidence-batch36-2026-09-25.batch-manifest.json'
const NEW_RAW = 'data/knowledge/raw-dossiers/species-evidence-batch36-2026-09-25.jsonl'
const NEW_SHARD = 'data/knowledge/catalogue-dossiers-species-evidence-batch36-2026-09-25.jsonl.br'
...
const EXPECTED_INPUT_SHA256 = '17a31b0357843d5801c05244bcf4d51180bcf423243e2f345ded2652ce1ecdcf'
...
assert.equal(input.batchId, 'species-evidence-batch36-2026-09-25')
assert.equal(input.releaseAlias, 'COL26.8')
assert.equal(input.baseAudit.indexedRecordCount, 6957)
assert.equal(input.baseAudit.indexedShardCount, 59)
  ...
  if (!alreadyApplied) {
    ...
    for (const update of input.updates.filter(item => item.targetShardPath === targetShard)) {
      ...
      const supplement = {
        ...
        generator: 'scripts/build-species-evidence-batch36.mjs',
