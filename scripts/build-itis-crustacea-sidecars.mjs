...
const CANONICAL_PATH = join(ROOT, 'data/sources/itis-crustacea-authority-crosswalk-col26.8.json.gz')
...
async function main() {
  ...
  const canonicalSource=jsonBytes(canonical);const canonicalBytes=Buffer.from(deterministicGzip(canonicalSource,{level:9}));mkdirSync(dirname(CANONICAL_PATH),{recursive:true});writeFileSync(CANONICAL_PATH,canonicalBytes);const result=makeRuntimeDescriptor({allMatches,upstreamOnly,source:{...source,currentRows:itis.currentRows,synonymRows:itis.synonymRows},sourceBytes,registryManifestBytes,ownershipBytes,canonicalBytes})
