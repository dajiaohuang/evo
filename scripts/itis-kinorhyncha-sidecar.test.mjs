...
describe('ITIS Kinorhyncha exact sidecar', () => {
  ...
  it('covers every strict COL26.8 Kinorhyncha species in deterministic non-overlapping ranges', () => {
    expect(descriptor.packageId).toBe('other-animals'); expect(descriptor.scope.colRootUsageId).toBe('B8VF5'); expect(descriptor.scope.colStrictAcceptedSpecies).toBe(362); expect(descriptor.scope.packageOutOfScopeStrictAcceptedSpecies).toBe(98799); expect(rows).toHaveLength(362); expect(new Set(rows.map((row) => row.colUsageId)).size).toBe(rows.length); expect(files.every((file) => file.sourceBytes <= descriptor.colUsageIdLocator.sourceShardLimitBytes)).toBe(true)
