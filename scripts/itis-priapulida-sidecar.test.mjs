...
const descriptorPath = join(packRoot, 'itis-priapulida-sidecar.json')
const ledgerPath = join(root, 'data', 'sources', 'itis-priapulida-sidecar-import-ledger.json')
...
describe('ITIS Priapulida exact sidecar shards', () => {
  ...
  it('pins the complete strict COL26.8 Priapulida partition', () => {
    expect(descriptor.packageId).toBe('other-animals')
    expect(descriptor.scope.colRootUsageId).toBe('B8VF9')
expect(descriptor.scope.colRootScientificName).toBe('Priapulida Théel, 1906')
    expect(descriptor.scope.colStrictAcceptedSpecies).toBe(23)
    expect(descriptor.scope.packageStrictAcceptedSpecies).toBe(99161)
    expect(descriptor.scope.packageOutOfScopeStrictAcceptedSpecies).toBe(99138)
    expect(descriptor.deliveryProfiles['web-light']).toMatchObject({ payload: 'summary-only', records: 0, files: [] })
    expect(descriptor.deliveryProfiles['native-full']).toMatchObject({ payload: 'complete', records: 23 })
    expect(descriptorBytes.length).toBeLessThan(64 * 1024)
    expect(rows).toHaveLength(23)
    expect(new Set(rows.map((record) => record.colUsageId)).size).toBe(23)
  ...
  it('checks descriptor, ledger and delivery-boundary hashes', () => {
    expect(descriptor.counts).toMatchObject({ total: 23, accepted: 19, synonymCurrentNameRedirect: 0, ambiguous: 0, unmatched: 4, itisCurrentSpecies: 19, itisSpeciesSynonymLinks: 2, itisUpstreamOnly: 0 })
    ...
    expect(ledger.scopeAudit.itisRoot).toEqual({ tsn: '563953', scientificName: 'Priapula', rank: 'Phylum', usage: 'valid' })
