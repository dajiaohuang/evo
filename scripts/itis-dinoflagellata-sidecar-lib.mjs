...
export function createItisNameIndex(currentRows, synonymRows) {
  ...
  function resolution(name, targetTsn) {
    ...
    if (!targets.has(targetTsn)) {
      ...
      if (!current) throw new Error(`ITIS synonym target is absent from the current Dinophyceae species query: ${targetTsn}`)
  ...
  for (const row of currentRows) {
    ...
    if (current.usage !== 'accepted') throw new Error(`Expected accepted ITIS Dinophyceae species: ${current.tsn}/${current.usage}`)
