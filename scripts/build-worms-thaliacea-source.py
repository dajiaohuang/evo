"""Project the pinned WoRMS Thaliacea ColDP archive into a COL-scoped sidecar."""
...
ARCHIVE = ROOT / 'data/sources/archives/checklistbank-1185-thaliacea-2026-09-01.zip'
METADATA = ROOT / 'data/sources/archives/checklistbank-1185-thaliacea-2026-09-01.metadata.json'
ARCHIVE_SHA = '75b42f6c9ec693068ccaf2c28bdc34ec28a786692b633f6226c311d3d348835a'
ARCHIVE_BYTES = 59152
COL_SOURCE = '1185'
COL_ROOTS = ('L2QHG',)
...
def project(archive, output_root=None):
    ...
    def write_shards(prefix, rows, role):
        ...
        for index, part in enumerate(parts):
            ...
            items.append(item)
        return items
    col_files = write_shards('worms-thaliacea', records, 'col-partition')
    source_files = write_shards('worms-thaliacea-source-only', upstream, 'upstream-only')
    descriptor = {'schemaVersion': 1, 'recordType': 'release-pinned-authority-archive-crosswalk',
                  'id': 'worms-thaliacea-archive-crosswalk', 'packageId': 'other-animals',
                  ...
                  'source': {'datasetId': '1185', 'title': metadata['title'], 'version': metadata['version'],
                             ...
                             'archiveUrl': 'https://api.checklistbank.org/dataset/1185/archive',
                             'archivePath': 'data/sources/archives/checklistbank-1185-thaliacea-2026-09-01.zip',
                             'metadataPath': 'data/sources/archives/checklistbank-1185-thaliacea-2026-09-01.metadata.json',
                             'archiveBytes': len(raw), 'archiveSha256': digest(raw), 'members': members},
                  'scope': {'colRootUsageIds': list(COL_ROOTS), 'scientificName': 'Thaliacea',
                  ...
                  'evidenceBoundary': {'en': 'Frozen WoRMS nomenclatural/source projection for the exact COL26.8 source-1185 Thaliacea root; not species-concept equivalence, a biological dossier, fossil evidence or expert review.',
                                       'zh': '精确 COL26.8 source-1185 Thaliacea 根节点范围的 WoRMS 冻结命名/来源投影；不是物种概念等同性、生物档案、化石证据或专家审查。'},
                  'limitations': ['Source-only rows are relative only to COL source dataset 1185.', 'Name.status is nomenclatural metadata and is not used as taxonomic acceptance.', 'Synonym.taxonID targets do not remove accepted Taxon rows.'],
                  ...
                  'deliveryProfiles': {'web-light': {'mode': 'summary-only', 'records': 0, 'files': [],
                                       ...
                                       'native-full': {'mode': 'complete', 'records': len(all_rows),
                                                       ...
                                                       'totalCompressedBytes': sum(x['bytes'] for x in col_files + source_files),
                                                       'totalSourceBytes': sum(x['sourceBytes'] for x in col_files + source_files)}}}
    descriptor_path = destination / 'worms-thaliacea-sidecar.json'
    descriptor_bytes = dump(descriptor, True); descriptor_path.write_bytes(descriptor_bytes)
    ledger = {'schemaVersion': 1, 'importType': 'COL26.8-to-WoRMS-1185-archive-projection',
              'source': descriptor['source'], 'registryManifestSha256': col_sha, 'registryInputs': col_inputs,
              'generatedBy': {'script': 'scripts/build-worms-thaliacea-source.py', 'scriptSha256': script_digest(Path(__file__)), 'hashNormalization': 'LF'},
              'outputs': {'descriptor': {'path': 'data/catalogue-of-life/releases/2026-08-20/resource-packs/other-animals/worms-thaliacea-sidecar.json', 'bytes': len(descriptor_bytes), 'sha256': digest(descriptor_bytes)},
    ...
    ledger_path = output_base / 'data/sources/worms-thaliacea-archive-1185-import-ledger.json'
