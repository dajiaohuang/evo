...
describe('ITIS Ctenophora exact sidecar shards', () => {
  ...
  const rows = files.flatMap((file, index) => readShard(file, index === 0 || index === files.length - 1))
  const upstream = descriptor.upstreamOnly.files.flatMap((file) => readShard(file, true))

  it('covers the exact COL26.8 Ctenophora partition once in bounded ranges', () => {
    expect(descriptor.packageId).toBe('other-animals')
    expect(descriptor.scope.colRoots).toEqual([{ id: 'B8V3L', scientificName: 'Ctenophora' }])
    expect(descriptor.scope.speciesByColRoot).toEqual({ B8V3L: 197 })
    expect(descriptor.scope.colStrictAcceptedSpecies).toBe(197)
    expect(descriptor.scope.packageOutOfScopeStrictAcceptedSpecies).toBe(98964)
    ...
    for (const [index, file] of files.entries()) {
      const shardRows = readShard(file)
  ...
  it('retains only exact ITIS evidence and locates every row in one shard', () => {
    expect(descriptor.sources.itis.roots).toEqual([{ tsn: '53856', scientificName: 'Ctenophora' }])
  ...
  it('separates ITIS-only species and records the mixed Other Animals boundary', () => {
    ...
    expect(descriptor.mixedResourcePack.inScopeSpecies + descriptor.mixedResourcePack.nonApplicableSpecies).toBe(descriptor.mixedResourcePack.acceptedSpeciesCount)
    expect(descriptor.mixedResourcePack.nonApplicableSpecies).toBe(98964)
