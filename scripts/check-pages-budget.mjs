...
const coreFiles = files.filter((path) => /[/\\]data[/\\]core[/\\]/.test(path))
...
const packageIds = existsSync(join(dist, 'data/packages')) ? readdirSync(join(dist, 'data/packages')) : []
for (const packageId of packageIds) {
  const packageFiles = filesBelow(join(dist, 'data/packages', packageId)).filter((path) => path.endsWith('.json.gz'))
  const searchPath = join(dist, 'data/package-search-index', `${packageId}.json.gz`)
...
if (!existsSync(swPath)) {
  ...
  if (/data\/(packages|occurrences|maps|downloads)\//.test(serviceWorker)) failures.push('service worker precaches package, occurrence, map or download data')
}
