...
const ITIS_LIVING_CLASS_BREAKDOWN = [
  { tsn: '154400', scientificName: 'Chilopoda', currentSpecies: 3197 },
  { tsn: '154409', scientificName: 'Diplopoda', currentSpecies: 2061 },
  { tsn: '154425', scientificName: 'Pauropoda', currentSpecies: 995 },
  { tsn: '154408', scientificName: 'Symphyla', currentSpecies: 235 },
]

const currentSpeciesQuery = `WITH RECURSIVE myriapoda_descendants(tsn) AS (
  SELECT ?1
  UNION ALL
  SELECT u.tsn FROM taxonomic_units u JOIN myriapoda_descendants d ON u.parent_tsn = d.tsn
...
const synonymQuery = `WITH RECURSIVE myriapoda_descendants(tsn) AS (
  SELECT ?1
  UNION ALL
  SELECT u.tsn FROM taxonomic_units u JOIN myriapoda_descendants d ON u.parent_tsn = d.tsn
  WHERE u.name_usage = 'valid'
), accepted_myriapoda_species(tsn) AS (
  SELECT u.tsn FROM myriapoda_descendants d
  JOIN taxonomic_units u ON u.tsn = d.tsn
  JOIN taxon_unit_types r ON r.kingdom_id = u.kingdom_id AND r.rank_id = u.rank_id
  WHERE lower(trim(r.rank_name)) = 'species' AND u.name_usage = 'valid'
)
...
JOIN accepted_myriapoda_species a ON a.tsn = s.tsn_accepted
...
async function loadColMyriapodaSpecies(manifest) {
  ...
  return { species: descendants.sort((left, right) => compareCodeUnits(left.id, right.id)), excludedSpecies }
}
...
function loadItis(sqlitePath) {
  ...
  try {
    ...
    if (!rootRecord || rootRecord.completename !== 'Myriapoda' || rootRecord.rank_name !== 'Subphylum' || rootRecord.name_usage !== 'valid') {
      throw new Error('Pinned ITIS Myriapoda root no longer has the expected valid subphylum identity')
    }
...
async function main() {
  ...
  if (argument < 0 || !process.argv[argument + 1]) throw new Error('Usage: node scripts/build-itis-myriapoda-sidecar.mjs --itis-sqlite <verified ITIS.sqlite>')
  ...
  const colResult = await loadColMyriapodaSpecies(JSON.parse(registryBytes))
  const colSpecies = colResult.species
  if (colSpecies.length !== COL_LIVING_CLASS_BREAKDOWN.reduce((sum, item) => sum + item.acceptedSpecies, 0)) throw new Error('Pinned COL26.8 Myriapoda class counts no longer match the living accepted-species total')
  ...
  if (currentRows.length !== ITIS_LIVING_CLASS_BREAKDOWN.reduce((sum, item) => sum + item.currentSpecies, 0)) throw new Error('Pinned ITIS Myriapoda class counts no longer match the audited current-species total')
  ...
  if (new Set(crosswalk.map((record) => record.colUsageId)).size !== colSpecies.length) throw new Error('COL Myriapoda records are not uniquely addressable')
  const upstreamOnly = currentRows.filter((row) => !evidencedTsns.has(String(row.tsn))).map((row) => ({
    ...
    basis: 'No strict COL26.8 Myriapoda accepted-species name or official ITIS species-synonym evidence resolves to this current ITIS TSN.',
  })).sort((left, right) => Number(left.currentName.tsn) - Number(right.currentName.tsn))
  mkdirSync(nomenclatureRoot, { recursive: true })
  for (const name of readdirSync(nomenclatureRoot)) if (/^itis-myriapoda-(?:sidecar|upstream-only)-\d{3,4}\.jsonl\.gz$/u.test(name)) rmSync(join(nomenclatureRoot, name))
  const shards = chunkBySourceBytes(crosswalk).map((records, indexNumber) => {
    const filename = `itis-myriapoda-sidecar-${String(indexNumber).padStart(4, '0')}.jsonl.gz`
  ...
  const upstreamPath = join(nomenclatureRoot, 'itis-myriapoda-upstream-only-0000.jsonl.gz')
  ...
  const descriptor = {
    ...
    exactMatching: {
      ...
      statuses: {
        accepted: 'The normalized COL name resolves to exactly one valid ITIS Myriapoda species and directly equals that current ITIS name.',
        'synonym-current-name-redirect': 'The normalized COL name equals official ITIS invalid species-name evidence that resolves to exactly one valid ITIS Myriapoda species.',
        ambiguous: 'The normalized exact evidence resolves to more than one valid ITIS Myriapoda species TSN.',
        unmatched: 'No normalized exact valid-name or official ITIS species-synonym evidence resolves to a valid ITIS Myriapoda species.',
      },
  ...
  const ledger = {
    schemaVersion: 1, importType: 'COL26.8-to-ITIS-exact-myriapoda-nomenclatural-sidecar',
    ...
    generatedBy: { scriptPath: 'scripts/build-itis-myriapoda-sidecar.mjs', scriptSha256: await sha256File(fileURLToPath(import.meta.url)), deterministic: 'Pinned input checksums, fixed roots, exact SQL, exact representation-only normalization, code-unit ID ordering and deterministic gzip; no wall-clock fields or fuzzy matching.' },
  }
