...
const descriptorPath = join(packRoot, 'itis-nematomorpha-sidecar.json')
const ledgerPath = join(root, 'data', 'sources', 'itis-nematomorpha-sidecar-import-ledger.json')
...
describe('ITIS Nematomorpha exact sidecar shards', () => {
  ...
  it('pins the complete strict COL26.8 Nematomorpha partition', () => {
    expect(descriptor.packageId).toBe('other-animals')
    expect(descriptor.scope.colRootUsageId).toBe('5B')
    expect(descriptor.scope.colRootScientificName).toBe('Nematomorpha')
    expect(descriptor.scope.colStrictAcceptedSpecies).toBe(356)
    expect(descriptor.scope.packageStrictAcceptedSpecies).toBe(99161)
    expect(descriptor.scope.packageOutOfScopeStrictAcceptedSpecies).toBe(98805)
    expect(descriptor.deliveryProfiles['web-light']).toMatchObject({ payload: 'summary-only', records: 0, files: [] })
    expect(descriptor.deliveryProfiles['native-full']).toMatchObject({ payload: 'complete', records: 356 })
    expect(descriptorBytes.length).toBeLessThan(64 * 1024)
    expect(rows).toHaveLength(356)
    expect(new Set(rows.map((record) => record.colUsageId)).size).toBe(356)
  ...
  it('retains exact evidence and explicit unmatched boundaries', () => {
    ...
    expect(upstream).toHaveLength(48)
  ...
  it('checks descriptor, ledger and delivery-boundary hashes', () => {
    expect(descriptor.counts).toMatchObject({ total: 356, accepted: 187, synonymCurrentNameRedirect: 6, ambiguous: 0, unmatched: 163, itisCurrentSpecies: 238, itisSpeciesSynonymLinks: 70, itisUpstreamOnly: 48 })
    ...
    expect(ledger.scopeAudit.itisRoot).toEqual({ tsn: '64183', scientificName: 'Nematomorpha', rank: 'Phylum', usage: 'valid' })
