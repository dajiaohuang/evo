"""Project two small WoRMS ColDP archives into exact COL26.8 sidecars.
...
def project(key: str, output_root: Path = ROOT) -> dict:
    ...
    descriptor = {"schemaVersion": 1, "recordType": "release-pinned-authority-archive-crosswalk",
                  ...
                  "totals": {"records": len(records),
                             ...
                             "upstreamOnlySourceBytes": sum(f["sourceBytes"] for f in source_files)},
                  ...
                  "limitations": ["Upstream-only rows retain null COL ownership.",
    ...
    ledger = {"schemaVersion": 1, "importType": "COL26.8-to-WoRMS-original-archive-projection",
              ...
              "scopeAudit": {"colRootUsageId": spec["root"], "colSpecies": len(col_rows),
                              ...
                              "sourceAcceptedSpecies": len(accepted),
                              "provisionalExcluded": member_counts["provisionalSpecies"],
