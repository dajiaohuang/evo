"""Project the frozen ChecklistBank Kinorhyncha archive into the exact COL scope."""
...
ARCHIVE_SHA='c1bcb2e24081c67db0f2a3c879642ff0d4f10bed75d606111f6c5e44cfd3d470'; ARCHIVE_BYTES=318152
SOURCE='1153'; ROOT_ID='B8VF5'; LIMIT=2*1024*1024
...
def source_name(name,taxon):
 return {'id':taxon['ID'],'taxonID':taxon['ID'],'nameID':name['ID'],'scientificName':name['scientificName'],'authorship':name.get('authorship'), 'rank':name['rank'],'status':'accepted','url':name.get('link') or taxon.get('link') or ''}
...
def read_col():
 ...
 if len(rows)!=362: raise ValueError(f'COL source scope changed: {len(rows)}')
...
def project(archive,metadata,output_root=None):
 ...
 def shards(prefix,data,role):
  ...
  for i,part in enumerate(parts):
   ...
   out.append(q)
  return out
 cf=shards('worms-kinorhyncha',records,'col-partition'); uf=shards('worms-kinorhyncha-source-only',upstream,'upstream-only'); totalbytes=sum(x['bytes'] for x in cf+uf)
 ...
 descriptor={'schemaVersion':1,'recordType':'release-pinned-authority-archive-crosswalk','id':'worms-kinorhyncha-archive-crosswalk','packageId':'other-animals','provider':'World Register of Marine Species via ChecklistBank','rowEncoding':'json','encoding':'gzip','mediaType':'application/json','role':'authority-crosswalk','colIdField':'colId','totalCountField':'total','source':{'datasetId':SOURCE,'title':meta['title'],'version':meta['version'],'versionDoi':meta['versionDoi'],'metadataPath':'data/sources/archives/checklistbank-1153-kinorhyncha-2026-09-01.metadata.json','metadataBytes':len(metadata_bytes),'metadataSha256':digest(metadata_bytes),'license':source_license,'licenseUrl':'https://creativecommons.org/licenses/by/4.0/','archiveUrl':'https://api.checklistbank.org/dataset/1153/archive','archivePath':'data/sources/archives/checklistbank-1153-kinorhyncha-2026-09-01.zip','archiveBytes':len(raw),'archiveSha256':digest(raw),'members':members},'scope':{'colRootUsageId':ROOT_ID,'scientificName':'Kinorhyncha','eligibleColSpecies':len(col),'sourceAcceptedSpecies':len(source),'sourceSpeciesRankTaxa':total,'excludedSourceProvisional':provisional},'matching':{'normalization':'NFC and whitespace normalization only; COL trailing authorship removed exactly.','prohibited':'No fuzzy, case-folded, accent-folded, synonym or species-concept matching.'},'counts':{'total':len(records),'records':len(records)+len(upstream),**counts,'upstreamOnly':len(upstream)},'files':cf,'upstreamOnlyFiles':uf,'totalCompressedBytes':totalbytes,'totalSourceBytes':sum(x['sourceBytes'] for x in cf+uf),'deliveryProfiles':{'web-light':{'mode':'summary-only','records':0,'files':[],'totalCompressedBytes':0,'totalSourceBytes':0},'native-full':{'mode':'complete','records':len(records)+len(upstream),'files':[x['path'] for x in cf+uf],'totalCompressedBytes':totalbytes,'totalSourceBytes':sum(x['sourceBytes'] for x in cf+uf)}},'evidenceBoundary':{'en':'Frozen exact nomenclatural crosswalk for source 1153 Kinorhyncha; not species-concept equivalence, a biological dossier, fossil evidence or expert review.','zh':'冻结的严格命名交叉映射；不是物种概念等同性、生物档案、化石证据或专家审查。'}}
 dpath=dest/'worms-kinorhyncha-sidecar.json'; db=dump(descriptor,True); dpath.write_bytes(db)
 ledger={'schemaVersion':1,'importType':'COL26.8-to-WoRMS-1153-archive-projection','source':descriptor['source'],'registryManifestSha256':colsha,'registryInputs':colinputs,'generatedBy':{'script':'scripts/build-worms-kinorhyncha-source.py','scriptSha256':digest(Path(__file__).read_bytes().replace(b'\r\n',b'\n')),'hashNormalization':'LF'},'outputs':{'descriptor':{'path':'data/catalogue-of-life/releases/2026-08-20/resource-packs/other-animals/worms-kinorhyncha-sidecar.json','bytes':len(db),'sha256':digest(db)},'files':cf,'upstreamOnlyFiles':uf},'scopeAudit':{'colRootUsageId':ROOT_ID,'colSpecies':len(col),'sourceAcceptedSpecies':len(source),'sourceSpeciesRankTaxa':total,'sourceProvisionalExcluded':provisional,'upstreamOnly':len(upstream)}}
 ledgerpath=base/'data/sources/worms-kinorhyncha-archive-1153-import-ledger.json'; ledgerpath.parent.mkdir(parents=True,exist_ok=True); ledgerpath.write_bytes(dump(ledger,True)); print(json.dumps(descriptor['counts']))
def main():
 p=argparse.ArgumentParser(); p.add_argument('--archive',type=Path,default=ROOT/'data/sources/archives/checklistbank-1153-kinorhyncha-2026-09-01.zip'); p.add_argument('--metadata',type=Path,default=ROOT/'data/sources/archives/checklistbank-1153-kinorhyncha-2026-09-01.metadata.json'); p.add_argument('--output-root',type=Path); a=p.parse_args(); project(a.archive,a.metadata,a.output_root)
