...
def project(archive,metadata,output_root=None):
 ...
 descriptor['id']='worms-kinorhyncha-archive-crosswalk'; descriptor['source'].update(datasetId='1153',title=meta['title'],version=meta['version'],versionDoi=meta['versionDoi'],metadataPath='data/sources/archives/checklistbank-1153-kinorhyncha-2026-09-01.metadata.json',archiveUrl='https://api.checklistbank.org/dataset/1153/archive',archivePath='data/sources/archives/checklistbank-1153-kinorhyncha-2026-09-01.zip'); descriptor['scope']['scientificName']='Kinorhyncha'; descriptor['evidenceBoundary']['en']='Frozen exact nomenclatural crosswalk for source 1153 Kinorhyncha; not species-concept equivalence, a biological dossier, fossil evidence or expert review.'
 ...
 ledger['importType']='COL26.8-to-WoRMS-1153-archive-projection'; ledger['generatedBy']['script']='scripts/build-worms-kinorhyncha-source.py'; ledger['outputs']['descriptor']['path']='data/catalogue-of-life/releases/2026-08-20/resource-packs/other-animals/worms-kinorhyncha-sidecar.json'; ledgerpath=base/'data/sources/worms-kinorhyncha-archive-1153-import-ledger.json'; ledgerpath.parent.mkdir(parents=True,exist_ok=True); ledgerpath.write_bytes(dump(ledger,True)); print(json.dumps(descriptor['counts']))
def main():
