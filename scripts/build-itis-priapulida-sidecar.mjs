...
const DESCRIPTOR_PATH = join(OUTPUT_ROOT, 'itis-priapulida-sidecar.json')
const LEDGER_PATH = join(REPOSITORY_ROOT, 'data', 'sources', 'itis-priapulida-sidecar-import-ledger.json')
const COL_ROOT_USAGE_ID = 'B8VF9'
const COL_ROOT_NAME = 'Priapulida Théel, 1906'
const ITIS_ROOT_TSN = 563953
const PACKAGE_ID = 'other-animals'
const SHARD_SOURCE_LIMIT_BYTES = 512 * 1024

const currentSpeciesQuery = `WITH RECURSIVE priapulida_descendants(tsn) AS (
  SELECT ?1
  UNION ALL
  SELECT u.tsn FROM taxonomic_units u
  JOIN priapulida_descendants d ON u.parent_tsn = d.tsn
...
const synonymQuery = `WITH RECURSIVE priapulida_descendants(tsn) AS (
  SELECT ?1
  UNION ALL
  SELECT u.tsn FROM taxonomic_units u
  JOIN priapulida_descendants d ON u.parent_tsn = d.tsn
  WHERE u.name_usage = 'valid'
), accepted_priapulida_species(tsn) AS (
  SELECT u.tsn FROM priapulida_descendants d
  JOIN taxonomic_units u ON u.tsn = d.tsn
  JOIN taxon_unit_types r ON r.kingdom_id = u.kingdom_id AND r.rank_id = u.rank_id
  WHERE lower(trim(r.rank_name)) = 'species' AND u.name_usage = 'valid'
)
...
JOIN accepted_priapulida_species a ON a.tsn = s.tsn_accepted
...
async function loadColPriapulidaSpecies(manifest) {
  const nodes = new Map()
  const species = []
...
function loadItis(sqlitePath) {
  ...
  try {
    ...
    if (!root || root.completename !== 'Priapula' || root.rank_name !== 'Phylum' || root.name_usage !== 'valid') {
      throw new Error('Pinned ITIS Priapulida root no longer has the expected valid phylum identity')
    }
...
async function main() {
  ...
  if (argument < 0 || !process.argv[argument + 1]) throw new Error('Usage: node scripts/build-itis-priapulida-sidecar.mjs --itis-sqlite <verified ITIS.sqlite>')
  ...
  const colSpecies = await loadColPriapulidaSpecies(JSON.parse(registryBytes))
  const packageIds = await loadPackageSpecies(resourcePack)
  if (!colSpecies.every((record) => packageIds.has(record.id))) throw new Error('COL Priapulida species escaped the other-animals ownership boundary')
  const nonApplicable = resourcePack.acceptedSpeciesCount - colSpecies.length
  if (nonApplicable !== 99138) throw new Error(`Unexpected other-animals non-applicable remainder: ${nonApplicable}`)
  ...
  if (itis.false) throw new Error(`Unexpected ITIS Priapulida scope: ${itis.currentRows.length}/${itis.synonymRows.length}`)
  ...
  if (new Set(crosswalk.map((record) => record.colUsageId)).size !== colSpecies.length) throw new Error('COL Priapulida records are not uniquely addressable')
  const upstreamOnly = itis.currentRows.filter((row) => !evidencedTsns.has(String(row.tsn))).map((row) => ({
    ...
    basis: 'No strict COL26.8 Priapulida accepted-species name or official ITIS species-synonym evidence resolves to this current ITIS Priapulida species.',
  })).sort((left, right) => Number(left.currentName.tsn) - Number(right.currentName.tsn))
  for (const name of readdirSync(OUTPUT_ROOT)) if (/^itis-priapulida-(?:sidecar|upstream-only)-\d{3}\.jsonl\.gz$/u.test(name)) rmSync(join(OUTPUT_ROOT, name))
  const shards = chunkBySourceBytes(crosswalk).map((records, indexNumber) => {
    const filename = `itis-priapulida-sidecar-${String(indexNumber).padStart(3, '0')}.jsonl.gz`
  ...
  const upstreamPath = join(OUTPUT_ROOT, 'itis-priapulida-upstream-only-000.jsonl.gz')
  ...
  const descriptor = {
    ...
    scope: {
      ...
      packageOwnership: 'other-animals is the deterministic COL26.8 residual route below Animalia (N); Priapulida B8VF9 has no more-specific static-package route.',
      nonApplicableBoundary: 'All other-animals strict accepted species not descending from B8VF9 are non-applicable to this Priapulida sidecar. They remain in the mixed resource pack and are not silently mapped to ITIS Priapulida.',
      sourceComposition,
    },
    sources: {
      col: {
    ...
    exactMatching: {
      ...
      statuses: {
        accepted: 'The normalized COL name resolves to exactly one valid ITIS Priapulida species and directly equals that current ITIS name.',
        'synonym-current-name-redirect': 'The normalized COL name equals official ITIS invalid species-name evidence that resolves to exactly one valid ITIS Priapulida species.',
        ambiguous: 'The normalized exact evidence resolves to more than one valid ITIS Priapulida species TSN.',
        unmatched: 'No normalized exact valid-name or official ITIS species-synonym evidence resolves to a valid ITIS Priapulida species.',
      },
    ...
    evidenceBoundary: {
      en: 'This CC0 ITIS sidecar is a frozen exact nomenclatural crosswalk for the COL26.8 Priapulida partition. It is not a final classification authority, phylogeny, species-concept equivalence assertion, biological dossier, fossil record or scientific-review record. The other-animals resource pack is mixed; its non-Priapulida remainder is explicitly out of scope.',
      zh: '此 CC0 ITIS 侧车是 COL26.8 鳃曳动物门分区的冻结严格命名交叉映射；它不是最终分类权威、系统发育树、物种概念等同性声明、生物档案、化石记录或科学审查记录。other-animals 是混合资源包，其余非鳃曳动物物种明确排除在本侧车之外。',
    },
  ...
  descriptor.evidenceBoundary.zh = '此 CC0 ITIS 侧车是 COL26.8 鳃曳动物门分区的冻结严格命名交叉映射；它不是最终分类权威、系统发育树、物种概念等同性声明、生物档案、化石记录或科学审查记录。other-animals 是混合资源包，其余非鳃曳动物物种明确排除在本侧车之外。'
  descriptor.deliveryProfiles = {
    'web-light': { payload: 'summary-only', records: 0, files: [], statement: 'GitHub Pages carries the descriptor and hashes without row-level Priapulida payload shards.' },
    'native-full': { payload: 'complete', records: colSpecies.length, files: [...shards, upstreamDescriptor], statement: 'Android and iOS carry every checksum-addressed Priapulida row shard, including the explicit empty upstream-only partition.' },
  }
  const descriptorBytes = jsonBytes(descriptor)
  writeFileSync(DESCRIPTOR_PATH, descriptorBytes)
  const ledger = {
    schemaVersion: 1,
    importType: 'COL26.8-to-ITIS-exact-priapulida-nomenclatural-sidecar',
