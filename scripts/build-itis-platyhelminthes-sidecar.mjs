...
const DESCRIPTOR_PATH = join(OUTPUT_ROOT, 'itis-platyhelminthes-sidecar.json')
const LEDGER_PATH = join(REPOSITORY_ROOT, 'data', 'sources', 'itis-platyhelminthes-sidecar-import-ledger.json')
const COL_ROOT_USAGE_ID = '7NF2H'
const COL_ROOT_NAME = 'Platyhelminthes Minot, 1876'
const ITIS_ROOT_TSN = 53963
const PACKAGE_ID = 'other-animals'
const SHARD_SOURCE_LIMIT_BYTES = 512 * 1024

const currentSpeciesQuery = `WITH RECURSIVE platyhelminthes_descendants(tsn) AS (
  SELECT ?1
  UNION ALL
  SELECT u.tsn FROM taxonomic_units u
  JOIN platyhelminthes_descendants d ON u.parent_tsn = d.tsn
...
FROM platyhelminthes_descendants d
...
const synonymQuery = `WITH RECURSIVE platyhelminthes_descendants(tsn) AS (
  SELECT ?1
  UNION ALL
  SELECT u.tsn FROM taxonomic_units u
  JOIN platyhelminthes_descendants d ON u.parent_tsn = d.tsn
  WHERE u.name_usage = 'valid'
), accepted_platyhelminthes_species(tsn) AS (
  SELECT u.tsn FROM platyhelminthes_descendants d
  JOIN taxonomic_units u ON u.tsn = d.tsn
  JOIN taxon_unit_types r ON r.kingdom_id = u.kingdom_id AND r.rank_id = u.rank_id
  WHERE lower(trim(r.rank_name)) = 'species' AND u.name_usage = 'valid'
)
...
JOIN accepted_platyhelminthes_species a ON a.tsn = s.tsn_accepted
...
async function loadColPlatyhelminthesSpecies(manifest) {
  const nodes = new Map()
  const species = []
...
function loadItis(sqlitePath) {
  ...
  try {
    ...
    if (!root || root.completename !== 'Platyhelminthes' || root.rank_name !== 'Phylum' || root.name_usage !== 'valid') {
      throw new Error('Pinned ITIS Platyhelminthes root no longer has the expected valid phylum identity')
    }
...
async function main() {
  ...
  if (argument < 0 || !process.argv[argument + 1]) throw new Error('Usage: node scripts/build-itis-platyhelminthes-sidecar.mjs --itis-sqlite <verified ITIS.sqlite>')
  ...
  const colSpecies = await loadColPlatyhelminthesSpecies(JSON.parse(registryBytes))
  const packageIds = await loadPackageSpecies(resourcePack)
  if (!colSpecies.every((record) => packageIds.has(record.id))) throw new Error('COL Platyhelminthes species escaped the other-animals ownership boundary')
  const nonApplicable = resourcePack.acceptedSpeciesCount - colSpecies.length
  if (nonApplicable !== 72154) throw new Error(`Unexpected other-animals non-applicable remainder: ${nonApplicable}`)
  ...
  if (itis.currentRows.length !== 8732 || itis.synonymRows.length !== 3566) throw new Error(`Unexpected ITIS Platyhelminthes scope: ${itis.currentRows.length}/${itis.synonymRows.length}`)
  ...
  if (new Set(crosswalk.map((record) => record.colUsageId)).size !== colSpecies.length) throw new Error('COL Platyhelminthes records are not uniquely addressable')
  const upstreamOnly = itis.currentRows.filter((row) => !evidencedTsns.has(String(row.tsn))).map((row) => ({
    ...
    basis: 'No strict COL26.8 Platyhelminthes accepted-species name or official ITIS species-synonym evidence resolves to this current ITIS Platyhelminthes species.',
  })).sort((left, right) => Number(left.currentName.tsn) - Number(right.currentName.tsn))
  for (const name of readdirSync(OUTPUT_ROOT)) if (/^itis-platyhelminthes-(?:sidecar|upstream-only)-\d{3}\.jsonl\.gz$/u.test(name)) rmSync(join(OUTPUT_ROOT, name))
  const shards = chunkBySourceBytes(crosswalk).map((records, indexNumber) => {
    const filename = `itis-platyhelminthes-sidecar-${String(indexNumber).padStart(3, '0')}.jsonl.gz`
  ...
  const upstreamPath = join(OUTPUT_ROOT, 'itis-platyhelminthes-upstream-only-000.jsonl.gz')
  ...
  const descriptor = {
    ...
    scope: {
      ...
      packageOwnership: 'other-animals is the deterministic COL26.8 residual route below Animalia (N); Platyhelminthes 7NF2H has no more-specific static-package route.',
      nonApplicableBoundary: 'All other-animals strict accepted species not descending from 7NF2H are non-applicable to this Platyhelminthes sidecar. They remain in the mixed resource pack and are not silently mapped to ITIS Platyhelminthes.',
      sourceComposition,
    },
    sources: {
      col: {
    ...
    exactMatching: {
      ...
      statuses: {
        accepted: 'The normalized COL name resolves to exactly one valid ITIS Platyhelminthes species and directly equals that current ITIS name.',
        'synonym-current-name-redirect': 'The normalized COL name equals official ITIS invalid species-name evidence that resolves to exactly one valid ITIS Platyhelminthes species.',
        ambiguous: 'The normalized exact evidence resolves to more than one valid ITIS Platyhelminthes species TSN.',
        unmatched: 'No normalized exact valid-name or official ITIS species-synonym evidence resolves to a valid ITIS Platyhelminthes species.',
      },
    ...
    evidenceBoundary: {
      en: 'This CC0 ITIS sidecar is a frozen exact nomenclatural crosswalk for the COL26.8 Platyhelminthes partition. It is not a final classification authority, phylogeny, species-concept equivalence assertion, biological dossier, fossil record or scientific-review record. The other-animals resource pack is mixed; its non-Platyhelminthes remainder is explicitly out of scope.',
      zh: '此 CC0 ITIS 侧车是 COL26.8 扁形动物分区的冻结严格命名交叉映射；它不是最终分类权威、系统发育树、物种概念等同性声明、生物档案、化石记录或科学审查记录。other-animals 资源包是混合包，其余非扁形动物物种被明确排除在本侧车之外。',
    },
  ...
  const ledger = {
    schemaVersion: 1,
    importType: 'COL26.8-to-ITIS-exact-platyhelminthes-nomenclatural-sidecar',
