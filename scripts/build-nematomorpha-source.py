...
def read_archive(path):
 ...
 return [dict(r,_ordinal=i+2,scientificName=name(r)) for i,r in enumerate(rows)],members,{'references':0,'nameReferences':0}
def read_col():
...
def main():
 ...
 source,members,reference_counts=read_archive(a.archive);col,colsha,inputs=read_col(); by={(norm(r['scientificName']),norm(r.get('AuthorString'))):r for r in source}; rec=[];used=set();counts={x:0 for x in ('accepted','redirect','ambiguous','unmatched','withheld')}
 ...
 # Keep the archive's empty bibliography explicit in the ledger.
 ledger['scopeAudit']['referenceRows']=reference_counts['references']; ledger['scopeAudit']['nameReferenceRows']=reference_counts['nameReferences']; lp.write_bytes(dump(ledger,True))
