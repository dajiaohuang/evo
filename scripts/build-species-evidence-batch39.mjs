...
const INPUT = 'data/sources/species-evidence-batch39-2026-09-25.json'
const OUTPUT = 'data/knowledge/species-evidence-batch39-2026-09-25.batch-manifest.json'
const NEW_RAW = 'data/knowledge/raw-dossiers/species-evidence-batch39-2026-09-25.jsonl'
const NEW_SHARD = 'data/knowledge/catalogue-dossiers-species-evidence-batch39-2026-09-25.jsonl.br'
...
const EXPECTED_INPUT_SHA256 = '0d2a5b5ed7a15f1e9772312475e6dc086668203c4a885518af5a96f4881ae7f3'
...
assert.equal(input.batchId, 'species-evidence-batch39-2026-09-25')
assert.equal(input.releaseAlias, 'COL26.8')
assert.equal(input.baseAudit.indexedRecordCount, 6966)
assert.equal(input.baseAudit.indexedShardCount, 62)
  ...
  if (!alreadyApplied) {
    ...
    for (const update of input.updates.filter(item => item.targetShardPath === targetShard)) {
      ...
      const supplement = {
        ...
        generator: 'scripts/build-species-evidence-batch39.mjs',
