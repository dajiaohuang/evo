...
const SOURCE = 'data/sources/primates-saguinus-bicolor-batch28-2026-09-25.json'
const RAW = 'data/knowledge/raw-dossiers/primates-saguinus-bicolor-batch28-2026-09-25.jsonl'
const SHARD = 'data/knowledge/catalogue-dossiers-primates-saguinus-bicolor-batch28-2026-09-25.jsonl.br'
const MANIFEST = 'data/knowledge/catalogue-dossiers-primates-saguinus-bicolor-batch28-2026-09-25.batch-manifest.json'
...
const EXPECTED_SOURCE_SHA256 = 'f483743da1f9df4fa8fd603aa3d9d5c8ca56da31907e5bd3b1f94d13496ee820'
const EXPECTED_REGISTRY_SHA256 = '8bee38bd7b937bb0040d5d2aeade08c02ab2b0044314ffe2641ba482a8a7a151'
const EXPECTED_BASE_HEAD = '3f8b4c78a82ee08cdd86130c2e6d2fb0b642e8fc'
...
assert.equal(source.batchId, 'primates-saguinus-bicolor-batch28-2026-09-25')
assert.equal(source.releaseAlias, 'COL26.8')
assert.equal(source.duplicateAudit.baseHead, EXPECTED_BASE_HEAD)
assert.equal(source.duplicateAudit.checkedIndexRecords, 6936)
...
for (const dossier of records) {
  assert.equal(dossier.colId, '4TZBJ')
  assert.equal(dossier.scientificName, 'Saguinus bicolor (Spix, 1823)')
...
const existingTargetRecord = existingRowsById.get('4TZBJ')
...
if (targetAlreadyIndexed) {
  ...
  assert.equal(existingTargetRecord?.colId, records[0].colId, 'Indexed Saguinus usage is missing from its expected shard')
...
const manifest = {
  ...
  generator: 'scripts/build-primates-saguinus-bicolor-batch28.mjs',
}
