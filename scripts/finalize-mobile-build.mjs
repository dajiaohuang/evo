...
const coreDefinition = JSON.parse(readFileSync(join(rootDir, 'data/pages-preview.json'), 'utf8'))
if (current.edition !== 'native-core' || current.deliveryProfile !== 'web-light'
  || current.previewScope?.catalogue !== 'omitted'
  || JSON.stringify(current.previewScope?.packageIds) !== JSON.stringify(coreDefinition.packageIds)
  || JSON.stringify(current.previewScope?.taxonIds) !== JSON.stringify(coreDefinition.taxonIds)
  || JSON.stringify(current.previewScope?.storyIds) !== JSON.stringify(coreDefinition.storyIds)
  || JSON.stringify(current.previewScope?.eventIds) !== JSON.stringify(coreDefinition.eventIds)) {
  throw new Error('Mobile build must use the versioned native-core content scope')
}
...
if (maps.paleotopography?.delivery?.profile !== 'web-preview'
  || maps.paleotopography?.delivery?.resolutionDegrees !== 0.3
  || maps.paleotopography?.delivery?.fullResolutionAvailableInNativeApps !== false
  || maps.paleotopography?.frames?.length !== 109) {
  throw new Error('Mobile build must retain only the selected 0.3-degree core map data')
}
...
if (files.some((file) => file.startsWith(`${outputRoot}${sep}sql${sep}`)
  || file.endsWith(`${sep}native-runtime-manifest.json`))) {
  throw new Error('Mobile core bundle must not include the offline SQL research runtime')
}
const totalBytes = files.reduce((sum, file) => sum + statSync(file).size, 0)
// Keep full scientific releases, registry rows, authority archives and query
// engines out of the app. The shell and selected core release have a 384 MiB cap.
const limitMiB = 384
...
console.log(`Mobile core-only contract passed: ${interactiveFiles.length} files, ${(bundledBytes / 1024 / 1024).toFixed(2)} MiB core release; app resources ${(totalBytes / 1024 / 1024).toFixed(2)} MiB; dataset ${current.datasetVersion}`)
