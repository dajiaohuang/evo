...
const descriptorPath = join(packRoot, 'itis-loricifera-sidecar.json')
const ledgerPath = join(root, 'data', 'sources', 'itis-loricifera-sidecar-import-ledger.json')
...
describe('ITIS Loricifera exact sidecar shards', () => {
  ...
  it('pins the complete strict COL26.8 Loricifera partition', () => {
    expect(descriptor.packageId).toBe('other-animals')
    expect(descriptor.scope.colRootUsageId).toBe('B8VF6')
    expect(descriptor.scope.colRootScientificName).toBe('Loricifera Kristensen, 1983')
    expect(descriptor.scope.colStrictAcceptedSpecies).toBe(46)
    expect(descriptor.scope.packageStrictAcceptedSpecies).toBe(99161)
    expect(descriptor.scope.packageOutOfScopeStrictAcceptedSpecies).toBe(99115)
    expect(descriptor.deliveryProfiles['web-light']).toMatchObject({ payload: 'summary-only', records: 0, files: [] })
    expect(descriptor.deliveryProfiles['native-full']).toMatchObject({ payload: 'complete', records: 46 })
    expect(descriptorBytes.length).toBeLessThan(64 * 1024)
    expect(rows).toHaveLength(46)
    expect(new Set(rows.map((record) => record.colUsageId)).size).toBe(46)
  ...
  it('checks descriptor, ledger and delivery-boundary hashes', () => {
    expect(descriptor.counts).toMatchObject({ total: 46, accepted: 22, synonymCurrentNameRedirect: 0, ambiguous: 0, unmatched: 24, itisCurrentSpecies: 22, itisSpeciesSynonymLinks: 0, itisUpstreamOnly: 0 })
    ...
    expect(ledger.scopeAudit.itisRoot).toEqual({ tsn: '202425', scientificName: 'Loricifera', rank: 'Phylum', usage: 'valid' })
