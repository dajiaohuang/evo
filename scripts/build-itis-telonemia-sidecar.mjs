...
const descriptorPath = join(outputRoot, 'itis-telonemia-sidecar.json')
const ledgerPath = join(root, 'data', 'sources', 'itis-telonemia-sidecar-import-ledger.json')
const packageId = 'protists-chromists'
const packageRoots = ['C', 'Z']
const targetName = 'Telonemia'
const nearRootNames = ['Telonema', 'Telonemida', 'Telonemidae']
const overlapScopes = ['Bigyra', 'Oomycota', 'Ochrophyta', 'Cercozoa', 'Ciliophora', 'Dinoflagellata', 'Apicomplexa', 'Amoebozoa', 'Euglenozoa', 'Haptophyta', 'Rhodophyta', 'Cryptophyta', 'Choanoflagellatea', 'Glaucophyta', 'Perkinsozoa', 'Labyrinthulomycetes', 'Opalozoa', 'Telonemia']
...
function parseArgs(argv) {
  ...
  if (!options.help && !options.itisSqlitePath) throw new Error('Usage: node scripts/build-itis-telonemia-sidecar.mjs --itis-sqlite <verified ITIS.sqlite>')
  return options
}
...
function loadItis(sqlitePath) {
  ...
  try {
    ...
    if (exactNameCandidates.length) throw new Error(`Pinned ITIS export now contains an exact ${targetName} root; reassess before regenerating: ${JSON.stringify(exactNameCandidates)}`)
    const placeholders = nearRootNames.map(() => '?').join(', ')
    const nearRootCandidates = database.prepare(`SELECT u.tsn, l.completename AS scientific_name, r.rank_name, u.name_usage, u.parent_tsn
      FROM taxonomic_units u JOIN longnames l ON l.tsn = u.tsn
      JOIN taxon_unit_types r ON r.kingdom_id = u.kingdom_id AND r.rank_id = u.rank_id
      WHERE lower(trim(l.completename)) IN (${placeholders}) ORDER BY u.tsn`).all(...nearRootNames)
...
async function main() {
  ...
  if (options.help) { console.log('Usage: node scripts/build-itis-telonemia-sidecar.mjs --itis-sqlite <verified ITIS.sqlite>'); return }
  ...
  for (const name of readdirSync(outputRoot)) if (/^itis-telonemia-(?:sidecar|upstream-only)-\d{4}\.jsonl\.gz$/u.test(name)) rmSync(join(outputRoot, name))
  const upstreamPath = join(outputRoot, 'itis-telonemia-upstream-only-0000.jsonl.gz')
  ...
  const scope = {
    ...
    boundary: `This boundary audit contains no ${targetName} rows because neither COL26.8 nor ITIS 2026-08-26 exposes an exact accepted root named ${targetName}. Nearby names are recorded only as audit evidence and are not substituted.`,
  }
  const rootBoundaryAudit = {
    ...
    overlapAudit: { inspectedScopes: overlapScopes, overlappingColUsageIds: [], overlappingItisTsns: [] },
    decision: `No exact ${targetName} root exists in either pinned authority. Nearby names are not taxonomic substitutes; no COL or ITIS range is asserted.`,
  }
  const exactMatching = { normalization: source.importLedger.normalization, statuses: { accepted: `Unavailable: no exact ${targetName} root exists in the pinned authorities.`, 'synonym-current-name-redirect': `Unavailable: no exact ${targetName} root exists in the pinned authorities.`, ambiguous: `Unavailable: no exact ${targetName} root exists in the pinned authorities.`, unmatched: `Unavailable: no exact ${targetName} partition exists.` }, prohibited: 'No fuzzy, edit-distance, phonetic, case-folded, diacritic-stripped, token-reordered, package-wide or taxon-substituted matching is used.' }
  const descriptor = {
    ...
    exactMatching, evidenceBoundary: { en: `This CC0 ITIS boundary audit is a frozen exact-root check for the declared COL26.8 Protists and Chromists package. It is not a global ${targetName} checklist, a final classification authority, a species-concept equivalence assertion, a biological dossier or a scientific-review record.`, zh: `此 CC0 ITIS 边界审计仅冻结声明的 COL26.8 原生生物与色界生物包精确根检查；它不是全球 ${targetName} 名录、最终分类权威、物种概念等同性声明、生物档案或科学审查记录。` },
    counts, colUsageIdLocator: { key: 'colUsageId', ordering: 'Unicode code-unit ascending', stableAddressing: `No COL row-level files exist because COL26.8 does not materialize an exact ${targetName} root.`, files: [] },
  ...
  const ledger = {
    schemaVersion: 1, importType: 'COL26.8-to-ITIS-exact-telonemia-root-boundary-audit',
    ...
    generatedBy: { scriptPath: repoPath(scriptPath), scriptSha256: await sha256File(scriptPath), deterministic: 'Pinned input checksums, exact COL and ITIS root audits, explicit nearby-root conflict record, and deterministic gzip; no wall-clock fields or fuzzy matching.' },
  }
