"""Project the pinned WoRMS Gastrotricha ColDP archive into a COL-scoped sidecar."""
...
ARCHIVE = ROOT / 'data/sources/archives/checklistbank-1122-gastrotricha-2026-09-01.zip'
METADATA = ROOT / 'data/sources/archives/checklistbank-1122-gastrotricha-2026-09-01.metadata.json'
ARCHIVE_SHA = '3828913cfe33e8a5184ad6ab86f9d824a897fa7f6ad85967365c292e33079653'
ARCHIVE_BYTES = 138207
COL_SOURCE = '1122'
COL_ROOTS = ('B8V3M',)
...
def project(archive, output_root=None):
    ...
    def write_shards(prefix, rows, role):
        ...
        for index, part in enumerate(parts):
            ...
            items.append(item)
        return items
    col_files = write_shards('worms-gastrotricha', records, 'col-partition')
    source_files = write_shards('worms-gastrotricha-source-only', upstream, 'upstream-only')
    descriptor = {'schemaVersion': 1, 'recordType': 'release-pinned-authority-archive-crosswalk',
                  'id': 'worms-gastrotricha-archive-crosswalk', 'packageId': 'other-animals',
                  'provider': 'World Register of Marine Species via ChecklistBank', 'rowEncoding': 'json',
                  'colIdField': 'colId', 'totalCountField': 'total',
                  'source': {'datasetId': '1122', 'title': metadata['title'], 'version': metadata['version'],
                             ...
                             'rightsHolder': 'WoRMS Editorial Board', 'archiveUrl': 'https://api.checklistbank.org/dataset/1122/archive',
                             'archivePath': 'data/sources/archives/checklistbank-1122-gastrotricha-2026-09-01.zip',
                             'metadataPath': 'data/sources/archives/checklistbank-1122-gastrotricha-2026-09-01.metadata.json',
                             'archiveBytes': len(raw), 'archiveSha256': digest(raw), 'members': members},
                  'scope': {'colRootUsageIds': list(COL_ROOTS), 'scientificName': 'Gastrotricha',
                  ...
                  'evidenceBoundary': {'en': 'Frozen WoRMS nomenclatural/source projection for the exact COL26.8 source-1122 Gastrotricha root; not species-concept equivalence, a biological dossier, fossil evidence or expert review.',
                                       'zh': '精确 COL26.8 source-1122 Gastrotricha 根节点范围的 WoRMS 冻结命名/来源投影；不是物种概念等同性、生物档案、化石证据或专家审查。'},
                  'limitations': ['Source-only rows are relative only to COL source dataset 1122.', 'Name.status is nomenclatural metadata and is not used as taxonomic acceptance.', 'Synonym.taxonID targets do not remove accepted Taxon rows.'],
    ...
    ledger_path = output_base / 'data/sources/worms-gastrotricha-archive-1122-import-ledger.json'
