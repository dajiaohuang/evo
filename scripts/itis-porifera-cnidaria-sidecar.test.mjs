...
const nomenclatureRoot = join(root, 'data/packages/invertebrata/sponges-cnidarians/nomenclature')
const descriptorPath = join(nomenclatureRoot, 'itis-porifera-cnidaria-sidecar.json')
const ledgerPath = join(root, 'data/sources/itis-porifera-cnidaria-sidecar-import-ledger.json')
...
describe('ITIS Porifera/Cnidaria exact sidecar shards', () => {
  ...
  it('preserves both exact COL roots without overlap and every accepted species once', () => {
    expect(descriptor.packageId).toBe('sponges-cnidarians')
    expect(descriptor.scope.colRootUsageIds).toEqual(['B8TXQ', 'CN2'])
    expect(descriptor.scope.colRootScientificNames).toEqual(['Porifera', 'Cnidaria'])
    expect(descriptor.scope.colStrictAcceptedSpecies).toBe(30521)
    expect(descriptor.scope.packageStrictAcceptedSpecies).toBe(30521)
    expect(descriptor.scope.packageOutOfScopeStrictAcceptedSpecies).toBe(0)
    expect(descriptor.scope.colStrictAcceptedSpeciesByRoot.Porifera).toBeGreaterThan(0)
    expect(descriptor.scope.colStrictAcceptedSpeciesByRoot.Cnidaria).toBeGreaterThan(0)
    expect(descriptorBytes.length).toBeLessThan(64 * 1024)
    expect(rows).toHaveLength(descriptor.counts.total)
    expect(rows).toHaveLength(descriptor.scope.colStrictAcceptedSpecies)
    ...
    expect(rows.filter((row) => row.colRootUsageId === 'B8TXQ')).toHaveLength(descriptor.scope.colStrictAcceptedSpeciesByRoot.Porifera)
    expect(rows.filter((row) => row.colRootUsageId === 'CN2')).toHaveLength(descriptor.scope.colStrictAcceptedSpeciesByRoot.Cnidaria)
  })

  it('uses only exact official ITIS evidence and resolves each detail through one shard', () => {
    expect(descriptor.sources.itis.rootTsns).toEqual({ Porifera: '46861', Cnidaria: '48738' })
  ...
  it('keeps ITIS-only species separate and records the Pages/native delivery boundary', () => {
    ...
    expect(ledger.outputs.colUsageIdShards).toEqual(files)
    expect(ledger.outputs.upstreamOnly).toEqual(descriptor.upstreamOnly.files[0])
    ...
    expect(ledger.deliveryContract.runtimeChange).toContain('no runtime')
  })
})
