...
const descriptorPath = join(packRoot, 'itis-cycliophora-sidecar.json')
const ledgerPath = join(root, 'data/sources/itis-cycliophora-sidecar-import-ledger.json')
const ITIS_ROOT_TSN = 563958
const COL_ROOT_USAGE_ID = '622CL'
...
function loadItis(sqlitePath) {
  ...
  try {
    ...
    if (!rootRecord || rootRecord.completename !== 'Cycliophora' || rootRecord.rank_name !== 'Phylum' || rootRecord.name_usage !== 'valid') throw new Error('Pinned ITIS root is not the expected valid Cycliophora phylum')
...
async function main() {
  ...
  if (argument < 0 || !process.argv[argument + 1]) throw new Error('Usage: node scripts/build-itis-cycliophora-sidecar.mjs --itis-sqlite <verified ITIS.sqlite>')
  ...
  if (colSpecies.length > packageCount) throw new Error('Cycliophora COL scope exceeds Other Animals ownership')
  ...
  crosswalk.sort((a, b) => compareCodeUnits(a.colUsageId, b.colUsageId)); if (new Set(crosswalk.map((record) => record.colUsageId)).size !== colSpecies.length) throw new Error('COL Cycliophora records are not uniquely addressable')
  const upstreamOnly = currentRows.filter((row) => !evidencedTsns.has(String(row.tsn))).map((row) => ({ colUsageId: null, currentName: currentName(row), basis: 'No strict COL26.8 Cycliophora accepted-species name or official ITIS species-synonym evidence resolves to this current ITIS TSN.' })).sort((a, b) => Number(a.currentName.tsn) - Number(b.currentName.tsn))
  mkdirSync(packRoot, { recursive: true }); for (const name of readdirSync(packRoot)) if (/^itis-cycliophora-(?:sidecar|upstream-only)-\d{4}\.jsonl\.gz$/u.test(name)) rmSync(join(packRoot, name))
  const shards = chunks(crosswalk).map((records, indexNumber) => { const path = join(packRoot, `itis-cycliophora-sidecar-${String(indexNumber).padStart(4, '0')}.jsonl.gz`); const sourcePayload = jsonlBytes(records); const bytes = Buffer.from(deterministicGzip(sourcePayload, { level: 9 })); writeFileSync(path, bytes); return shardDescriptor(path, records, bytes, sourcePayload) })
  const upstreamSource = jsonlBytes(upstreamOnly); const upstreamBytes = Buffer.from(deterministicGzip(upstreamSource, { level: 9 })); const upstreamPath = join(packRoot, 'itis-cycliophora-upstream-only-0000.jsonl.gz'); writeFileSync(upstreamPath, upstreamBytes)
  ...
  const descriptor = { schemaVersion: 1, sidecarType: 'release-pinned-exact-nomenclatural-crosswalk', packageId: PACKAGE_ID,
    scope: { packageRootUsageId: 'N', packageRootScientificName: 'Animalia', colRootUsageId: COL_ROOT_USAGE_ID, colRootScientificName: 'Cycliophora', colStrictAcceptedSpecies: colSpecies.length, packageStrictAcceptedSpecies: packageCount, packageOutOfScopeStrictAcceptedSpecies: packageCount - colSpecies.length, nonApplicableRemainder: 'All other strict accepted COL26.8 species assigned to Other Animals remain outside this Cycliophora sidecar.', boundary: 'This sidecar covers only strict accepted COL26.8 species descending from exact Cycliophora root 622CL; all remaining mixed-pack species are explicitly out of scope.' },
    ...
    exactMatching: { normalization: source.importLedger.normalization, statuses: { accepted: 'The normalized COL name resolves to exactly one valid ITIS Cycliophora species and directly equals that current ITIS name.', 'synonym-current-name-redirect': 'The normalized COL name equals official ITIS invalid species-name evidence that resolves to exactly one valid ITIS Cycliophora species.', ambiguous: 'The normalized exact evidence resolves to more than one valid ITIS Cycliophora species TSN.', unmatched: 'No normalized exact valid-name or official ITIS species-synonym evidence resolves to a valid ITIS Cycliophora species.' }, prohibited: 'No fuzzy, edit-distance, phonetic, case-folded, diacritic-stripped, token-reordered or taxon-substituted matching is used.' },
    evidenceBoundary: { en: 'This CC0 ITIS sidecar is a frozen exact nomenclatural crosswalk for the declared Cycliophora partition. It is not a global cycliophoran checklist, final classification authority, phylogeny, species-concept equivalence assertion, biological dossier or scientific-review record.', zh: '此 CC0 ITIS 侧车是已声明 Cycliophora 分区的冻结严格命名交叉映射；它不是全球环口动物名录、最终分类权威、系统发育、物种概念等同性声明、生物档案或科学审查记录。' },
