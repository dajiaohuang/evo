...
const sourcePath = resolve(root, 'data/sources/primates-trachypithecus-francoisi-batch20-2026-09-24.json')
...
const manifestPath = resolve(root, 'data/knowledge/catalogue-dossiers-primates-trachypithecus-francoisi-batch20-2026-09-24.batch-manifest.json')
...
assert.equal(source.batchId, 'primates-trachypithecus-francoisi-batch20-2026-09-24')
assert.equal(source.duplicateAudit.baseHead, 'b0aed43289798ab41ba46b627a866af224b46959')
...
for (const [key, value] of Object.entries({
  colId: '57SDB', scientificName: 'Trachypithecus francoisi (de Pousargues, 1898)',
  authorship: '(de Pousargues, 1898)', rank: 'species', sourceDatasetId: '2144',
...
assert.equal(usages.length, 1, 'Expected one exact accepted COL26.8 usage in the search projection')
for (const [key, value] of Object.entries({
  scientificName: dossier.scientificName, authorship: dossier.authorship,
  rank: 'species', status: 'accepted', sourceDatasetId: dossier.sourceDatasetId,
...
const targetMatches = (registry.acceptedTargets.routes[targetPrefix] ?? []).flatMap(jsonl).filter(item => item.id === dossier.colId)
assert.equal(targetMatches.length, 0, 'Expected this hierarchy/search accepted species to remain outside the acceptedTargets subset')
...
assert.ok(dossier.classificationPath.some(node => node.id === '7YDX' && node.rank === 'genus' && node.scientificName === 'Trachypithecus Reichenbach, 1862'))
...
for (const assessment of Object.values(dossier.facets)) {
  ...
  for (const claim of assessment.claims ?? []) {
    ...
    for (const id of claim.sourceIds) {
      const sourceRecord = dossier.sources.find(item => item.id === id)
      assert.equal(sourceRecord.licenseAssessment, 'item-level-verified', `Claim source license is not item-level verified: ${id}`)
      assert.ok(sourceRecord.rightsEvidenceUrl && sourceRecord.rightsEvidenceLocator)
      assert.ok(sourceRecord.stableId && sourceRecord.publishedAt && sourceRecord.accessedAt)
      assert.ok(sourceRecord.rightsHolder && sourceRecord.licenseVersion && sourceRecord.licenseUrl && sourceRecord.licenseAppliesTo && sourceRecord.attribution)
    }
  }
}
...
assert.equal(dossier.facets.distribution.status, 'conflicted')
assert.ok(dossier.facets.distribution.claims.some(claim => claim.text.includes('Laos') && claim.text.includes('does not directly verify')))
...
const manifest = {
  ...
  generator: 'scripts/build-primates-trachypithecus-francoisi-batch20.mjs',
}
