...
const descriptorPath = join(nomenclatureRoot, 'itis-placozoa-sidecar.json')
...
describe('ITIS Placozoa exact sidecar shards', () => {
  ...
  it('covers the exact COL26.8 Placozoa partition once in deterministic, non-overlapping ranges', () => {
    expect(descriptor.packageId).toBe('other-animals')
    expect(descriptor.scope.colRootUsageId).toBe('B8V3N')
    expect(descriptor.scope.colStrictAcceptedSpecies).toBe(4)
  ...
  it('retains only exact ITIS evidence and addresses each COL species through one shard', () => {
    expect(descriptor.sources.itis.rootTsn).toBe('563955')
