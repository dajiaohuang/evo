...
const ITIS_ROOT_TSN = 82697
const COL_ROOT_USAGE_ID = 'KZWYC'
const EXCLUDED_COL_ROOT_USAGE_ID = 'TRL'
const PACKAGE_ID = 'trilobites-chelicerates'
const SHARD_SOURCE_LIMIT_BYTES = 2 * 1024 * 1024

const currentSpeciesQuery = `WITH RECURSIVE chelicerata_descendants(tsn) AS (
  SELECT ?1
  UNION ALL
  SELECT u.tsn FROM taxonomic_units u JOIN chelicerata_descendants d ON u.parent_tsn = d.tsn
...
FROM chelicerata_descendants d
...
const synonymQuery = `WITH RECURSIVE chelicerata_descendants(tsn) AS (
  SELECT ?1
  UNION ALL
  SELECT u.tsn FROM taxonomic_units u JOIN chelicerata_descendants d ON u.parent_tsn = d.tsn
  WHERE u.name_usage = 'valid'
), accepted_chelicerata_species(tsn) AS (
  SELECT u.tsn FROM chelicerata_descendants d
  JOIN taxonomic_units u ON u.tsn = d.tsn
  JOIN taxon_unit_types r ON r.kingdom_id = u.kingdom_id AND r.rank_id = u.rank_id
  WHERE lower(trim(r.rank_name)) = 'species' AND u.name_usage = 'valid'
)
...
JOIN accepted_chelicerata_species a ON a.tsn = s.tsn_accepted
...
async function loadColRootSpecies(manifest, rootUsageId) {
...
function loadItis(sqlitePath) {
  ...
  try {
    ...
    if (!rootRecord || rootRecord.completename !== 'Chelicerata' || rootRecord.rank_name !== 'Subphylum' || rootRecord.name_usage !== 'valid') {
      throw new Error('Pinned ITIS Chelicerata root no longer has the expected valid subphylum identity')
    }
...
async function main() {
  ...
  if (argument < 0 || !process.argv[argument + 1]) throw new Error('Usage: node scripts/build-itis-chelicerata-sidecar.mjs --itis-sqlite <verified ITIS.sqlite>')
  ...
  const [colSpecies, excludedTrilobitaSpecies] = await Promise.all([
    loadColRootSpecies(JSON.parse(registryBytes), COL_ROOT_USAGE_ID),
    loadColRootSpecies(JSON.parse(registryBytes), EXCLUDED_COL_ROOT_USAGE_ID),
  ])
  if (!Number.isInteger(packageCount) || colSpecies.length + excludedTrilobitaSpecies.length !== packageCount) {
    throw new Error(`COL/package scope audit is inconsistent: Chelicerata=${colSpecies.length}, Trilobita=${excludedTrilobitaSpecies.length}, package=${packageCount}`)
  }
  ...
  if (new Set(crosswalk.map((record) => record.colUsageId)).size !== colSpecies.length) throw new Error('COL Chelicerata records are not uniquely addressable')
  const upstreamOnly = currentRows.filter((row) => !evidencedTsns.has(String(row.tsn))).map((row) => ({
    ...
    basis: 'No strict COL26.8 Chelicerata accepted-species name or official ITIS species-synonym evidence resolves to this current ITIS TSN.',
  })).sort((left, right) => Number(left.currentName.tsn) - Number(right.currentName.tsn))
  mkdirSync(nomenclatureRoot, { recursive: true })
  for (const name of readdirSync(nomenclatureRoot)) if (/^itis-chelicerata-(?:sidecar|upstream-only)-\d{3,4}\.jsonl\.gz$/u.test(name)) rmSync(join(nomenclatureRoot, name))
  const shards = chunkBySourceBytes(crosswalk).map((records, indexNumber) => {
    const filename = `itis-chelicerata-sidecar-${String(indexNumber).padStart(4, '0')}.jsonl.gz`
  ...
  const upstreamPath = join(nomenclatureRoot, 'itis-chelicerata-upstream-only-0000.jsonl.gz')
  ...
  const descriptor = {
    ...
    scope: {
      colRootUsageId: COL_ROOT_USAGE_ID, colRootScientificName: 'Chelicerata', colStrictAcceptedSpecies: colSpecies.length,
      excludedColRootUsageId: EXCLUDED_COL_ROOT_USAGE_ID, excludedColRootScientificName: 'Trilobita', excludedStrictAcceptedSpecies: excludedTrilobitaSpecies.length,
      packageStrictAcceptedSpecies: packageCount, packageOutOfScopeStrictAcceptedSpecies: packageCount - colSpecies.length,
      boundary: 'This mixed package contains extinct Trilobita and living Chelicerata. This sidecar covers only strict accepted COL26.8 species descending from exact Chelicerata root KZWYC; all Trilobita species are explicitly non-applicable because current ITIS has no Trilobita authority branch.',
    },
    sources: {
    ...
    exactMatching: {
      ...
      statuses: {
        accepted: 'The normalized COL name resolves to exactly one valid ITIS Chelicerata species and directly equals that current ITIS name.',
        'synonym-current-name-redirect': 'The normalized COL name equals official ITIS invalid species-name evidence that resolves to exactly one valid ITIS Chelicerata species.',
        ambiguous: 'The normalized exact evidence resolves to more than one valid ITIS Chelicerata species TSN.',
        unmatched: 'No normalized exact valid-name or official ITIS species-synonym evidence resolves to a valid ITIS Chelicerata species.',
      },
    ...
    evidenceBoundary: {
      en: 'This CC0 ITIS sidecar is a frozen exact nomenclatural crosswalk, not a global chelicerate checklist, final classification authority, phylogeny, species-concept equivalence assertion, biological dossier or scientific-review record.',
      zh: '此 CC0 ITIS 侧车是冻结的严格命名交叉映射；它不是全球螯肢动物名录、最终分类权威、系统发育树、物种概念等同性声明、生物档案或科学审查记录。',
    },
  ...
  const ledger = {
    schemaVersion: 1, importType: 'COL26.8-to-ITIS-exact-chelicerata-nomenclatural-sidecar',
    ...
    generatedBy: { scriptPath: 'scripts/build-itis-chelicerata-sidecar.mjs', scriptSha256: await sha256File(fileURLToPath(import.meta.url)), deterministic: 'Pinned input checksums, fixed roots, exact SQL, exact representation-only normalization, code-unit ID ordering and deterministic gzip; no wall-clock fields or fuzzy matching.' },
  }
