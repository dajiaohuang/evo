...
const INPUT = 'data/sources/species-evidence-batch35-2026-09-25.json'
const OUTPUT = 'data/knowledge/species-evidence-batch35-2026-09-25.batch-manifest.json'
const NEW_RAW = 'data/knowledge/raw-dossiers/species-evidence-batch35-2026-09-25.jsonl'
const NEW_SHARD = 'data/knowledge/catalogue-dossiers-species-evidence-batch35-2026-09-25.jsonl.br'
...
const EXPECTED_INPUT_SHA256 = '83240ec198114c26ac5e268f20a8fabbefafa33e3ce759f5aa4e42acfde9d5a6'
...
assert.equal(input.batchId, 'species-evidence-batch35-2026-09-25')
assert.equal(input.releaseAlias, 'COL26.8')
assert.equal(input.baseAudit.indexedRecordCount, 6954)
assert.equal(input.baseAudit.indexedShardCount, 58)
  ...
  if (!alreadyApplied) {
    ...
    for (const update of input.updates.filter(item => item.targetShardPath === targetShard)) {
      ...
      const supplement = {
        ...
        generator: 'scripts/build-species-evidence-batch35.mjs',
