...
describe('ITIS Phoronida exact sidecar shards', () => {
  ...
  it('pins every strict COL26.8 Phoronida species exactly once in non-overlapping code-unit ranges', () => {
    expect(descriptor.packageId).toBe('other-animals')
    expect(descriptor.scope.colRootUsageId).toBe('5P')
    expect(descriptor.counts.total).toBe(19)
    expect(descriptorBytes.length).toBeLessThan(64 * 1024)
    expect(rows).toHaveLength(19)
    expect(new Set(rows.map((record) => record.colUsageId)).size).toBe(19)
  ...
  it('checks descriptor, shard inventory and delivery boundary bytes', () => {
    ...
    expect(ledger.scopeAudit.colStrictAcceptedSpecies).toBe(19)
    expect(ledger.scopeAudit.itisRoot.tsn).toBe('155456')
