...
describe('ITIS Insecta exact sidecar shards', () => {
  ...
  it('limits the mixed package to the exact Insecta root and preserves every in-scope COL species once', () => {
    expect(descriptor.packageId).toBe('crustaceans-insects')
    expect(descriptor.scope.colRootUsageId).toBe('H6')
  ...
  it('uses exact official ITIS evidence only and resolves each detail through one shard', () => {
    expect(descriptor.sources.itis.rootTsn).toBe('99208')
