...
const descriptorPath = join(nomenclatureRoot, 'itis-entoprocta-sidecar.json')
const ledgerPath = join(root, 'data/sources/itis-entoprocta-sidecar-import-ledger.json')
const ITIS_ROOT_TSN = 156732
const COL_ROOT_USAGE_ID = 'BDTX4'
...
async function loadPackSpecies(manifest) {
  const species = []
  for (const file of manifest.files) await eachGzipJsonLine(join(resourcePackRoot, ...file.path.split('/').slice(1)), (record) => species.push(record))
  if (species.length !== manifest.acceptedSpeciesCount || new Set(species.map((record) => record.id)).size !== species.length) throw new Error('Other Animals resource pack count or IDs changed')
  return species
}
...
function loadItis(sqlitePath) {
  ...
  try {
    ...
    if (!rootRecord || rootRecord.completename !== 'Entoprocta' || rootRecord.rank_name !== 'Phylum' || rootRecord.name_usage !== 'valid') throw new Error('Pinned ITIS root is not the expected valid Entoprocta phylum')
    ...
    const kamptozoaAliases = database.prepare(`SELECT s.tsn AS tsn, l.completename AS scientificName, u.name_usage AS usage, s.tsn_accepted AS acceptedTsn
      FROM synonym_links s JOIN taxonomic_units u ON u.tsn = s.tsn JOIN longnames l ON l.tsn = s.tsn
      WHERE s.tsn = 914161 ORDER BY s.tsn_accepted`).all()
    if (JSON.stringify(kamptozoaAliases.map((row) => [row.tsn, row.scientificName, row.usage, row.acceptedTsn])) !== JSON.stringify([[914161, 'Kamptozoa', 'invalid', 156732], [914161, 'Kamptozoa', 'invalid', 563958]])) {
      throw new Error('Pinned ITIS Kamptozoa alias boundary changed; do not treat the synonym name as an additive Entoprocta root')
    }
    return { rootRecord, kamptozoaAliases, maxima, currentRows: database.prepare(currentSpeciesQuery).all(ITIS_ROOT_TSN), synonymRows: database.prepare(synonymQuery).all(ITIS_ROOT_TSN) }
...
function chunkBySourceBytes(records) {
  ...
  for (const record of records) {
    ...
    if (size > SHARD_SOURCE_LIMIT_BYTES) throw new Error(`COL ${record.colUsageId} exceeds the shard size limit`)
    if (current.length && bytes + size > SHARD_SOURCE_LIMIT_BYTES) { chunks.push(current); current = []; bytes = 0 }
    current.push(record); bytes += size
  }
  if (current.length) chunks.push(current)
  return chunks
}
function outputDescriptor(path, records, bytes, sourceBytes) { return { path: repoPath(path), records: records.length, firstColUsageId: records[0]?.colUsageId ?? null, lastColUsageId: records.at(-1)?.colUsageId ?? null, bytes: bytes.length, sha256: sha256(bytes), sourceBytes: sourceBytes.length, sourceSha256: sha256(sourceBytes) } }
...
function sourceDatasetCounts(records) { return Object.fromEntries(Object.entries(records.reduce((counts, record) => { const key = record.sourceDatasetId ?? 'null'; counts[key] = (counts[key] ?? 0) + 1; return counts }, {})).sort(([left], [right]) => Number(left) - Number(right))) }

async function main() {
  const argument = process.argv.indexOf('--itis-sqlite')
  if (argument < 0 || !process.argv[argument + 1]) throw new Error('Usage: node scripts/build-itis-entoprocta-sidecar.mjs --itis-sqlite <verified ITIS.sqlite>')
  ...
  const registryManifestPath = join(registryRoot, 'manifest.json'); const registryBytes = readFileSync(registryManifestPath); const ownershipBytes = readFileSync(ownershipPath); const resourcePackManifestPath = join(resourcePackRoot, 'manifest.json'); const resourcePackBytes = readFileSync(resourcePackManifestPath)
  ...
  const ownership = JSON.parse(ownershipBytes); const resourcePack = JSON.parse(resourcePackBytes); const colSpecies = await loadColSpecies(JSON.parse(registryBytes)); const packSpecies = await loadPackSpecies(resourcePack); const packIds = new Set(packSpecies.map((record) => record.id))
  if (resourcePack.packageId !== PACKAGE_ID || resourcePack.acceptedSpeciesCount !== ownership.packageCounts[PACKAGE_ID] || colSpecies.some((record) => !packIds.has(record.id))) throw new Error('Entoprocta and Other Animals ownership boundaries changed')
  const { rootRecord, kamptozoaAliases, maxima, currentRows, synonymRows } = loadItis(sqlitePath)
  ...
  crosswalk.sort((a, b) => compareCodeUnits(a.colUsageId, b.colUsageId)); if (new Set(crosswalk.map((record) => record.colUsageId)).size !== colSpecies.length) throw new Error('COL Entoprocta records are not uniquely addressable')
  const upstreamOnly = currentRows.filter((row) => !evidencedTsns.has(String(row.tsn))).map((row) => ({ colUsageId: null, currentName: currentName(row), basis: 'No strict COL26.8 Entoprocta accepted-species name or official ITIS species-synonym evidence resolves to this current ITIS TSN.' })).sort((a, b) => Number(a.currentName.tsn) - Number(b.currentName.tsn))
  mkdirSync(nomenclatureRoot, { recursive: true }); for (const name of readdirSync(nomenclatureRoot)) if (/^itis-entoprocta-(?:sidecar|upstream-only)-\d{4}\.jsonl\.gz$/u.test(name)) rmSync(join(nomenclatureRoot, name))
  const shards = chunkBySourceBytes(crosswalk).map((records, number) => { const path = join(nomenclatureRoot, `itis-entoprocta-sidecar-${String(number).padStart(4, '0')}.jsonl.gz`); const sourcePayload = jsonlBytes(records); const compressed = Buffer.from(deterministicGzip(sourcePayload, { level: 9 })); writeFileSync(path, compressed); return outputDescriptor(path, records, compressed, sourcePayload) })
  const upstreamPath = join(nomenclatureRoot, 'itis-entoprocta-upstream-only-0000.jsonl.gz'); const upstreamSource = jsonlBytes(upstreamOnly); const upstreamCompressed = Buffer.from(deterministicGzip(upstreamSource, { level: 9 })); writeFileSync(upstreamPath, upstreamCompressed)
  ...
  const counts = { total: crosswalk.length, accepted: crosswalk.filter((record) => record.status === 'accepted').length, synonymCurrentNameRedirect: crosswalk.filter((record) => record.status === 'synonym-current-name-redirect').length, ambiguous: crosswalk.filter((record) => record.status === 'ambiguous').length, unmatched: crosswalk.filter((record) => record.status === 'unmatched').length, itisCurrentSpecies: currentRows.length, itisSpeciesSynonymLinks: synonymRows.length, itisUpstreamOnly: upstreamOnly.length }
  const descriptor = { schemaVersion: 1, sidecarType: 'release-pinned-exact-nomenclatural-crosswalk', packageId: PACKAGE_ID,
    scope: { colRootUsageId: COL_ROOT_USAGE_ID, colRootScientificName: 'Entoprocta', colStrictAcceptedSpecies: colSpecies.length, packageStrictAcceptedSpecies: resourcePack.acceptedSpeciesCount, packageOutOfScopeStrictAcceptedSpecies: resourcePack.acceptedSpeciesCount - colSpecies.length, boundary: 'other-animals is the deterministic Animalia remainder after more-specific static-package routes. This sidecar covers only strict accepted COL26.8 species descending from exact Entoprocta root BDTX4; every other other-animals-owned species is explicitly outside this sidecar.', itisSynonymBoundary: { excludedName: 'Kamptozoa', tsn: '914161', usage: 'invalid', reason: 'The same invalid ITIS synonym TSN has multiple accepted targets in the pinned export, so it is audit evidence only and is never added as a second Entoprocta root.', observedAcceptedTsns: kamptozoaAliases.map((row) => String(row.acceptedTsn)) } },
    mixedResourcePack: { packageId: PACKAGE_ID, manifestPath: repoPath(resourcePackManifestPath), manifestSha256: sha256(resourcePackBytes), acceptedSpeciesCount: packSpecies.length, inScopeSpecies: colSpecies.length, outOfScopeSpecies: packSpecies.length - colSpecies.length, packageSourceDatasetCounts: sourceDatasetCounts(packSpecies), entoproctaSourceDatasetCounts: sourceDatasetCounts(colSpecies), outOfScopeBoundary: 'All remaining accepted species in Other Animals are not Entoprocta and receive no ITIS Entoprocta match in this sidecar.' },
    ...
    exactMatching: { normalization: source.importLedger.normalization, statuses: { accepted: 'The normalized COL name resolves to exactly one valid ITIS Entoprocta species and directly equals that current ITIS name.', 'synonym-current-name-redirect': 'The normalized COL name equals official ITIS invalid species-name evidence that resolves to exactly one valid ITIS Entoprocta species.', ambiguous: 'The normalized exact evidence resolves to more than one valid ITIS Entoprocta species TSN.', unmatched: 'No normalized exact valid-name or official ITIS species-synonym evidence resolves to a valid ITIS Entoprocta species.' }, prohibited: 'No fuzzy, edit-distance, phonetic, case-folded, diacritic-stripped, token-reordered or taxon-substituted matching is used.' },
    evidenceBoundary: { en: 'This CC0 ITIS sidecar is a frozen exact nomenclatural crosswalk for the declared Entoprocta partition inside the mixed Other Animals resource pack. ITIS Kamptozoa is an invalid synonym with multiple accepted targets in this export and is deliberately not an additive root. This is not a global entoproctan checklist, final classification authority, phylogeny, species-concept equivalence assertion, biological dossier or scientific-review record.', zh: '此 CC0 ITIS 侧车是混合“其他动物”资源包中已声明 Entoprocta 分区的冻结严格命名交叉映射。ITIS 中的 Kamptozoa 是在此导出中具有多个接受目标的无效同义名，故明确不作为可相加的根。它不是全球内肛动物名录、最终分类权威、系统发育、物种概念等同性声明、生物档案或科学审查记录。' },
