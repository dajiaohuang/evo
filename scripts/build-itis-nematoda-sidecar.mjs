...
const descriptorPath = join(nomenclatureRoot, 'itis-nematoda-sidecar.json')
const ledgerPath = join(root, 'data/sources/itis-nematoda-sidecar-import-ledger.json')
const ITIS_ROOT_TSN = 59490
const COL_ROOT_USAGE_ID = 'NM'
const PACKAGE_ID = 'other-animals'
const SHARD_SOURCE_LIMIT_BYTES = 2 * 1024 * 1024

const currentSpeciesQuery = `WITH RECURSIVE nematoda_descendants(tsn) AS (
  SELECT ?1
  UNION ALL
  SELECT u.tsn FROM taxonomic_units u JOIN nematoda_descendants d ON u.parent_tsn = d.tsn
...
const synonymQuery = `WITH RECURSIVE nematoda_descendants(tsn) AS (
  SELECT ?1
  UNION ALL
  SELECT u.tsn FROM taxonomic_units u JOIN nematoda_descendants d ON u.parent_tsn = d.tsn
  WHERE u.name_usage = 'valid'
), accepted_nematoda_species(tsn) AS (
  SELECT u.tsn FROM nematoda_descendants d
  JOIN taxonomic_units u ON u.tsn = d.tsn
  JOIN taxon_unit_types r ON r.kingdom_id = u.kingdom_id AND r.rank_id = u.rank_id
  WHERE lower(trim(r.rank_name)) = 'species' AND u.name_usage = 'valid'
)
...
JOIN accepted_nematoda_species a ON a.tsn = s.tsn_accepted
...
async function loadColNematodaSpecies(manifest) {
...
function loadItis(sqlitePath) {
  ...
  try {
    ...
    if (!rootRecord || rootRecord.completename !== 'Nematoda' || rootRecord.rank_name !== 'Phylum' || rootRecord.name_usage !== 'valid') {
      throw new Error('Pinned ITIS Nematoda root no longer has the expected valid phylum identity')
    }
...
async function main() {
  ...
  if (argument < 0 || !process.argv[argument + 1]) throw new Error('Usage: node scripts/build-itis-nematoda-sidecar.mjs --itis-sqlite <verified ITIS.sqlite>')
  ...
  const colSpecies = await loadColNematodaSpecies(JSON.parse(registryBytes))
  ...
  if (new Set(crosswalk.map((record) => record.colUsageId)).size !== colSpecies.length) throw new Error('COL Nematoda records are not uniquely addressable')
  const upstreamOnly = currentRows.filter((row) => !evidencedTsns.has(String(row.tsn))).map((row) => ({
    ...
    basis: 'No strict COL26.8 Nematoda accepted-species name or official ITIS species-synonym evidence resolves to this current ITIS TSN.',
  })).sort((left, right) => Number(left.currentName.tsn) - Number(right.currentName.tsn))
  mkdirSync(nomenclatureRoot, { recursive: true })
  for (const name of readdirSync(nomenclatureRoot)) if (/^itis-nematoda-(?:sidecar|upstream-only)-\d{3,4}\.jsonl\.gz$/u.test(name)) rmSync(join(nomenclatureRoot, name))
  const shards = chunkBySourceBytes(crosswalk).map((records, indexNumber) => {
    const filename = `itis-nematoda-sidecar-${String(indexNumber).padStart(4, '0')}.jsonl.gz`
  ...
  const upstreamPath = join(nomenclatureRoot, 'itis-nematoda-upstream-only-0000.jsonl.gz')
  ...
  const descriptor = {
    ...
    scope: {
      colRootUsageId: COL_ROOT_USAGE_ID, colRootScientificName: 'Nematoda', colStrictAcceptedSpecies: colSpecies.length,
      packageStrictAcceptedSpecies: packageCount, packageOutOfScopeStrictAcceptedSpecies: packageCount - colSpecies.length,
      boundary: 'other-animals is the deterministic Animalia remainder after every more-specific static-package route. This sidecar covers only strict accepted COL26.8 species descending from the exact Nematoda root NM; every other other-animals-owned species is explicitly non-applicable.',
    },
    sources: {
    ...
    exactMatching: {
      ...
      statuses: {
        accepted: 'The normalized COL name resolves to exactly one valid ITIS Nematoda species and directly equals that current ITIS name.',
        'synonym-current-name-redirect': 'The normalized COL name equals official ITIS invalid species-name evidence that resolves to exactly one valid ITIS Nematoda species.',
        ambiguous: 'The normalized exact evidence resolves to more than one valid ITIS Nematoda species TSN.',
        unmatched: 'No normalized exact valid-name or official ITIS species-synonym evidence resolves to a valid ITIS Nematoda species.',
      },
    ...
    evidenceBoundary: {
      en: 'This CC0 ITIS sidecar is a frozen exact nomenclatural crosswalk, not a global nematode checklist, final classification authority, phylogeny, species-concept equivalence assertion, biological dossier or scientific-review record.',
      zh: '此 CC0 ITIS 侧车是冻结的严格命名交叉映射；它不是全球线虫名录、最终分类权威、系统发育树、物种概念等同性声明、生物档案或科学审查记录。',
    },
  ...
  const ledger = {
    schemaVersion: 1, importType: 'COL26.8-to-ITIS-exact-nematoda-nomenclatural-sidecar',
    ...
    generatedBy: { scriptPath: 'scripts/build-itis-nematoda-sidecar.mjs', scriptSha256: await sha256File(fileURLToPath(import.meta.url)), deterministic: 'Pinned input checksums, fixed roots, exact SQL, exact representation-only normalization, code-unit ID ordering and deterministic gzip; no wall-clock fields or fuzzy matching.' },
  }
