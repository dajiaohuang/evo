...
const descriptorPath = join(nomenclatureRoot, 'itis-rotifera-sidecar.json')
const ledgerPath = join(root, 'data/sources/itis-rotifera-sidecar-import-ledger.json')
const ITIS_ROOT_TSN = 58239
const COL_ROOT_USAGE_ID = '5Y'
const PACKAGE_ID = 'other-animals'
const SHARD_SOURCE_LIMIT_BYTES = 512 * 1024

const currentSpeciesQuery = `WITH RECURSIVE rotifera_descendants(tsn) AS (
  SELECT ?1
  UNION ALL
SELECT u.tsn FROM taxonomic_units u JOIN rotifera_descendants d ON u.parent_tsn = d.tsn
...
const synonymQuery = `WITH RECURSIVE rotifera_descendants(tsn) AS (
  SELECT ?1
  UNION ALL
SELECT u.tsn FROM taxonomic_units u JOIN rotifera_descendants d ON u.parent_tsn = d.tsn
  WHERE u.name_usage = 'valid'
), accepted_rotifera_species(tsn) AS (
SELECT u.tsn FROM rotifera_descendants d
  JOIN taxonomic_units u ON u.tsn = d.tsn
  JOIN taxon_unit_types r ON r.kingdom_id = u.kingdom_id AND r.rank_id = u.rank_id
  WHERE lower(trim(r.rank_name)) = 'species' AND u.name_usage = 'valid'
)
...
JOIN accepted_rotifera_species a ON a.tsn = s.tsn_accepted
...
async function loadColRotiferaSpecies(manifest) {
...
if (!rootRecord || rootRecord.completename !== 'Rotifera' || rootRecord.rank_name !== 'Phylum' || rootRecord.name_usage !== 'valid') {
      throw new Error('Pinned ITIS Rotifera root no longer has the expected valid phylum identity')
    }
...
async function main() {
  ...
  if (argument < 0 || !process.argv[argument + 1]) throw new Error('Usage: node scripts/build-itis-rotifera-sidecar.mjs --itis-sqlite <verified ITIS.sqlite>')
...
const colSpecies = await loadColRotiferaSpecies(JSON.parse(registryBytes))
...
if (new Set(crosswalk.map((record) => record.colUsageId)).size !== colSpecies.length) throw new Error('COL Rotifera records are not uniquely addressable')
  const upstreamOnly = currentRows.filter((row) => !evidencedTsns.has(String(row.tsn))).map((row) => ({
    ...
    basis: 'No strict COL26.8 Rotifera accepted-species name or official ITIS species-synonym evidence resolves to this current ITIS TSN.',
  })).sort((left, right) => Number(left.currentName.tsn) - Number(right.currentName.tsn))
  mkdirSync(nomenclatureRoot, { recursive: true })
for (const name of readdirSync(nomenclatureRoot)) if (/^itis-rotifera-(?:sidecar|upstream-only)-\d{3,4}\.jsonl\.gz$/u.test(name)) rmSync(join(nomenclatureRoot, name))
  const shards = chunkBySourceBytes(crosswalk).map((records, indexNumber) => {
const filename = `itis-rotifera-sidecar-${String(indexNumber).padStart(4, '0')}.jsonl.gz`
...
const upstreamPath = join(nomenclatureRoot, 'itis-rotifera-upstream-only-0000.jsonl.gz')
  ...
  const descriptor = {
    ...
    scope: {
      ...
      colRootUsageId: COL_ROOT_USAGE_ID, colRootScientificName: 'Rotifera', colStrictAcceptedSpecies: colSpecies.length,
      packageStrictAcceptedSpecies: packageCount, packageOutOfScopeStrictAcceptedSpecies: packageCount - colSpecies.length,
      nonApplicableRemainder: 'Every strict accepted COL26.8 species owned by the other-animals remainder route below N but not descending from 5Y; this complete remainder is explicitly excluded from the Rotifera sidecar.',
      boundary: 'This mixed package contains many animal phyla and broader Animalia navigation content. This sidecar covers only the strict accepted COL26.8 species descending from the exact Rotifera root 5Y; all remaining package-owned species are explicitly out of scope.',
    },
    sources: {
    ...
    exactMatching: {
      ...
      statuses: {
accepted: 'The normalized COL name resolves to exactly one valid ITIS Rotifera species and directly equals that current ITIS name.',
'synonym-current-name-redirect': 'The normalized COL name equals official ITIS invalid species-name evidence that resolves to exactly one valid ITIS Rotifera species.',
ambiguous: 'The normalized exact evidence resolves to more than one valid ITIS Rotifera species TSN.',
unmatched: 'No normalized exact valid-name or official ITIS species-synonym evidence resolves to a valid ITIS Rotifera species.',
      },
...
en: 'This CC0 ITIS sidecar is a frozen exact nomenclatural crosswalk, not a global rotifer checklist, final classification authority, phylogeny, species-concept equivalence assertion, biological dossier or scientific-review record.',
  ...
  const ledger = {
schemaVersion: 1, importType: 'COL26.8-to-ITIS-exact-rotifera-nomenclatural-sidecar',
...
generatedBy: { scriptPath: 'scripts/build-itis-rotifera-sidecar.mjs', scriptSha256: await sha256File(fileURLToPath(import.meta.url)), deterministic: 'Pinned input checksums, fixed roots, exact SQL, exact representation-only normalization, code-unit ID ordering and deterministic gzip; no wall-clock fields or fuzzy matching.' },
  }
