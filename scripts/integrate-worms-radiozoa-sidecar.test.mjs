...
import { integrateWormsRadiozoa } from './integrate-worms-radiozoa-sidecar.mjs'

const fixtures = []
afterEach(async () => { await Promise.all(fixtures.splice(0).map((path) => rm(path, { recursive: true, force: true }))) })

describe('Radiozoa resource-pack integration', () => {
  it('preserves unrelated extensions and collection summary across repeated integration', async () => {
    const root = await mkdtemp(join(tmpdir(), 'worms-radiozoa-'))
    fixtures.push(root)
    const pack = join(root, 'data/catalogue-of-life/releases/2026-08-20/resource-packs/protists-chromists')
    await mkdir(pack, { recursive: true })
    const descriptor = {
      schemaVersion: 1, id: 'worms-radiozoa-archive-crosswalk', recordType: 'release-pinned-authority-archive-crosswalk',
      provider: 'World Register of Marine Species via ChecklistBank', packageId: 'protists-chromists', source: { license: 'CC-BY-4.0' },
      counts: { total: 1, accepted: 1, redirect: 0, ambiguous: 0, unmatched: 0, withheld: 0, upstreamOnly: 1 },
      files: [{ path: 'worms-radiozoa-sidecar-0000.json.gz', records: 1, bytes: 3, sourceBytes: 3, sha256: 'a'.repeat(64), sourceSha256: 'a'.repeat(64) }],
      upstreamOnlyFiles: [{ path: 'worms-radiozoa-upstream-only-0000.json.gz', records: 1, bytes: 5, sourceBytes: 5, sha256: 'b'.repeat(64), sourceSha256: 'b'.repeat(64) }],
    }
    ...
    await writeFile(join(pack, 'worms-radiozoa-sidecar.json'), `${JSON.stringify(descriptor)}\n`)
    await writeFile(join(pack, 'manifest.json'), `${JSON.stringify({ packageId: 'protists-chromists', extensions: [unrelated] })}\n`)
    ...
    await writeFile(collectionPath, `${JSON.stringify({ packs: [{ packageId: 'protists-chromists', extensionCount: 1, extensionFileCount: 2 }] })}\n`)

    const first = integrateWormsRadiozoa({ rootDir: root })
    ...
    expect(firstManifest.extensions.map(({ id }) => id)).toEqual(['unrelated-authority', 'worms-radiozoa-archive-crosswalk'])
    ...
    const second = integrateWormsRadiozoa({ rootDir: root })
