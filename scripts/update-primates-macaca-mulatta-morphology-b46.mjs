...
const SOURCE_PATH = join(ROOT, 'data', 'sources', 'primates-macaca-mulatta-morphology-b46-2026-09-26.json')
...
const MANIFEST_PATH = join(ROOT, 'data', 'knowledge', 'primates-macaca-mulatta-morphology-b46-2026-09-26.update-manifest.json')
const REGISTRY_ROOT = join(ROOT, 'data', 'catalogue-of-life', 'releases', '2026-08-20', 'registry')
const EXPECTED_SOURCE_SHA256 = '0015009c90ff8d2f9852209993e8e7fb48ee9a60a8c895dbd98289025887b765'
const FACETS = ['morphology', 'lifeHistory', 'ecology', 'evolution', 'distribution', 'fossil', 'conservation']
const TARGET_FACETS = ['morphology', 'evolution']
...
assert.equal(sha256(sourceBytes), EXPECTED_SOURCE_SHA256, 'Frozen B46 evidence source changed')
const source = JSON.parse(sourceBytes.toString('utf8'))
assert.equal(source.batchId, 'primates-macaca-mulatta-morphology-batch46-2026-09-26')
...
function validateEvidence(dossier) {
  ...
  for (const item of source.sources) {
    ...
    for (const key of ['stableId', 'rightsHolder', 'licenseVersion', 'licenseAppliesTo', 'attribution', 'scope']) {
      assert.ok(item[key], 'B46 source missing ' + key)
    }
...
assert.equal(indexed.count, source.audit.indexedRecordCount, 'B46 dossier index count changed since identity audit')
const targetMatches = [...indexed.recordsById.values()].filter(row => row.colId === source.target.colId)
assert.equal(targetMatches.length, source.audit.targetRecordCount, 'B46 target must occur exactly once')
...
if (alreadyApplied) {
  assert.equal(alreadyApplied.sourceSha256, EXPECTED_SOURCE_SHA256, 'B46 source digest differs from prior application')
  assert.equal(sha256(rawBytesBefore), metadata.rawSha256, 'B46 raw JSONL digest differs after prior application')
  ...
  assert.deepEqual(originalRows.map(row => row.colId).filter(id => id !== source.target.colId).sort(), [...source.audit.siblingColIds].sort(), 'B46 sibling list differs from audited shard')
  const baseline = originalRows.find(row => row.colId === source.target.colId)
  assert.ok(baseline, 'Missing B46 target in audited raw shard')
  assert.equal(sha256(Buffer.from(JSON.stringify(baseline), 'utf8')), source.audit.previousTargetRecordSha256, 'B46 target record differs from audited baseline')
  ...
  assert.ok(!baseline.sources.some(item => item.id === source.sources[0].id), 'B46 source id already occurs on target')
  assert.ok(!baseline.sources.some(item => item.stableId === source.sources[0].stableId), 'B46 stable source ID already occurs on target')

  const allOccurrences = [...indexed.recordsById.values()].flatMap(record => (record.sources ?? []).map(item => ({ colId: record.colId, source: item })))
  assert.equal(allOccurrences.filter(entry => entry.source.stableId === source.sources[0].stableId).length, source.audit.sourceOccurrenceCountBeforeUpdate, 'B46 DOI occurrence count changed')
  assert.equal(allOccurrences.filter(entry => entry.source.id === source.sources[0].id).length, 0, 'B46 source id is already indexed')
  ...
  dossier.identity.scope = 'COL26.8 usage 3WWNQ only. Existing evidence concerns wild Chinese genomic samples, one free-ranging troop in Buxa Tiger Reserve, India, and high-ranking males in a provisioned, semi-free-ranging Cayo Santiago cohort observed in 2013. New morphology and selection claims concern a separate managed free-ranging Cayo Santiago capture-release cohort measured in 2015. These distinct study scopes do not support species-wide generalization.'
  dossier.lifeStatusScope.wild = 'Wild Chinese population-genomic evidence and ecology evidence from one free-ranging Buxa Tiger Reserve troop remain site-specific. Existing Cayo Santiago life-history evidence concerns provisioned, semi-free-ranging high-ranking males in the 2013 mating season. New morphology and selection evidence concerns a separate managed free-ranging cohort measured in 2015; neither Cayo sample represents native-range or all-population variation.'
  dossier.completeness = {
    status: 'incomplete',
    reasons: [
      'The dossier contains site-specific population-genomic, ecology, high-ranking-male life-history, and new Cayo Santiago morphology and selection evidence; each finding is limited to its sample, measured traits, and study place.',
      'Fossil evidence, morphological variation across the species range, conservation status, and systematic coverage of the accepted species concept remain incomplete or unassessed.',
  ...
  const rawLines = originalLines.map(line => {
    ...
    assert.deepEqual(indexed.recordsById.get(row.colId), row, 'B46 sibling changed: ' + row.colId)
    return line
  })
  const rawBytes = Buffer.from(rawLines.join('\n') + '\n', 'utf8')
  assert.ok(!rawBytes.includes(0x0d), 'B46 raw JSONL must use LF line endings')
  ...
  assert.deepEqual(roundTrip, rawBytes, 'B46 Brotli round trip must preserve exact raw bytes')
  ...
  const updateAudit = {
    ...
    mode: 'in-place-add-cayo-santiago-morphology-and-selection-evidence-to-existing-record',
  ...
  const supplementalUpdate = {
    ...
    generator: relative(join(ROOT, 'scripts', 'update-primates-macaca-mulatta-morphology-b46.mjs')),
    baseHead: source.audit.baseHead,
    indexedRecordCountAtAudit: indexed.count,
  ...
  assert.ok(!metadata.supplementalUpdates.some(item => item.batchId === source.batchId), 'B46 metadata audit already exists')
