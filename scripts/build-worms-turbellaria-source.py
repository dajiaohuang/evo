...
ARCHIVE = ROOT / 'data/sources/archives/checklistbank-1193-turbellaria-2026-09-01.zip'
METADATA = ROOT / 'data/sources/archives/checklistbank-1193-turbellaria-2026-09-01.metadata.json'
...
def project(archive, output_root=None):
    ...
    def write_shards(prefix, rows, role):
        ...
        for index, part in enumerate(parts):
            ...
            items.append(item)
        return items
    col_files = write_shards('worms-turbellaria', records, 'col-partition')
    source_files = write_shards('worms-turbellaria-source-only', upstream, 'upstream-only')
    descriptor = {'schemaVersion': 1, 'recordType': 'release-pinned-authority-archive-crosswalk',
                  ...
                  'evidenceBoundary': {'en': 'Frozen WoRMS nomenclatural/source projection for the exact COL26.8 source-1193 Turbellaria roots; not species-concept equivalence, a biological dossier, fossil evidence or expert review.',
                                       'zh': '精确 COL26.8 source-1193 涡虫类根节点范围的 WoRMS 冻结命名/来源投影；不是物种概念等同性、生物档案、化石证据或专家审查。'},
    ...
    ledger_path = output_base / 'data/sources/worms-turbellaria-archive-1193-import-ledger.json'
