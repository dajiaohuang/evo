...
const INPUT = 'data/sources/species-evidence-batch37-2026-09-25.json'
const OUTPUT = 'data/knowledge/species-evidence-batch37-2026-09-25.batch-manifest.json'
const NEW_RAW = 'data/knowledge/raw-dossiers/species-evidence-batch37-2026-09-25.jsonl'
const NEW_SHARD = 'data/knowledge/catalogue-dossiers-species-evidence-batch37-2026-09-25.jsonl.br'
...
const EXPECTED_INPUT_SHA256 = 'b151fbe349685f173bcdfa905b89848b24b3f613ffed2d6b3291ba2a5343e0da'
...
assert.equal(input.batchId, 'species-evidence-batch37-2026-09-25')
assert.equal(input.releaseAlias, 'COL26.8')
assert.equal(input.baseAudit.indexedRecordCount, 6960)
assert.equal(input.baseAudit.indexedShardCount, 60)
  ...
  if (!alreadyApplied) {
    ...
    for (const update of input.updates.filter(item => item.targetShardPath === targetShard)) {
      ...
      const supplement = {
        ...
        generator: 'scripts/build-species-evidence-batch37.mjs',
