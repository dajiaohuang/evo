...
import { createItisNameIndex, matchColSpecies } from './itis-dinoflagellata-sidecar-lib.mjs'
...
const descriptorPath = join(packRoot, 'itis-dinoflagellata-sidecar.json')
const ledgerPath = join(root, 'data/sources/itis-dinoflagellata-sidecar-import-ledger.json')
const ITIS_ROOT_TSN = 9874
const COL_ROOT_USAGE_ID = '622D3'
...
const synonymQuery = `WITH RECURSIVE descendants(tsn) AS (
  ...
  SELECT u.tsn FROM descendants d JOIN taxonomic_units u ON u.tsn = d.tsn JOIN taxon_unit_types r ON r.kingdom_id = u.kingdom_id AND r.rank_id = u.rank_id WHERE lower(trim(r.rank_name)) = 'species' AND u.name_usage = 'accepted'
...
function shardDescriptor(path, records, bytes, sourceBytes) { return { path: repoPath(path), records: records.length, firstColUsageId: records[0]?.colUsageId ?? null, lastColUsageId: records.at(-1)?.colUsageId ?? null, bytes: bytes.length, sha256: sha256(bytes), sourceBytes: sourceBytes.length, sourceSha256: sha256(sourceBytes) } }
...
function loadItis(sqlitePath) {
  ...
  try {
    ...
    if (!rootRecord || rootRecord.completename !== 'Dinophyceae' || rootRecord.rank_name !== 'Class' || rootRecord.name_usage !== 'accepted') throw new Error('Pinned ITIS root is not the expected accepted Dinophyceae class')
...
async function main() {
  ...
  if (argument < 0 || !process.argv[argument + 1]) throw new Error('Usage: node scripts/build-itis-dinoflagellata-sidecar.mjs --itis-sqlite <verified ITIS.sqlite>')
  ...
  if (colSpecies.length > packageCount) throw new Error('Dinophyceae COL scope exceeds Protists and Chromists ownership')
  ...
  crosswalk.sort((a, b) => compareCodeUnits(a.colUsageId, b.colUsageId)); if (new Set(crosswalk.map((record) => record.colUsageId)).size !== colSpecies.length) throw new Error('COL Dinophyceae records are not uniquely addressable')
  const upstreamOnly = currentRows.filter((row) => !evidencedTsns.has(String(row.tsn))).map((row) => ({ colUsageId: null, currentName: currentName(row), basis: 'No strict COL26.8 Dinophyceae accepted-species name or official ITIS species-synonym evidence resolves to this current ITIS TSN.' })).sort((a, b) => Number(a.currentName.tsn) - Number(b.currentName.tsn))
  mkdirSync(packRoot, { recursive: true }); for (const name of readdirSync(packRoot)) if (/^itis-dinoflagellata-(?:sidecar|upstream-only)-\d{4}\.jsonl\.gz$/u.test(name)) rmSync(join(packRoot, name))
  const shards = chunks(crosswalk).map((records, indexNumber) => { const path = join(packRoot, `itis-dinoflagellata-sidecar-${String(indexNumber).padStart(4, '0')}.jsonl.gz`); const sourcePayload = jsonlBytes(records); const bytes = Buffer.from(deterministicGzip(sourcePayload, { level: 9 })); writeFileSync(path, bytes); return shardDescriptor(path, records, bytes, sourcePayload) })
  const upstreamSource = jsonlBytes(upstreamOnly); const upstreamBytes = Buffer.from(deterministicGzip(upstreamSource, { level: 9 })); const upstreamPath = join(packRoot, 'itis-dinoflagellata-upstream-only-0000.jsonl.gz'); writeFileSync(upstreamPath, upstreamBytes)
  ...
  const descriptor = { schemaVersion: 1, sidecarType: 'release-pinned-exact-nomenclatural-crosswalk', packageId: PACKAGE_ID,
    scope: { packageRootUsageId: 'C', packageRootScientificName: 'Chromista', colRootUsageId: COL_ROOT_USAGE_ID, colRootScientificName: 'Dinophyceae (Dinoflagellata)', colStrictAcceptedSpecies: colSpecies.length, packageStrictAcceptedSpecies: packageCount, packageOutOfScopeStrictAcceptedSpecies: packageCount - colSpecies.length, nonApplicableRemainder: 'All other strict accepted COL26.8 species assigned to Protists and Chromists remain outside this Dinophyceae sidecar.', boundary: 'This sidecar covers only strict accepted COL26.8 species descending from exact Dinophyceae root 622D3; all remaining mixed-pack species are explicitly out of scope.' },
    ...
    exactMatching: { normalization: source.importLedger.normalization, statuses: { accepted: 'The normalized COL name resolves to exactly one valid ITIS Dinophyceae species and directly equals that current ITIS name.', 'synonym-current-name-redirect': 'The normalized COL name equals official ITIS invalid species-name evidence that resolves to exactly one valid ITIS Dinophyceae species.', ambiguous: 'The normalized exact evidence resolves to more than one valid ITIS Dinophyceae species TSN.', unmatched: 'No normalized exact valid-name or official ITIS species-synonym evidence resolves to a valid ITIS Dinophyceae species.' }, prohibited: 'No fuzzy, edit-distance, phonetic, case-folded, diacritic-stripped, token-reordered or taxon-substituted matching is used.' },
    evidenceBoundary: { en: 'This CC0 ITIS sidecar is a frozen exact nomenclatural crosswalk for the declared Dinophyceae (Dinoflagellata) partition. It is not a global dinoflagellate checklist, final classification authority, phylogeny, species-concept equivalence assertion, biological dossier or scientific-review record.', zh: '此 CC0 ITIS 侧车是已声明 Dinophyceae（Dinoflagellata）分区的冻结严格命名交叉映射；它不是全球甲藻名录、最终分类权威、系统发育、物种概念等同性声明、生物档案或科学审查记录。' },
