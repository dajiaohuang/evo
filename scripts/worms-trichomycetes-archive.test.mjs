...
test('1033 projection is the exact 96-row Ichthyosporea boundary',()=>{
 const d=JSON.parse(fs.readFileSync(base+'worms-trichomycetes-sidecar.json'));
 assert.equal(d.id,'worms-trichomycetes-archive-crosswalk'); assert.equal(d.scope.sourceClass,'Ichthyosporea');
 assert.deepEqual(d.counts,{total:96,accepted:96,redirect:0,ambiguous:0,unmatched:0,withheld:0,upstreamOnly:0,records:96});
 const rows=JSON.parse(zlib.gunzipSync(fs.readFileSync(base+'worms-trichomycetes-000.json.gz')));
