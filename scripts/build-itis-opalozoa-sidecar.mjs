...
const descriptorPath = join(outputRoot, 'itis-opalozoa-sidecar.json')
const ledgerPath = join(root, 'data', 'sources', 'itis-opalozoa-sidecar-import-ledger.json')
const packageId = 'protists-chromists'
const packageRoots = ['C', 'Z']
const targetName = 'Opalozoa'
const nearRootNames = ['Opalinata']
...
async function eachGzipJsonLine(path, visit) {
  const lines = createInterface({ input: createReadStream(path).pipe((await import('node:zlib')).createGunzip()), crlfDelay: Infinity })
...
function parseArgs(argv) {
  ...
  if (!options.help && !options.itisSqlitePath) throw new Error('Usage: node scripts/build-itis-opalozoa-sidecar.mjs --itis-sqlite <verified ITIS.sqlite>')
  return options
}
...
function loadItis(sqlitePath) {
  ...
  try {
    ...
    const nearRootCandidates = database.prepare(`SELECT u.tsn, l.completename AS scientific_name, r.rank_name, u.name_usage, u.parent_tsn
      FROM taxonomic_units u JOIN longnames l ON l.tsn = u.tsn
      JOIN taxon_unit_types r ON r.kingdom_id = u.kingdom_id AND r.rank_id = u.rank_id
      WHERE lower(trim(l.completename)) IN ('opalinata') ORDER BY u.tsn`).all()
...
async function main() {
  ...
  if (options.help) { console.log('Usage: node scripts/build-itis-opalozoa-sidecar.mjs --itis-sqlite <verified ITIS.sqlite>'); return }
  ...
  for (const name of readdirSync(outputRoot)) if (/^itis-opalozoa-(?:sidecar|upstream-only)-\d{4}\.jsonl\.gz$/u.test(name)) rmSync(join(outputRoot, name))
  const upstreamPath = join(outputRoot, 'itis-opalozoa-upstream-only-0000.jsonl.gz')
  ...
  const scope = {
    ...
    boundary: `This boundary audit contains no ${targetName} rows because neither COL26.8 nor ITIS 2026-08-26 exposes an exact accepted root named ${targetName}. The nearby ITIS name Opalinata (TSN 43846) is recorded only as audit evidence and is not substituted.`,
  }
  const rootBoundaryAudit = {
    ...
    overlapAudit: { inspectedScopes: ['Bigyra', 'Oomycota', 'Ochrophyta', 'Cercozoa', 'Ciliophora', 'Dinoflagellata', 'Apicomplexa', 'Amoebozoa', 'Euglenozoa', 'Haptophyta', 'Rhodophyta', 'Cryptophyta', 'Choanoflagellatea', 'Perkinsozoa', 'Labyrinthulomycetes'], overlappingColUsageIds: [], overlappingItisTsns: [] },
    decision: `No exact ${targetName} root exists in either pinned authority. The nearby accepted/valid ITIS names are not taxonomic substitutes; no COL or ITIS range is asserted.`,
  }
  ...
  const ledger = {
    schemaVersion: 1, importType: 'COL26.8-to-ITIS-exact-opalozoa-root-boundary-audit',
