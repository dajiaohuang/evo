...
function profileRangeLabel(profile, language) {
  if (profile.rangeEvidenceLevel === 'withheld-no-range-evidence') return language === 'zh' ? '区间暂无可发布证据' : 'Range unavailable'
  return `${profile.firstAppearance}–${profile.lastAppearance || (language === 'zh' ? '现今' : 'Present')} Ma`
}
...
function pageHtml({ language, title, description, path, alternatePath, type = 'WebPage', robots = 'index,follow', jsonLd = {}, breadcrumbs = [], body, mapControls = false, directoryControls = false }) {
  ...
  const crumbHtml = breadcrumbs.map((entry, index) => {
    let href = entry.url.replace(origin, '')
    if (staticPages) {
      const collection = /\/#\/(catalog|events|stories)$/.exec(href)?.[1]
      if (collection) href = `${languageRoot}${collection === 'catalog' ? 'taxa' : collection}/`
      else if (href === `${basePath}/`) href = languageRoot
    }
    return `${index ? ' / ' : ''}<a href="${escapeHtml(href)}">${escapeHtml(entry.label)}</a>`
  }).join('')
  const navigation = staticPages
    ? `<a href="${languageRoot}">${text.home}</a><a href="${languageRoot}map/">${language === 'zh' ? '地图' : 'Map'}</a><a href="${languageRoot}taxa/">${text.catalog}</a><a href="${languageRoot}stories/">${text.stories}</a><a href="${languageRoot}methods/">${text.methods}</a><a href="${languageRoot}apps/">${language === 'zh' ? '应用' : 'Apps'}</a>`
...
<body><a class="skip-link" href="#content" tabindex="0">${language === 'zh' ? '跳到正文' : 'Skip to content'}</a><header class="site"><a href="${staticPages ? languageRoot : `${basePath}/`}">EVO ATLAS</a><nav aria-label="${language === 'zh' ? '主导航' : 'Main navigation'}">${navigation}</nav><span class="language"><a lang="en" href="${(language === 'en' ? url : alternateUrl).replace(origin, '')}">EN</a><a lang="zh-CN" href="${(language === 'zh' ? url : alternateUrl).replace(origin, '')}">中文</a></span></header><main class="page" id="content">${crumbHtml ? `<div class="crumbs" role="navigation" aria-label="${text.breadcrumbs}">${crumbHtml}</div>` : ''}${renderedBody}</main><footer>EVO ATLAS / ${escapeHtml(manifest.datasetVersion)} · ${staticPages ? 'Static reading edition' : 'Static-first'} · Source-aware · Open data</footer></body></html>`
}
...
function loadChineseTranslations() {
  ...
  for (const [fileName, dictionaryName, scriptKind] of [['baseChinese.ts', 'zh', ts.ScriptKind.TS], ['marineZh.ts', 'marineZh', ts.ScriptKind.TS], ['cetartiodactylaZh.ts', 'cetartiodactylaZh', ts.ScriptKind.TS], ['carnivoraZh.ts', 'carnivoraZh', ts.ScriptKind.TS], ['turtleLepidosaurZh.ts', 'turtleLepidosaurZh', ts.ScriptKind.TS], ['crocBirdZh.ts', 'crocBirdZh', ts.ScriptKind.TS], ['primatesZh.ts', 'primatesZh', ts.ScriptKind.TS], ['otherMammalsZh.ts', 'otherMammalsZh', ts.ScriptKind.TS], ['dinosaurZh.ts', 'dinosaurZh', ts.ScriptKind.TS], ['spongesCnidariansZh.ts', 'spongesCnidariansZh', ts.ScriptKind.TS], ['molluscsBrachiopodsZh.ts', 'molluscsBrachiopodsZh', ts.ScriptKind.TS], ['trilobitesCheliceratesZh.ts', 'trilobitesCheliceratesZh', ts.ScriptKind.TS], ['crustaceansInsectsZh.ts', 'crustaceansInsectsZh', ts.ScriptKind.TS]]) {
...
for (const story of stories) {
  ...
  for (const language of ['en', 'zh']) {
    ...
    const body = `<span class="eyebrow">${text.stories} / ${story.durationMinutes} ${language === 'zh' ? '分钟' : 'min'}</span><h1>${escapeHtml(title)}</h1><p class="dek">${escapeHtml(description)}</p><p class="notice">${staticPages ? (language === 'zh' ? '每一步标明所依据的主张，引用列于下方。交互探索状态可在完整应用中打开；故事本身仍是编辑综合。' : 'Each step names its evidence claims, with references below. Interactive Explorer states are available in the full application; the narrative remains an editorial synthesis.') : (language === 'zh' ? '每一步都链接到主张与可复现的探索器状态，但故事本身仍是编辑综合。' : 'Every step links to claims and a reproducible Explorer state, while the narrative remains an editorial synthesis.')}</p><section><h2>${language === 'zh' ? '故事步骤' : 'Story sequence'}</h2>${steps}</section><section><h2>${text.references}</h2>${renderReferences(records, language)}</section><div class="actions"><a class="button" href="${basePath}/#/stories?id=${encodeURIComponent(story.id)}">${text.open} ↗</a></div>`
...
for (const trait of traitGroups) {
  ...
  for (const language of ['en', 'zh']) {
    ...
    const linkedProfiles = trait.profiles.map((profile) => `<li><a href="${basePath}/${language === 'zh' ? 'zh/' : ''}taxa/${encodeURIComponent(profile.treeNodeId)}/"><strong>${escapeHtml(language === 'zh' ? profile.commonNameZh : profile.commonName)}</strong><span><em>${escapeHtml(profile.scientificName)}</em> · ${escapeHtml(profileRangeLabel(profile, language))}</span></a></li>`).join('')
...
for (const asset of media) {
  ...
  for (const language of ['en', 'zh']) {
    ...
    const path = language === 'en' ? englishPath : chinesePath
    const alternatePath = language === 'en' ? chinesePath : englishPath
    const title = language === 'zh' ? (asset.titleZh ?? localize(language, asset.title)) : asset.title
    ...
    const rightsNotice = isInterpretive
      ...
      : (language === 'zh' ? '本目录记录不授予复用权；请在原始来源核验创作者、许可和上下文。' : 'This catalog record does not grant reuse rights; verify creator, license and context at the original source.')
    const image = assetUrl ? `<img class="hero-media" src="${escapeHtml(assetUrl)}" width="1280" height="800" loading="eager" alt="${escapeHtml(language === 'zh' ? asset.altTextZh : asset.altText)}">` : ''
    const uncertainty = isInterpretive ? `<section><h2>${language === 'zh' ? '解释边界与不确定性' : 'Interpretive boundary and uncertainty'}</h2><p class="notice"><strong>${escapeHtml(rightsNotice)}</strong><br>${escapeHtml(language === 'zh' ? asset.uncertaintyZh : asset.uncertainty)}</p></section>` : ''
    const body = `<span class="eyebrow">${text.catalog} / ${language === 'zh' ? '媒体' : 'media'}</span><h1>${escapeHtml(title)}</h1><p class="dek">${escapeHtml(description)}</p>${image}<aside class="status"><strong>${escapeHtml(localize(language, asset.rightsStatus))}</strong><div class="pills"><span class="pill ${asset.rightsStatus === 'cleared-for-reuse' ? 'good' : 'warn'}">${escapeHtml(localize(language, asset.license))}</span></div><p>${escapeHtml(localize(language, asset.licenseNote))}</p></aside><div class="facts"><div><small>${language === 'zh' ? '类型' : 'Type'}</small><strong>${escapeHtml(localize(language, asset.type))}</strong></div><div><small>${language === 'zh' ? '创作者' : 'Creator'}</small><strong>${escapeHtml(localize(language, asset.creator))}</strong></div><div><small>${language === 'zh' ? '主题范围' : 'Subject scope'}</small><strong>${escapeHtml(asset.subjectScope)}</strong></div></div>${uncertainty}<section><h2>${language === 'zh' ? '权利与替代文本' : 'Rights and alternative text'}</h2><p>${escapeHtml(language === 'zh' ? asset.altTextZh : asset.altText)}</p>${isInterpretive ? '' : `<p class="notice">${escapeHtml(rightsNotice)}</p>`}</section><div class="actions"><a class="button" href="${escapeHtml(asset.sourceUrl)}" rel="noreferrer">${isInterpretive ? (language === 'zh' ? '生成记录、许可与哈希' : 'Generation record, licence and hashes') : (language === 'zh' ? '打开原始来源' : 'Open original source')} ↗</a><a class="button secondary" href="${basePath}/${language === 'zh' ? 'zh/' : ''}taxa/${encodeURIComponent(asset.taxonId)}/">${language === 'zh' ? '查看关联类群' : 'View linked taxon'} ↗</a><a class="button secondary" href="${escapeHtml(issueUrl({ entityId: `media:${asset.id}`, pageUrl: canonicalUrl }))}">${text.report} ↗</a></div>`
    write(`${path}index.html`, pageHtml({ language, title, description, path, alternatePath, type: 'CreativeWork', jsonLd: { identifier: asset.id, creator: asset.creator, license: asset.license, conditionsOfAccess: asset.rightsStatus, isBasedOn: asset.sourceUrl, about: asset.subjectScope, dateModified: asset.reviewedAt, ...(assetUrl ? { contentUrl: `${origin}${assetUrl}` } : {}) }, breadcrumbs: [{ label: text.home, url: `${basePath}/` }, { label: language === 'zh' ? '媒体' : 'Media', url: `${basePath}/${language === 'zh' ? 'zh/' : ''}media/` }, { label: title, url: `${baseUrl}/${path}` }], body }))
    mediaPageCount += 1
  }
}
...
for (const language of ['en', 'zh']) {
  ...
  if (staticPages) body = `<span class="eyebrow">${text.methods}</span><h1>${escapeHtml(title)}</h1><p class="dek">${language === 'zh' ? '本站在构建时从版本化来源生成可直接阅读的 HTML，保留原有主张、引用、审阅状态和局限。本站提供可切换年代的轻量地图；完整地形、多图层、数据查询与本地 SQL 属于完整应用，不在此静态阅读版运行。' : 'This site generates readable HTML from versioned sources at build time, preserving claims, citations, review status and limitations. This edition includes an age-selectable lightweight map. Full terrain, additional layers, data queries and local SQL belong to the full application.'}</p><section><h2>${text.limitations}</h2><ol>${manifest.limitations.map((limitation) => `<li>${escapeHtml(localize(language, limitation))}</li>`).join('')}</ol></section><div class="actions"><a class="button" href="${repositoryUrl}/blob/main/docs/data-methods.md">${language === 'zh' ? '完整方法文档' : 'Full methods documentation'}</a><a class="button secondary" href="${basePath}/${language === 'zh' ? 'zh/' : ''}apps/">${language === 'zh' ? '完整应用' : 'Full applications'}</a></div>`
...
const pageCounts = {
  ...(staticPages ? { home: 2, apps: 2, maps: readingMapCount } : {}),
  taxa: taxonPageCount,
