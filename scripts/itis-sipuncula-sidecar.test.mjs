...
const descriptorPath = join(packRoot, 'itis-sipuncula-sidecar.json')
const ledgerPath = join(root, 'data', 'sources', 'itis-sipuncula-sidecar-import-ledger.json')
...
describe('ITIS Sipuncula exact sidecar shards', () => {
  ...
  it('pins the complete strict COL26.8 Sipuncula partition', () => {
    expect(descriptor.packageId).toBe('other-animals')
    expect(descriptor.scope.colRootUsageId).toBe('63')
    expect(descriptor.scope.colRootScientificName).toBe('Sipuncula')
    expect(descriptor.scope.colStrictAcceptedSpecies).toBe(146)
    expect(descriptor.scope.packageStrictAcceptedSpecies).toBe(99161)
    expect(descriptor.scope.packageOutOfScopeStrictAcceptedSpecies).toBe(99015)
    expect(descriptorBytes.length).toBeLessThan(64 * 1024)
    expect(rows).toHaveLength(146)
    expect(new Set(rows.map((record) => record.colUsageId)).size).toBe(146)
  ...
  it('checks descriptor, ledger and delivery-boundary hashes', () => {
    expect(descriptor.counts).toMatchObject({ total: 146, accepted: 146, synonymCurrentNameRedirect: 0, ambiguous: 0, unmatched: 0, itisCurrentSpecies: 205, itisSpeciesSynonymLinks: 575, itisUpstreamOnly: 59 })
    ...
    expect(ledger.scopeAudit.itisRoot).toEqual({ tsn: '154520', scientificName: 'Sipuncula', rank: 'Phylum', usage: 'valid' })
