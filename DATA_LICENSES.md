...
| Paleogeographic geometry | not bundled | The legacy, provenance-unknown GeoJSON snapshots were removed in dataset `2026.08-m2` | Paleogeographic geometry is not included in the current `2026.08-static-v4-rc1` deployment. Legacy repository history may still contain the withdrawn provenance-unknown snapshots and must not be treated as licensed project data. A future snapshot must record source dataset/version/URL, license, attribution, processing script/commit, reconstruction age/model and geometry checksum before release. |
