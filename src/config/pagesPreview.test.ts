...
describe('Pages preview edition policy', () => {
  ...
  it('keeps core and selected resource routes, while closing full directories/tools', () => {
    ...
    expect(isPreviewRouteLocked('explore', new URLSearchParams('taxon=primates'), true)).toBe(false)
    ...
    expect(isPreviewRouteLocked('stories', new URLSearchParams('id=primates-evidence-without-an-ancestor-ladder'), true)).toBe(false)
  ...
  it('does not allow a non-selected Explorer entity to bypass the package boundary', () => {
    ...
    expect(isPreviewTaxonAllowed('primates')).toBe(true)
    expect(isPreviewTaxonAllowed('canis_lupus')).toBe(false)
    ...
    expect(isPreviewStoryAllowed('primates-evidence-without-an-ancestor-ladder')).toBe(true)
