...
import type { ItisNomenclatureRecord } from '../../data-client/types'
...
function EvidencePanel({ config, colId, packageId, zh }: CatalogueItisEvidenceProps) {
    ...
    try {
      ...
      if (candidate.delivery.completeRows && candidate.delivery.profile === 'native-full') {
        const result = packageId === 'other-animals'
          ? await loadCatalogueItisOtherAnimalsRecord(config.scope as never, colId)
          : await loadCatalogueItisProtistsRecord(config.scope as never, colId)
        if (requestId !== requestRef.current) return
  ...
  return <details open={open} className="catalogue-authority-disclosure" onToggle={(event) => {
    ...
    if (!loading && (!extension || failed)) void load()
  }}>
    ...
    {open && <div className="catalogue-source-card">
      ...
      {extension && <>
        ...
        {extension.evidenceBoundary && <p>{extension.evidenceBoundary}</p>}
        <dl>
        ...
        {extension.delivery.completeRows && !loading && !failed && <RecordDetail record={record} zh={zh} />}
      </>}
    </div>}
  </details>
}
